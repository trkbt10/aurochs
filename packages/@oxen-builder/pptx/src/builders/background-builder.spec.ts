...
describe("applyBackground", () => {
  it("applies solid color background from hex string", () => {
    ...
    const cSld = (sld as { children: readonly unknown[] }).children[0] as { name: string; children: readonly unknown[] };
  ...
  it("replaces existing background", () => {
    ...
    const sld = result.children[0]!;
    const cSldResult = (sld as { children: readonly unknown[] }).children[0] as { name: string; children: readonly unknown[] };
    // Should have exactly one p:bg
    const bgChildren = cSldResult.children.filter(
      (c) => (c as { name?: string }).name === "p:bg",
    );
