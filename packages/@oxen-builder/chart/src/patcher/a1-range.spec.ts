...
describe("createRange", () => {
  it("creates column range", () => {
    const range = createRange({ startCol: "A", startRow: 2, itemCount: 5, direction: "column" });
  ...
  it("creates row range", () => {
    const range = createRange({ startCol: "B", startRow: 1, itemCount: 5, direction: "row" });
  ...
  it("uses absolute references by default", () => {
    const range = createRange({ startCol: "A", startRow: 1, itemCount: 3, direction: "column" });
    expect(range.start.colAbsolute).toBe(true);
    expect(range.start.rowAbsolute).toBe(true);
  });

  it("supports relative references", () => {
    const range = createRange({ startCol: "A", startRow: 1, itemCount: 3, direction: "column", absolute: false });
