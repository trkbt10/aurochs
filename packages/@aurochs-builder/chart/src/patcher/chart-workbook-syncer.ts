 ...
 * ```
 */
export function syncChartToWorkbook(workbook: XlsxWorkbook, chartData: ChartDataSpec): XlsxWorkbook {
 ...
 */
function updateWorksheetWithChartData(worksheet: XlsxWorksheet, chartData: ChartDataSpec): XlsxWorksheet {
  ...
  for (let i = 0; i < series.length; i++) {
    headerCells.push(createCell(i + 2, 1, { type: "string", value: series[i].name }));
  }
  ...
  for (let rowIdx_ = 0; rowIdx_ < categories.length; rowIdx_++) {
    ...
    // Column A: category
    rowCells.push(createCell(1, rowNumber, { type: "string", value: categories[rowIdx_] }));
 ...
 */
function getCellFromWorksheet(worksheet: XlsxWorksheet, col: number, row: number): Cell | undefined {
