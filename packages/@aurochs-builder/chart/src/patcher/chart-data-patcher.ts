...
function serializePtList(ptName: "c:pt", values: readonly string[]): readonly XmlElement[] {
  return values.map((value, idx) =>
    createElement(ptName, { idx: String(idx) }, [createElement("c:v", {}, [createText(value)])]),
  );
}

function patchCache(cache: XmlElement, values: readonly string[]): XmlElement {
...
function patchContainerSeries(container: XmlElement, data: ChartDataSpec): XmlElement {
...
function patchAllSeriesCategories(chartSpace: XmlElement, categories: readonly string[]): XmlElement {
