 ...
 */
export function serializeNumFmts(numberFormats: readonly XlsxNumberFormat[]): XmlElement | undefined {
  // Filter to only custom formats (ID >= 164)
  const customFormats = numberFormats.filter((format) => (format.numFmtId as number) >= FIRST_CUSTOM_FORMAT_ID);
