...
import { numFmtId } from "@aurochs-office/xlsx/domain/types";
import type { XlsxNumberFormat } from "@aurochs-office/xlsx/domain/style/number-format";
import { BUILTIN_NUMBER_FORMATS } from "@aurochs-office/xlsx/domain/style/number-format";
...
describe("serializeNumFmt", () => {
  ...
  it("should escape special characters when serialized to XML string", () => {
    ...
    expect(xml).toBe('<numFmt numFmtId="172" formatCode="&quot;$&quot;#,##0.00 &amp; [&lt;100]"/>');
  });
  ...
  it("should produce valid XML with escaped characters", () => {
    ...
    expect(xml).toBe('<numFmt numFmtId="164" formatCode="#,##0.00 &quot;円&quot;"/>');
  });
});
...
describe("serializeNumFmts", () => {
  ...
  it("should serialize single custom format", () => {
    const formats: XlsxNumberFormat[] = [{ numFmtId: numFmtId(164), formatCode: "#,##0.00" }];
  ...
  it("should treat ID 163 as built-in (last built-in ID)", () => {
    const formats: XlsxNumberFormat[] = [{ numFmtId: numFmtId(163), formatCode: "some-format" }];
  ...
  it("should treat ID 164 as custom (first custom ID)", () => {
    const formats: XlsxNumberFormat[] = [{ numFmtId: numFmtId(164), formatCode: "some-format" }];
