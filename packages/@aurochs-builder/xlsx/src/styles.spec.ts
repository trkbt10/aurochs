...
import type {
  ...
  XlsxAlignment,
  XlsxProtection,
} from "@aurochs-office/xlsx/domain/style/types";
import { createDefaultStyleSheet } from "@aurochs-office/xlsx/domain/style/types";
import { numFmtId, fontId, fillId, borderId } from "@aurochs-office/xlsx/domain/types";
...
describe("serializeAlignment", () => {
  ...
  it("should serialize all horizontal alignment types", () => {
    const types = ["left", "center", "right", "fill", "justify", "centerContinuous", "distributed"] as const;
...
describe("serializeCellXfs", () => {
  ...
  it("should serialize single cellXf", () => {
    ...
    expect(result).toBe('<cellXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellXfs>');
  });
...
describe("serializeCellStyleXfs", () => {
  ...
  it("should serialize single cellStyleXf", () => {
    ...
    expect(result).toBe('<cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs>');
  });
...
describe("serializeCellStyles", () => {
  ...
  it("should serialize single cellStyle", () => {
    ...
    expect(result).toBe('<cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles>');
  });
...
describe("serializeStyleSheet", () => {
  ...
  it("should serialize complex stylesheet with multiple entries", () => {
    const styleSheet: XlsxStyleSheet = {
      ...
      cellStyleXfs: [{ numFmtId: numFmtId(0), fontId: fontId(0), fillId: fillId(0), borderId: borderId(0) }],
...
describe("round-trip compatibility", () => {
  ...
  it("should produce XML that matches expected format for cellXf with alignment", () => {
    ...
    const result = serializeElement(serializeCellXf(xf));
    expect(result).toContain('<alignment horizontal="center" vertical="center" wrapText="1"/>');
  });
