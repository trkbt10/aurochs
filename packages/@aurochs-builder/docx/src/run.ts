...
export function serializeRunFonts(fonts: DocxRunFonts): XmlElement {
  ...
  if (fonts.ascii) {
    attrs["w:ascii"] = fonts.ascii;
  }
  if (fonts.hAnsi) {
    attrs["w:hAnsi"] = fonts.hAnsi;
  }
  if (fonts.eastAsia) {
    attrs["w:eastAsia"] = fonts.eastAsia;
  }
  if (fonts.cs) {
    attrs["w:cs"] = fonts.cs;
  }
  if (fonts.asciiTheme) {
    attrs["w:asciiTheme"] = fonts.asciiTheme;
  }
  if (fonts.hAnsiTheme) {
    attrs["w:hAnsiTheme"] = fonts.hAnsiTheme;
  }
  if (fonts.eastAsiaTheme) {
    attrs["w:eastAsiaTheme"] = fonts.eastAsiaTheme;
  }
  if (fonts.csTheme) {
    attrs["w:cstheme"] = fonts.csTheme;
  }
...
export function serializeColor(color: DocxColor, elementName: string = "w:color"): XmlElement {
  ...
  if (color.val) {
    attrs["w:val"] = color.val;
  }
  if (color.themeColor) {
    attrs["w:themeColor"] = color.themeColor;
  }
  if (color.themeTint !== undefined) {
    attrs["w:themeTint"] = String(color.themeTint);
  }
  if (color.themeShade !== undefined) {
    attrs["w:themeShade"] = String(color.themeShade);
  }
...
export function serializeShading(shading: DocxShading, elementName: string = "w:shd"): XmlElement {
  ...
  if (shading.themeColor) {
    attrs["w:themeColor"] = shading.themeColor;
  }
  if (shading.themeFill) {
    attrs["w:themeFill"] = shading.themeFill;
  }
...
export function serializeUnderline(underline: DocxUnderline): XmlElement {
  ...
  if (underline.val) {
    attrs["w:val"] = underline.val;
  }
  if (underline.color) {
    attrs["w:color"] = underline.color;
  }
  if (underline.themeColor) {
    attrs["w:themeColor"] = underline.themeColor;
  }
...
export function serializeRunProperties(props: DocxRunProperties | undefined): XmlElement | undefined {
  ...
  // Font properties
  if (props.rFonts) {
    children.push(serializeRunFonts(props.rFonts));
  }
  ...
  // Color
  if (props.color) {
    children.push(serializeColor(props.color));
  }
  ...
  // Underline
  if (props.u) {
    children.push(serializeUnderline(props.u));
  }

  // Border
  if (props.bdr) {
    children.push(serializeRunBorder(props.bdr));
  }

  // Shading
  if (props.shd) {
    children.push(serializeShading(props.shd));
  }
  ...
  if (props.eastAsianLayout) {
    ...
    if (props.eastAsianLayout.combine !== undefined) {
      eaAttrs["w:combine"] = props.eastAsianLayout.combine ? "1" : "0";
    }
    if (props.eastAsianLayout.combineBrackets) {
      eaAttrs["w:combineBrackets"] = props.eastAsianLayout.combineBrackets;
    }
    if (props.eastAsianLayout.vert !== undefined) {
      eaAttrs["w:vert"] = props.eastAsianLayout.vert ? "1" : "0";
    }
    if (props.eastAsianLayout.vertCompress !== undefined) {
      eaAttrs["w:vertCompress"] = props.eastAsianLayout.vertCompress ? "1" : "0";
    }
...
function serializeBreak(content: { type: "break"; breakType?: string; clear?: string }): XmlElement {
  ...
  if (content.breakType) {
    attrs["w:type"] = content.breakType;
  }
  if (content.clear) {
    attrs["w:clear"] = content.clear;
  }
...
function serializeSymbol(content: { type: "symbol"; font?: string; char?: string }): XmlElement {
  const attrs: Record<string, string> = {};
  if (content.font) {
    attrs["w:font"] = content.font;
  }
  if (content.char) {
    attrs["w:char"] = content.char;
  }
