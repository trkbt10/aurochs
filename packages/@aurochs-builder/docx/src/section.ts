...
export function serializePageSize(pgSz: DocxPageSize): XmlElement {
  ...
  if (pgSz.w !== undefined) {
    attrs["w:w"] = String(pgSz.w);
  }
  if (pgSz.h !== undefined) {
    attrs["w:h"] = String(pgSz.h);
  }
  if (pgSz.orient) {
    attrs["w:orient"] = pgSz.orient;
  }
  if (pgSz.code !== undefined) {
    attrs["w:code"] = String(pgSz.code);
  }
...
export function serializePageMargins(pgMar: DocxPageMargins): XmlElement {
  ...
  if (pgMar.top !== undefined) {
    attrs["w:top"] = String(pgMar.top);
  }
  if (pgMar.right !== undefined) {
    attrs["w:right"] = String(pgMar.right);
  }
  if (pgMar.bottom !== undefined) {
    attrs["w:bottom"] = String(pgMar.bottom);
  }
  if (pgMar.left !== undefined) {
    attrs["w:left"] = String(pgMar.left);
  }
  if (pgMar.header !== undefined) {
    attrs["w:header"] = String(pgMar.header);
  }
  if (pgMar.footer !== undefined) {
    attrs["w:footer"] = String(pgMar.footer);
  }
  if (pgMar.gutter !== undefined) {
    attrs["w:gutter"] = String(pgMar.gutter);
  }
...
export function serializePageBorders(pgBorders: DocxPageBorders): XmlElement {
  ...
  if (pgBorders.zOrder) {
    attrs["w:zOrder"] = pgBorders.zOrder;
  }
  if (pgBorders.display) {
    attrs["w:display"] = pgBorders.display;
  }
  if (pgBorders.offsetFrom) {
    attrs["w:offsetFrom"] = pgBorders.offsetFrom;
  }

  const children: XmlNode[] = [];

  if (pgBorders.top) {
    children.push(serializeBorderEdge(pgBorders.top, "top"));
  }
  if (pgBorders.left) {
    children.push(serializeBorderEdge(pgBorders.left, "left"));
  }
  if (pgBorders.bottom) {
    children.push(serializeBorderEdge(pgBorders.bottom, "bottom"));
  }
  if (pgBorders.right) {
    children.push(serializeBorderEdge(pgBorders.right, "right"));
  }
...
function serializeColumn(col: DocxColumn): XmlElement {
  ...
  if (col.w !== undefined) {
    attrs["w:w"] = String(col.w);
  }
  if (col.space !== undefined) {
    attrs["w:space"] = String(col.space);
  }
...
export function serializeLineNumbering(lnNumType: DocxLineNumbering): XmlElement {
  ...
  if (lnNumType.countBy !== undefined) {
    attrs["w:countBy"] = String(lnNumType.countBy);
  }
  if (lnNumType.start !== undefined) {
    attrs["w:start"] = String(lnNumType.start);
  }
  if (lnNumType.distance !== undefined) {
    attrs["w:distance"] = String(lnNumType.distance);
  }
  if (lnNumType.restart) {
    attrs["w:restart"] = lnNumType.restart;
  }
...
export function serializePageNumberType(pgNumType: DocxPageNumberType): XmlElement {
  ...
  if (pgNumType.fmt) {
    attrs["w:fmt"] = pgNumType.fmt;
  }
  if (pgNumType.start !== undefined) {
    attrs["w:start"] = String(pgNumType.start);
  }
  if (pgNumType.chapStyle !== undefined) {
    attrs["w:chapStyle"] = String(pgNumType.chapStyle);
  }
  if (pgNumType.chapSep) {
    attrs["w:chapSep"] = pgNumType.chapSep;
  }
...
export function serializeDocGrid(docGrid: DocxDocGrid): XmlElement {
  ...
  if (docGrid.type) {
    attrs["w:type"] = docGrid.type;
  }
  if (docGrid.linePitch !== undefined) {
    attrs["w:linePitch"] = String(docGrid.linePitch);
  }
  if (docGrid.charSpace !== undefined) {
    attrs["w:charSpace"] = String(docGrid.charSpace);
  }
