 ...
 *
 * @see @aurochs/zip - Shared ZIP abstraction
 */
...
function applySlideEditsWithMedia({
  ...
  slidePath,
  pkg,
}: ApplySlideEditsWithMediaOptions): XmlDocument {
 ...
 */
function processOleGraphicFrame(frame: GraphicFrame, slidePath: string, pkg: ZipPackage): GraphicFrame {
...
function collectMasterLayoutThemeUpdates(doc: PresentationDocument): CollectedUpdates {
  ...
  return {
    layoutUpdates: Array.from(layoutMap.entries()).map(([layoutPath, layoutXml]) => ({ layoutPath, layoutXml })),
    masterUpdates: Array.from(masterMap.entries()).map(([masterPath, masterXml]) => ({ masterPath, masterXml })),
    themeUpdates: Array.from(themeMap.entries()).map(([themePath, themeXml]) => ({ themePath, themeXml })),
  };
}
