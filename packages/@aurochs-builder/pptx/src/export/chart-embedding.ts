...
import type { ChartDataUpdate } from "@aurochs-office/chart/reader";
import { resolveEmbeddedXlsxPath } from "@aurochs-office/chart/reader";
import { syncChartToWorkbook } from "@aurochs-builder/chart/patcher";
...
export async function updateEmbeddedXlsx({
  ...
  chartPath,
  chartData,
}: UpdateEmbeddedXlsxOptions): Promise<void> {
  ...
  try {
    ...
    const xlsxGetFile = async (path: string): Promise<string | undefined> => pkg.readText(path) ?? undefined;
...
export async function syncAllChartEmbeddings(
  ...
  for (const [chartPath, chartData] of chartUpdates) {
    updatePromises.push(updateEmbeddedXlsx({ getFileContent, setFileContent, chartPath, chartData }));
  }
 ...
 * ```
 */
export function listEmbeddedXlsx(pptxFileList: readonly string[]): readonly string[] {
  return pptxFileList.filter((path) => path.toLowerCase().endsWith(".xlsx") && path.includes("embeddings/"));
}
