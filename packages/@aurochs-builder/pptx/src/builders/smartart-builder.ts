...
import { parseXml, serializeDocument, getChildren, isXmlElement } from "@aurochs/xml";
import type { ZipPackage } from "@aurochs/zip";
import { patchDiagram, type DiagramChange, type DiagramFiles } from "@aurochs-builder/pptx/patcher";
...
function findDiagramPaths(
  ...
  const pathsFromRels = rels.reduce(
    ...
    },
    {
      ...
      quickStyle: null as string | null,
    },
  );
 ...
 */
export function applySmartArtUpdates(pkg: ZipPackage, slidePath: string, specs: readonly SmartArtUpdateSpec[]): void {
  if (specs.length === 0) {
    return;
  }
