...
import { createElement, createText, type XmlElement, type XmlDocument } from "@aurochs/xml";
import {
  setAttribute,
  setAttributes,
  removeAttribute,
...
describe("replaceChild", () => {
  it("replaces first child matching predicate", () => {
    ...
    const result = replaceChild(parent, (child) => (child as XmlElement).name === "target", newChild);
  ...
  it("returns unchanged if no match", () => {
    ...
    const result = replaceChild(parent, (child) => (child as XmlElement).name === "nonexistent", newChild);
...
describe("setChildren", () => {
  it("replaces all children", () => {
    const parent = createTestElement("parent", {}, [createTestElement("old")]);
...
describe("updateChildByName", () => {
  it("updates child with given name using updater function", () => {
    ...
    const result = updateChildByName(parent, "a:off", (el) => setAttribute(el, "x", "100"));
...
describe("getShapeIds", () => {
  it("returns all shape IDs from spTree", () => {
    ...
    const sp2 = createTestElement("p:sp", {}, [
      createTestElement("p:nvSpPr", {}, [createTestElement("p:cNvPr", { id: "3" })]),
    ]);
    const pic = createTestElement("p:pic", {}, [
      createTestElement("p:nvPicPr", {}, [createTestElement("p:cNvPr", { id: "4" })]),
    ]);
  ...
  it("includes IDs from nested groups", () => {
    const innerSp = createTestElement("p:sp", {}, [
      createTestElement("p:nvSpPr", {}, [createTestElement("p:cNvPr", { id: "5" })]),
    ]);
    const grpSp = createTestElement("p:grpSp", {}, [
      createTestElement("p:nvGrpSpPr", {}, [createTestElement("p:cNvPr", { id: "4" })]),
      innerSp,
    ]);
    const spTree = createTestElement("p:spTree", {}, [grpSp]);
...
describe("updateAtPath", () => {
  it("updates element at path", () => {
    ...
    const result = updateAtPath(root, ["p:spPr", "a:xfrm", "a:off"], (el) => setAttribute(el, "x", "100"));

    const updatedOff = ((result.children[0] as XmlElement).children[0] as XmlElement).children[0] as XmlElement;
