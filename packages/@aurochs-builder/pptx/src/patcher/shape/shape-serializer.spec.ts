...
import type { SpShape, GrpShape, PicShape, CxnShape, GraphicFrame } from "@aurochs-office/pptx/domain/shape";
...
describe("shape-serializer", () => {
  ...
  it("serializes a text shape", () => {
    ...
    expect(textNode && !isXmlElement(textNode) ? (textNode as { type: "text"; value: string }).value : undefined).toBe(
      "Hello",
    );
  });
