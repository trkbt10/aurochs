...
export function addOverride(contentTypesXml: XmlDocument, partName: string, contentType: string): XmlDocument {
  if (!partName) {
...
export function removeUnusedContentTypes(contentTypesXml: XmlDocument, usedParts: readonly string[]): XmlDocument {
  const root = getDocumentRoot(contentTypesXml);
  if (!root || root.name !== "Types") {
...
function findOverride(typesEl: XmlElement, partName: string): XmlElement | undefined {
  return typesEl.children.find(
    (child): child is XmlElement =>
      isXmlElement(child) && child.name === "Override" && child.attrs.PartName === partName,
  );
}
