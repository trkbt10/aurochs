...
import type { ParagraphProperties, TextLevelStyle, TextStyleLevels } from "@aurochs-office/pptx/domain";
...
function buildPatchedChildren(serialized: XmlElement, patch: ParagraphProperties): XmlElement[] {
  ...
  if (shouldPatchBulletStyle(patch)) {
    children.push(
      ...serialized.children.filter(
        (c): c is XmlElement => isXmlElement(c) && (BULLET_ELEMENT_NAMES as readonly string[]).includes(c.name),
      ),
    );
  }
...
function buildRemovalSet(patch: ParagraphProperties): ReadonlySet<string> {
  ...
  if (shouldPatchBulletStyle(patch)) {
    for (const name of BULLET_ELEMENT_NAMES) {
      remove.add(name);
    }
  }
 ...
 */
export function patchTextStyleLevelElement(existing: XmlElement, patch: ParagraphProperties): XmlElement {
  ...
  const keptChildren = existing.children.filter((c) => !(isXmlElement(c) && removal.has(c.name)));

  const toInsert = buildPatchedChildren(serialized, patch);

  const extLstIndex = keptChildren.findIndex((c) => isXmlElement(c) && c.name === "a:extLst");
...
export function patchTextStyleLevelByNumber(
  ...
  if (existing) {
    return upsertChildBeforeExtLst(styleElement, childName, patchTextStyleLevelElement(existing, patch));
  }
  const created = renameElement(serializeParagraphProperties(patch), childName);
