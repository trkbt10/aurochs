...
function serializeTableRow(row: TableRow): XmlElement {
  return createElement("a:tr", { h: ooxmlEmu(row.height) }, row.cells.map(serializeTableCell));
}
...
function patchMergeCell(cell: XmlElement, attrs: Readonly<Record<string, string | undefined>>): XmlElement {
