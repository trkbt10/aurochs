...
function findTxBodyIndex(shape: XmlElement): number {
  return shape.children.findIndex((c) => isXmlElement(c) && (c.name === "p:txBody" || c.name === "a:txBody"));
}
...
function removeTxBody(shape: XmlElement): XmlElement {
  return {
    ...shape,
    children: shape.children.filter((c) => !(isXmlElement(c) && (c.name === "p:txBody" || c.name === "a:txBody"))),
  };
}
