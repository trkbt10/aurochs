 ...
 */
export function patchSlideXml(slideXml: XmlDocument, changes: readonly ShapeChange[]): XmlDocument {
  return changes.reduce((doc, change) => applyShapeChange(doc, change), slideXml);
}

/** Apply a single shape change to a document */
function applyShapeChange(doc: XmlDocument, change: ShapeChange): XmlDocument {
  switch (change.type) {
    case "removed":
      return applyRemoval(doc, change.shapeId);
    case "added":
      return applyAddition(doc, change);
    case "modified":
      return applyModification(doc, change.shapeId, change.changes);
  }
}
...
function applyAddition(doc: XmlDocument, change: ShapeAdded): XmlDocument {
  ...
  return updateDocumentRoot(doc, (root) => {
    ...
    return updateChildByName(root, "p:cSld", (cSldEl) => replaceChildByName(cSldEl, "p:spTree", updatedSpTree));
  });
}
...
function applyRemoval(doc: XmlDocument, shapeId: string): XmlDocument {
  return updateDocumentRoot(doc, (root) => {
    ...
    // Update the tree
    return updateChildByName(root, "p:cSld", (cSldEl) => replaceChildByName(cSldEl, "p:spTree", newSpTree));
  });
}
 ...
 */
function applyModification(doc: XmlDocument, shapeId: string, changes: readonly PropertyChange[]): XmlDocument {
  return updateDocumentRoot(doc, (root) => {
    // Find p:cSld/p:spTree
    const cSld = getChild(root, "p:cSld");
    if (!cSld) {
    ...
    const modifiedShape = changes.reduce((currentShape, change) => applyPropertyChange(currentShape, change), shape);
 ...
 */
function applyPropertyChange(shape: XmlElement, change: PropertyChange): XmlElement {
 ...
 */
function applyTransformChange(shape: XmlElement, change: PropertyChange & { property: "transform" }): XmlElement {
  ...
  const spPr = shape.children.find((c): c is XmlElement => isXmlElement(c) && c.name === spPrName);
  ...
  const newSpPr = xfrm ? replaceChildByName(spPr, "a:xfrm", newXfrm) : prependXfrm(spPr, newXfrm);
...
function applyFillChange(shape: XmlElement, change: PropertyChange & { property: "fill" }): XmlElement {
  ...
  const fillNames = new Set(["a:noFill", "a:solidFill", "a:gradFill", "a:blipFill", "a:pattFill", "a:grpFill"]);

  const originalChildren = [...spPr.children];
  const firstFillIndex = originalChildren.findIndex((c) => isXmlElement(c) && fillNames.has(c.name));

  const keptChildren = originalChildren.filter((c) => !(isXmlElement(c) && fillNames.has(c.name)));
  ...
  const newSpPr = {
    ...spPr,
    children: [...keptChildren.slice(0, insertIndex), fillElement, ...keptChildren.slice(insertIndex)],
  };
...
function applyLineChange(shape: XmlElement, change: PropertyChange & { property: "line" }): XmlElement {
  ...
  const originalChildren = [...spPr.children];
  const existingLnIndex = originalChildren.findIndex((c) => isXmlElement(c) && c.name === "a:ln");

  const keptChildren = originalChildren.filter((c) => !(isXmlElement(c) && c.name === "a:ln"));
  ...
  const insertIndex = computeLineInsertIndex(existingLnIndex, originalChildren, keptChildren);

  const newSpPr = {
    ...spPr,
    children: [...keptChildren.slice(0, insertIndex), lnElement, ...keptChildren.slice(insertIndex)],
  };
...
function applyTextBodyChange(shape: XmlElement, change: PropertyChange & { property: "textBody" }): XmlElement {
...
function applyEffectsChange(shape: XmlElement, change: PropertyChange & { property: "effects" }): XmlElement {
  ...
  const originalChildren = [...spPr.children];
  const firstEffectIndex = originalChildren.findIndex((c) => isXmlElement(c) && effectNames.has(c.name));

  const keptChildren = originalChildren.filter((c) => !(isXmlElement(c) && effectNames.has(c.name)));
  ...
  const newSpPr = {
    ...spPr,
    children: [...keptChildren.slice(0, insertIndex), effectElement, ...keptChildren.slice(insertIndex)],
  };
...
function computeLineInsertIndex(
  existingLnIndex: number,
  originalChildren: readonly unknown[],
  keptChildren: readonly unknown[],
): number {
  if (existingLnIndex === -1) {
    return findInsertIndexByName(keptChildren, ["a:effectLst", "a:effectDag"]);
  }
  return originalChildren
    .slice(0, existingLnIndex)
    .filter((c) => !(isXmlElement(c) && (c as XmlElement).name === "a:ln")).length;
}
 ...
 */
function applyGeometryChange(shape: XmlElement, change: PropertyChange & { property: "geometry" }): XmlElement {
  ...
  const originalChildren = [...spPr.children];
  const existingGeomIndex = originalChildren.findIndex((c) => isXmlElement(c) && geometryNames.has(c.name));

  // Remove existing geometry elements
  const keptChildren = originalChildren.filter((c) => !(isXmlElement(c) && geometryNames.has(c.name)));
  ...
  if (!newGeometry) {
    ...
    const newSpPr = {
      ...spPr,
      children: [...keptChildren.slice(0, insertIndex), defaultGeom, ...keptChildren.slice(insertIndex)],
    };
    return replaceChildByName(shape, spPrName, newSpPr);
  }
  ...
  const newSpPr = {
    ...spPr,
    children: [...keptChildren.slice(0, insertIndex), geomElement, ...keptChildren.slice(insertIndex)],
  };
...
function applyBlipFillChange(shape: XmlElement, change: PropertyChange & { property: "blipFill" }): XmlElement {
  if (shape.name !== "p:pic") {
    return shape;
  }
...
export function hasShapes(doc: XmlDocument): boolean {
  ...
  return spTree.children.some(
    (child) => isXmlElement(child) && ["p:sp", "p:pic", "p:grpSp", "p:cxnSp", "p:graphicFrame"].includes(child.name),
  );
}

function ensureUniqueIdsForInsertion(shape: ShapeAdded["shape"], existingIds: readonly string[]): ShapeAdded["shape"] {
...
/** Resolve a connector connection ref, updating shape ID if mapped */
function resolveConnectionRef(
  connection: { readonly shapeId: string; readonly siteIndex: number } | undefined,
  hasNewId: boolean | undefined | null,
  idMap: ReadonlyMap<string, string>,
): typeof connection {
  if (hasNewId && connection) {
    return { ...connection, shapeId: idMap.get(connection.shapeId)! };
  }
  return connection;
}
