...
describe("serializeSlideTransition", () => {
  ...
  it("throws for unsupported attribute combinations", () => {
    expect(() => serializeSlideTransition({ type: "fade", direction: "r" } as SlideTransition)).toThrow(
      'direction is not supported for transition type "fade"',
    );

    expect(() => serializeSlideTransition({ type: "wipe", orientation: "vert" } as SlideTransition)).toThrow(
      'orientation is not supported for transition type "wipe"',
    );

    expect(() => serializeSlideTransition({ type: "fade", spokes: 4 } as SlideTransition)).toThrow(
      'spokes is not supported for transition type "fade"',
    );

    expect(() => serializeSlideTransition({ type: "fade", inOutDirection: "in" } as SlideTransition)).toThrow(
      'inOutDirection is not supported for transition type "fade"',
    );
  });
});
