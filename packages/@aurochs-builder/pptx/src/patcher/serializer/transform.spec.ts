...
describe("patchTransformElement", () => {
  ...
  it("replaces a:off/a:ext while preserving other children", () => {
    const existing = createExistingXfrm({ rot: "123", flipH: "1", foo: "bar" }, [
    ...
    const updated = patchTransformElement(
      existing,
      createTransform({ x: 96, y: 48, width: 192, height: 96, rotation: 45 }),
    );
  ...
  it("removes rot attribute when rotation becomes 0", () => {
    const existing = createExistingXfrm({ rot: "2700000" }, [
      createElement("a:off", { x: "1", y: "2" }),
      createElement("a:ext", { cx: "3", cy: "4" }),
    ]);
