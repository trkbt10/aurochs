...
function requireSldIdLst(root: XmlElement): XmlElement {
  const sldIdLst = root.children.find((c): c is XmlElement => isXmlElement(c) && c.name === "p:sldIdLst");
  if (!sldIdLst) {
...
function getSldIdElements(sldIdLst: XmlElement): XmlElement[] {
  return sldIdLst.children.filter((c): c is XmlElement => isXmlElement(c) && c.name === "p:sldId");
}

/**
 * Add a slide to <p:sldIdLst>.
 */
type AddSlideToListArgs = [presentationXml: XmlDocument, slideId: number, rId: string, position?: number];
...
export function addSlideToList(...args: AddSlideToListArgs): XmlDocument {
  ...
  return updateDocumentRoot(presentationXml, (root) => {
    ...
    nextSlides.splice(insertIndex, 0, createElement("p:sldId", { id: String(slideId), "r:id": rId }));
 ...
 */
export function reorderSlideInList(presentationXml: XmlDocument, slideId: number, newPosition: number): XmlDocument {
  if (!Number.isInteger(slideId) || slideId <= 0) {
