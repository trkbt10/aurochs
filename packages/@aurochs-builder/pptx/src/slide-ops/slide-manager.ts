...
function findSlidePartTarget(presentationRelsXml: XmlDocument, rId: string): string {
  ...
  if (rel.type !== RELATIONSHIP_TYPES.SLIDE) {
    throw new Error(`SlideManager: relationship ${rId} is not a slide relationship (Type=${rel.type})`);
  }
  return rel.target;
}
...
export async function addSlide(
  ...
  const updatedContentTypesXml = addOverride(contentTypesXml, `/ppt/slides/${slideFilename}.xml`, CONTENT_TYPES.SLIDE);
