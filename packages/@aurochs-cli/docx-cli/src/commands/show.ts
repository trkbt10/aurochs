...
function extractCellText(cell: DocxTable["rows"][number]["cells"][number]): string {
  return cell.content
    ...
    .map((p) =>
      p.content
        .map((c) => (c.type === "run" ? c.content.map((rc) => (rc.type === "text" ? rc.value : "")).join("") : ""))
        .join(""),
    )
    .join("\n");
}
...
export async function runShow(filePath: string, sectionNumber: number): Promise<Result<ShowData>> {
  try {
    ...
    const content = section.content.map(serializeBlockContent).filter((c): c is BlockContentJson => c !== undefined);
