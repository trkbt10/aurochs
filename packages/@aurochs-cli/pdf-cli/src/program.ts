/**
 * @file CLI program definition for pdf-cli
 */
...
/** Create the pdf CLI program with all registered commands. */
export function createProgram(): Command {
  const program = new Command();

  program
    .name("pdf")
    .description("CLI tool for inspecting PDF files")
    .version("0.1.0")
    .option("-o, --output <mode>", "Output mode (json|pretty|mermaid)", "pretty");

  program
    .command("info")
    .description("Display PDF metadata")
  ...
  program
    .command("build")
    .description("Build PdfDocument JSON from a build spec")
