...
export function dateToSerial(date: Date, dateSystem: XlsxDateSystem = "1900"): number {
  ...
  const serial = { value: (date.getTime() - epoch) / MILLISECONDS_PER_DAY };
  ...
  if (dateSystem === "1900" && serial.value > 59) {
    serial.value += 1;
  }
...
export function serialToDate(serial: number, dateSystem: XlsxDateSystem = "1900"): Date {
  ...
  const adjustedSerial = { value: dateSystem === "1904" ? serial + EXCEL_1904_TO_1900_DAY_OFFSET : serial };

  // Excel 1900 date system bug: serial 60 = 1900-02-29 (doesn't exist)
  // Excel treats 1900 as a leap year, so serials > 59 are off by 1 day
  if (dateSystem === "1900" && adjustedSerial.value > 59) {
    adjustedSerial.value -= 1;
  }

  const millisecondsOffset = Math.round(adjustedSerial.value * MILLISECONDS_PER_DAY);
