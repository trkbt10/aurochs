...
import { parseNumFmt, parseNumFmts, isCustomFormat, resolveFormatCode } from "./number-format";
...
describe("parseNumFmt", () => {
  ...
  it("should throw when numFmtId is missing", () => {
    ...
    expect(() => parseNumFmt(parseRoot(xml))).toThrow('Required attribute "numFmtId" is missing or invalid');
  });
...
describe("parseNumFmts", () => {
  ...
  it("should parse single numFmt element", () => {
    ...
    expect(result).toEqual([{ numFmtId: numFmtId(164), formatCode: "#,##0.00" }]);
  });
...
describe("resolveFormatCode", () => {
  ...
  describe("priority", () => {
    it("should prefer built-in over custom for same ID (hypothetical)", () => {
      ...
      const conflictingFormats: readonly XlsxNumberFormat[] = [{ numFmtId: numFmtId(0), formatCode: "CUSTOM" }];
