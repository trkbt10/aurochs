 ...
 * @see ECMA-376 Part 4, Section 18.8.32 (patternFill)
 */
export function parsePatternFill(patternFillElement: XmlElement): XlsxPatternFill {
  const patternType = (getAttr(patternFillElement, "patternType") ?? "none") as XlsxPatternType;
 ...
 * @see ECMA-376 Part 4, Section 18.8.24 (gradientFill)
 */
export function parseGradientFill(gradientFillElement: XmlElement): XlsxGradientFill {
  const gradientType = (getAttr(gradientFillElement, "type") ?? "linear") as "linear" | "path";
