...
import { getChildren, getAttr } from "@aurochs/xml";
 ...
 */
export function parseNumFmts(numFmtsElement: XmlElement | undefined): readonly XlsxNumberFormat[] {
  if (!numFmtsElement) {
    return [];
  }
 ...
 * ```
 */
export function resolveFormatCode(numFmtIdValue: number, customFormats: readonly XlsxNumberFormat[]): string {
