...
import { extractChartDataFromWorkbook, resolveEmbeddedXlsxPath } from "./chart-workbook-reader";
...
function createWorkbookWithChartData(): XlsxWorkbook {
  const rows: XlsxRow[] = [
    ...
    {
      rowNumber: rowIdx(1),
      cells: [createStringCell(1, 1, ""), createStringCell(2, 1, "Sales"), createStringCell(3, 1, "Costs")],
    },
    // Row 2: Q1
    {
      rowNumber: rowIdx(2),
      cells: [createStringCell(1, 2, "Q1"), createNumberCell(2, 2, 100), createNumberCell(3, 2, 80)],
    },
    // Row 3: Q2
    {
      rowNumber: rowIdx(3),
      cells: [createStringCell(1, 3, "Q2"), createNumberCell(2, 3, 120), createNumberCell(3, 3, 85)],
    },
    // Row 4: Q3
    {
      rowNumber: rowIdx(4),
      cells: [createStringCell(1, 4, "Q3"), createNumberCell(2, 4, 140), createNumberCell(3, 4, 90)],
    },
    // Row 5: Q4
    {
      rowNumber: rowIdx(5),
      cells: [createStringCell(1, 5, "Q4"), createNumberCell(2, 5, 160), createNumberCell(3, 5, 95)],
    },
  ];
...
describe("extractChartDataFromWorkbook", () => {
  ...
  test("throws error for invalid sheet index", () => {
    ...
    expect(() => extractChartDataFromWorkbook(workbook, 5)).toThrow(/sheet index 5 out of range/);
  });

  test("throws error for negative sheet index", () => {
    const workbook = createWorkbookWithChartData();

    expect(() => extractChartDataFromWorkbook(workbook, -1)).toThrow(/sheet index -1 out of range/);
  });
