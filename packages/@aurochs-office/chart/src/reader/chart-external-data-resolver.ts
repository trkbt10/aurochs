...
export function resolveChartExternalData(
  ...
  if (!resourceId) {
    throw new Error(`resolveChartExternalData: c:externalData missing r:id attribute in ${chartPath}`);
  }
 ...
 */
export function parseFormulaSheetName(formula: string): { sheetName: string; rangeRef: string } | undefined {
  if (!formula) {
    return undefined;
  }
