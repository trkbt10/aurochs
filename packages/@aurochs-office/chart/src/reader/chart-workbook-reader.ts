...
import { parseXml, getByPath, getChildren, getAttr } from "@aurochs/xml";
 ...
 * ```
 */
export function extractChartDataFromWorkbook(workbook: XlsxWorkbook, sheetIndex: number = 0): ChartDataUpdate {
...
function findDataDimensions(worksheet: XlsxWorksheet): { maxRow: number; maxCol: number } {
  return worksheet.rows.reduce(
    (acc, row) => {
      ...
      const maxCellCol = row.cells.reduce((colAcc, cell) => Math.max(colAcc, cell.address.col as number), acc.maxCol);
      return {
 ...
 */
const RELATIONSHIP_TYPE_PACKAGE = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/package";
