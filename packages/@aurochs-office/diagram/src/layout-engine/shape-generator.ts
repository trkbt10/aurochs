...
import { type LayoutNode, type LayoutBounds, type LayoutContext, createDefaultContext } from "./types";
import { createAlgorithmRegistry, getLayoutAlgorithm } from "./algorithms";
import { applyConstraintsToLayout } from "./constraints";
import { processForEach, processChoose, createForEachContext, type ForEachContext } from "./iteration";
import {
  resolveNodeStyle,
  createStyleContext,
 ...
 */
function createLayoutContext(layoutNode: DiagramLayoutNode, config: ShapeGenerationConfig): LayoutContext {
 ...
 */
function calculateTotalBounds(shapes: readonly LayoutShapeResult[], defaultBounds: LayoutBounds): LayoutBounds {
  if (shapes.length === 0) {
