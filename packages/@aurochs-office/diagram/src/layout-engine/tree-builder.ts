...
export type DiagramTreeNode = {
  ...
  /** Format-specific shape properties for rendering. */
  readonly shapeProperties?: unknown;
  /** Format-specific text content for this node. */
...
export type DiagramTextExtractor = (textBody: unknown) => string | undefined;
...
export function buildDiagramTree(dataModel: DiagramDataModel): DiagramTreeBuildResult {
  ...
  // eslint-disable-next-line no-restricted-syntax -- Updated during recursive tree building
  let maxDepth = 0;
...
export function countNodes(
  ...
  // eslint-disable-next-line no-restricted-syntax -- Incremented during tree traversal
  let count = 0;
 ...
 */
/**
 * Get the plain text content from a node's text body using a caller-provided extractor.
 *
 * The layout engine is format-agnostic: it does not assume a concrete TextBody structure.
 */
export function getNodeTextWith(
  node: DiagramTreeNode,
  extractText: DiagramTextExtractor
): string {
  const textBody = node.textBody;
  if (textBody === undefined) {
    return "";
  }
  return extractText(textBody) ?? "";
}
...
export function getAncestors(node: DiagramTreeNode): DiagramTreeNode[] {
  ...
  // eslint-disable-next-line no-restricted-syntax -- Traversed upward through parent chain
...
export function getRoot(node: DiagramTreeNode): DiagramTreeNode {
  // eslint-disable-next-line no-restricted-syntax -- Traversed upward to find root
  let current = node;
...
export function calculateMaxDepth(nodes: readonly DiagramTreeNode[]): number {
  // eslint-disable-next-line no-restricted-syntax -- Updated during recursive traversal
  let max = 0;
