...
import type { ParagraphAlignment } from "@aurochs-office/ooxml/domain/text";
import { resolveBulletConfig, createNumberingContext, type NumberingContext } from "./numbering-resolver";
import { createStyleResolver, resolveRunPropertiesWithStyles, type ResolvedRunProperties } from "./docx-style-resolver";
 ...
 */
function createFloatingImageConfig(content: DocxRunContent, paragraphIndex: number): FloatingImageConfig | undefined {
  if (content.type !== "drawing") {
    return undefined;
  }
...
function convertTabStops(tabs: DocxTabStops | undefined): readonly LayoutTabStop[] {
  ...
  return tabs.tabs.map((tab) => ({
    ...
    alignment:
      tab.val === "center" ? "center" : tab.val === "right" ? "right" : tab.val === "decimal" ? "decimal" : "left",
  }));
}
 ...
 */
export function createParagraphLayoutContext(numbering?: DocxNumbering, styles?: DocxStyles): ParagraphLayoutContext {
  return {
    numbering,
...
export function paragraphToLayoutInput(
  ...
  const endParaFontSize = props?.rPr?.sz !== undefined ? pt(props.rPr.sz / 2) : pt(SPEC_DEFAULT_FONT_SIZE_PT);
 ...
 */
function extractFloatingImagesFromParagraph(paragraph: DocxParagraph, paragraphIndex: number): FloatingImageConfig[] {
 ...
 */
export function extractFloatingImages(paragraphs: readonly DocxParagraph[]): FloatingImageConfig[] {
  const floatingImages: FloatingImageConfig[] = [];
