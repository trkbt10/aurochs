...
import type { PageFlowConfig, ColumnConfig, WritingMode } from "@aurochs-office/text-layout";
...
import { textDirectionToWritingMode } from "@aurochs-office/text-layout";
 ...
 */
function columnsToColumnConfig(cols: DocxColumns | undefined): ColumnConfig | undefined {
  ...
  if (!equalWidth && cols.col !== undefined && cols.col.length > 0) {
    const columnWidths: Pixels[] = cols.col.map((c) => (c.w !== undefined ? twipsToPx(c.w) : px(0)));
    return {
      num: cols.num,
      space,
      equalWidth: false,
 ...
 * ```
 */
export function sectionPropertiesToPageConfig(sectPr: DocxSectionProperties | undefined): PageFlowConfig {
