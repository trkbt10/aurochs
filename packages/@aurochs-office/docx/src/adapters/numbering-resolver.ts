...
import type { NumberFormat } from "@aurochs-office/ooxml/domain/numbering";
import type { DocxNumbering, DocxAbstractNum, DocxLevel } from "../domain/numbering";
import type { DocxNumberingProperties } from "../domain/paragraph";
import type { BulletConfig } from "@aurochs-office/text-layout";
 ...
 */
function getCounters(context: NumberingContext, numId: number, levels: readonly DocxLevel[]): number[] {
 ...
 */
function incrementCounter(counters: number[], ilvl: number, levels: readonly DocxLevel[]): void {
 ...
 */
function getLevelFormats(abstractNum: DocxAbstractNum, upToIlvl: number): NumberFormat[] {
