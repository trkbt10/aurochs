...
import type { DocTextbox } from "../domain/types";
...
export function extractTextboxes(options: {
  readonly textCps: readonly number[];
  readonly fullText: string;
  readonly txbxTextStart: number;
  readonly buildParagraphs?: SubdocParagraphBuilder;
}): readonly DocTextbox[] {
  const { textCps, fullText, txbxTextStart, buildParagraphs } = options;
