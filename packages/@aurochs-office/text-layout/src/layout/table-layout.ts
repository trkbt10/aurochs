 ...
 */
function calculateColumnWidths(table: LayoutTableInput, availableWidth: Pixels): readonly Pixels[] {
...
function getCellX(params: {
  ...
  const cellSpacingValue = cellSpacing as number;
  const widthBefore = columnWidths.slice(0, gridColIndex).reduce((sum, w) => sum + (w as number), 0);
 ...
 */
function buildMergeGrid(table: LayoutTableInput, columnWidths: readonly Pixels[]): GridCell[][] {
 ...
 */
function calculateRowSpans(mergeGrid: GridCell[][], numRows: number, numCols: number): Map<string, number> {
    ...
    for (let c = 0; c < numCols; c++) {
      ...
      if (cell.isOrigin) {
        ...
        for (let nextRow = r + 1; nextRow < numRows; nextRow++) {
          ...
          if (nextCell.ownerRow === cell.ownerRow && nextCell.ownerCol === cell.ownerCol) {
            spanRef.value++;
          } else {
            break;
          }
 ...
 */
export function layoutTable(table: LayoutTableInput, config: TableLayoutConfig): LayoutTableResult {
  ...
  const cellSpacingValue = cellSpacing as number;
  const totalColumnWidth = columnWidths.reduce((sum, w) => sum + (w as number), 0);
  ...
  const rowSpans = calculateRowSpans(mergeGrid, table.rows.length, columnWidths.length);
  ...
  for (let rowIndex = 0; rowIndex < table.rows.length; rowIndex++) {
    ...
    for (let cellIndex = 0; cellIndex < row.cells.length; cellIndex++) {
      ...
      const cell = row.cells[cellIndex];
      const { padding, paragraphs, borders, backgroundColor, verticalAlign, gridSpan, vMerge } = cell;
      ...
      const contentWidth = px((cellWidth as number) - (padding.left as number) - (padding.right as number));

      // Layout cell content
      const content = layoutCellContent(paragraphs, contentWidth);

      // Calculate minimum cell height
      const minHeight = (content.height as number) + (padding.top as number) + (padding.bottom as number);
  ...
  function getCellTotalRowHeight(params: {
    ...
    const spannedHeights = rowHeights.slice(rowIndex, rowIndex + cell.rowSpan).reduce((sum, h) => sum + h, 0);
