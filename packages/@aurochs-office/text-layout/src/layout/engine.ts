...
import { SPEC_DEFAULT_FONT_SIZE_PT } from "@aurochs-office/docx/domain/ecma376-defaults";
...
function layoutParagraph(params: {
  ...
  const firstLineWidth = px((contentWidth as number) - marginLeft - marginRight - indent - (bulletWidth as number));
  ...
  const {
    ...
    lineHeights,
    pageBreaksAfter,
  } = breakIntoLines({ spans: measuredSpans, firstLineWidth, nextLineWidth, wrapMode });
...
function calculateLineX(params: {
  ...
  const { alignment, marginLeft, lineWidth, availableWidth, indent, bulletWidth } = params;
...
function positionSpans(params: {
  ...
  const shouldJustify =
    (alignment === "justify" && !isLastLine && spans.length > 0) || (alignment === "distributed" && spans.length > 0);
