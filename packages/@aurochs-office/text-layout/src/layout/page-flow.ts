...
import {
  ...
  twipsToPx,
} from "@aurochs-office/docx/domain/ecma376-defaults";
import type {
 ...
 */
function transformLineToVerticalRl(line: LayoutLine, config: PageFlowConfig): LayoutLine {
 ...
 */
function transformLineToVerticalLr(line: LayoutLine, config: PageFlowConfig): LayoutLine {
  const marginLeft = config.marginLeft as number;
  const marginTop = config.marginTop as number;
 ...
 */
function transformPageForVertical(page: PageLayout, writingMode: WritingMode, config: PageFlowConfig): PageLayout {
  if (writingMode === "horizontal-tb") {
    return page;
  }
  ...
  return {
    ...
    paragraphs: page.paragraphs.map((para) => transformParagraphForVertical(para, writingMode, config)),
  };
}
 ...
 */
function countLinesThatFit(paragraph: LayoutParagraphResult, remainingSpace: number): number {
 ...
 * Start a new page.
 */
function startNewPage(state: FlowState, config: PageFlowConfig): void {
 ...
 */
function startNextColumn(state: FlowState, config: PageFlowConfig): void {
  const columns = getColumnConfig(config);
...
function addParagraphToPage(params: {
  ...
  state.totalHeight = Math.max(state.totalHeight, state.currentPage.pageStartY + state.currentPage.currentY);
}
...
function addParagraphWithOffset(state: FlowState, paragraph: LayoutParagraphResult, xOffset: number): void {
  const paragraphHeight = getParagraphHeight(paragraph);
 ...
 * @see ECMA-376-1:2016 Section 17.18.77 (ST_SectionMark)
 */
function handleSectionBreak(state: FlowState, config: PageFlowConfig, breakType: SectionBreakType): void {
  switch (breakType) {
    case "nextPage":
      // Start a new page
      if (state.currentPage.paragraphs.length > 0) {
...
function canFitInRemainingSpace(
  ...
  const contentHeight = getContentHeight(config);
  const remainingSpace = contentHeight - state.currentPage.currentY + (config.marginTop as number);
  const totalHeight = paragraphsToFit.reduce((sum, para) => sum + getParagraphHeight(para), 0);
 ...
 */
function findPageForParagraph(pages: readonly PageLayout[], paragraphIndex: number): number {
 ...
 */
function getParagraphY(pages: readonly PageLayout[], paragraphIndex: number): number {
 ...
 */
function calculateHorizontalPosition(image: FloatingImageConfig, config: PageFlowConfig): number {
  const pageWidth = config.pageWidth as number;
  const marginLeft = config.marginLeft as number;
  const marginRight = config.marginRight as number;
 ...
 */
function calculateVerticalPosition(image: FloatingImageConfig, config: PageFlowConfig, paragraphY: number): number {
 ...
 */
function getVerticalRefY(ref: FloatingImageConfig["verticalRef"], config: PageFlowConfig, paragraphY: number): number {
  const pageHeight = config.pageHeight as number;
  const marginTop = config.marginTop as number;
  const marginBottom = config.marginBottom as number;
...
export function flowIntoPages(input: PageFlowInput): PagedLayoutResult {
  ...
  paragraphs.forEach((paragraph, i) => {
    ...
    if (hint?.keepWithNext === true && i + 1 < paragraphs.length) {
      ...
      if (!canFitInRemainingSpace(state, paragraphsToKeep, config) && state.currentPage.paragraphs.length > 0) {
        startNewPage(state, config);
      }
    }
...
export function createSinglePageLayout(params: {
  ...
  if (isVertical(writingMode) && config !== undefined) {
    const transformedParagraphs = paragraphs.map((para) => transformParagraphForVertical(para, writingMode, config));
