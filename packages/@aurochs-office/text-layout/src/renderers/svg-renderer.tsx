 ...
 */
function applyTextTransform(text: string, transform: "none" | "uppercase" | "lowercase" | undefined): string {
 ...
 */
function getTextDecorationWithHyperlink(span: PositionedSpan): string | undefined {
  if (span.linkId !== undefined) {
...
function renderSpan(params: {
  ...
  const { span, x, lineY, dominantBaseline, key, writingMode = "horizontal-tb" } = params;
...
function renderLine(params: {
  ...
  const { line, fontAlignment, paragraphIndex, lineIndex, writingMode = "horizontal-tb" } = params;
 ...
 */
export function TextOverlay({ layoutResult, selection, cursor, showCursor = true }: TextOverlayProps): ReactNode {
  const elements: ReactNode[] = [];
  ...
  if (selection !== undefined && selection.length > 0) {
    elements.push(<g key="selection">{renderSelectionRects(selection)}</g>);
  }
