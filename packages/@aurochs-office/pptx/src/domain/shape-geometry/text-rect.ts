 ...
 * @see ECMA-376 Part 1, Section 20.1.9.22
 */
function resolveTextRect(textRect: TextRect, context: GuideContext): ResolvedTextRect {
 ...
 */
export function calculateTextRect(geometry: Geometry | undefined, width: number, height: number): ResolvedTextRect {
 ...
 */
function getPresetTextRect(preset: string, width: number, height: number): ResolvedTextRect {
 ...
 */
export function isInsideTextRect(textRect: ResolvedTextRect, x: number, y: number): boolean {
...
export function applyTextInsets({
  ...
  tIns = 0,
  bIns = 0,
}: ApplyTextInsetsOptions): ResolvedTextRect {
