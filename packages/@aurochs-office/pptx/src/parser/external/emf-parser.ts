...
import { base64ArrayBuffer } from "@aurochs/buffer";
...
function readPolygonCounts({ view, startOffset, endOffset, count }: ReadPolygonCountsOptions): {
  counts: number[];
  nextOffset: number;
} {
  const counts: number[] = [];
...
function buildPolyPolygonPath({
  ...
  stride,
  readPoint,
}: BuildPolyPolygonPathOptions): string {
  const offsetState = { value: startOffset };
...
function processComment(view: DataView, offset: number): string | null {
