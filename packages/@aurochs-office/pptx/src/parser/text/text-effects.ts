...
} from "@aurochs-office/drawing-ml/domain/text-effects";
 ...
 */
function resolveColorToHex(color: Color | undefined, colorContext: ColorContext): string | undefined {
  if (color === undefined) {
    return undefined;
  }
 ...
 */
function shadowToOffset(direction: number, distance: number): { dx: number; dy: number } {
 ...
 */
function resolveShadow(shadow: ShadowEffect, colorContext: ColorContext): TextShadowConfig | undefined {
  ...
  const { dx, dy } = shadowToOffset(shadow.direction as number, shadow.distance as number);
 ...
 */
function resolveGlow(glow: GlowEffect, colorContext: ColorContext): TextGlowConfig | undefined {
