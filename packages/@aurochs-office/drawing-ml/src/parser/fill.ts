...
import { findChild, getAttr, getChild, getChildren, type XmlElement } from "@aurochs/xml";
import { parseColorFromParent } from "./color";
import { getAngleAttr, getBoolAttrOr, getPercent100kAttr } from "./primitive";

const FILL_ELEMENT_NAMES = ["a:noFill", "a:solidFill", "a:gradFill", "a:blipFill", "a:pattFill", "a:grpFill"] as const;
...
export function findFillElement(parent: XmlElement): XmlElement | undefined {
  return findChild(parent, (child) => FILL_ELEMENT_NAMES.includes(child.name as (typeof FILL_ELEMENT_NAMES)[number]));
}
