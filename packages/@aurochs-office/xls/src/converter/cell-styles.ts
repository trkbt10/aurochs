...
import type { XlsxCellStyle } from "@aurochs-office/xlsx/domain/style/types";
...
export function convertXlsStylesToXlsxCellStyles(
  ...
  for (const style of xls.styles) {
    ...
    if (style.kind === "builtIn") {
      ...
      if (name !== baseName) {
        warnIfPossible(ctx, {
          code: "STYLE_DUPLICATE_NAME",
          where: "STYLE",
          message: `Duplicate cell style name; renamed: ${baseName} -> ${name}`,
        });
      }
      names.add(name);
    ...
    if (!baseName) {
      ...
      warnIfPossible(ctx, {
        code: "STYLE_EMPTY_NAME",
        where: "STYLE",
        message: `User-defined STYLE has an empty name; using generated name: ${name}`,
      });
      names.add(name);
      out.push({ name, xfId });
      continue;
