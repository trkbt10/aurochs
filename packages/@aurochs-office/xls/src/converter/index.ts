...
function toXlsxFormula(cell: XlsCell, ctx: XlsParseContext): Cell["formula"] | undefined {
  ...
  const expression = tryConvertBiffRpnToFormulaExpression(
    cell.formula.tokens,
    { baseRow: cell.row, baseCol: cell.col },
    ctx,
  );
  if (!expression) {
    return undefined;
...
function toXlsxRows(
  ...
  return rows.map((r) => ({
    ...r,
    cells: [...r.cells].sort((a, b) => (a.address.col as number) - (b.address.col as number)),
  }));
}
...
export function convertXlsToXlsx(xls: XlsWorkbook, ctx: XlsParseContext = { mode: "strict" }): XlsxWorkbook {
  ...
  const sheets: XlsxWorksheet[] = xls.sheets.map((s, idx) =>
    toXlsxWorksheet({ sheet: s, sheetId: idx + 1, dateSystem: xls.dateSystem, xfIndexToStyleId, ctx }),
  );
  return {
