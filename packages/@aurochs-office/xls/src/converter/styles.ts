...
import { borderId, fillId, fontId, numFmtId, styleId } from "@aurochs-office/xlsx/domain/types";
...
function isEmptyBorder(border: XlsxBorder): boolean {
  return (
    border.left === undefined && border.right === undefined && border.top === undefined && border.bottom === undefined
  );
}
...
export function convertXlsStylesToXlsxStyles(
  xls: XlsWorkbook,
  ctx: XlsParseContext = { mode: "strict" },
): XlsStyleConversionResult {
  if (!xls) {
  ...
  function buildCellXfs(): readonly XlsxCellXf[] {
    ...
    return cellXfs.map((entry) => {
      ...
      return convertXlsXfToXlsxCellXf({
        xf: entry.xf,
        xfIndex: entry.xfIndex,
        fonts,
        getBorderId,
        getFillId,
        xfId,
  ...
  for (const xfIndex of usedXfIndices) {
    ...
    const converted = convertXlsXfToXlsxCellXf({
      xf,
      xfIndex,
      fonts,
      getBorderId,
