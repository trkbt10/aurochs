...
function parsePtgRef(
  bytes: Uint8Array,
  offset: number,
  ctx: BiffFormulaContext,
): { readonly expr: string; readonly next: number } {
    ...
    if (xlsCtx) {
      ...
      if (xlsCtx.mode === "strict") {
        ...
        warnOrThrow(
          xlsCtx,
          warning,
          new Error(`Failed to convert BIFF formula tokens at r=${ctx.baseRow} c=${ctx.baseCol}: ${msg}`),
        );
      }
    }
    return undefined;
