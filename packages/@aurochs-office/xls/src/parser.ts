    ...
    try {
      ...
      if (err instanceof CfbFormatError && /signature/i.test(err.message)) {
        warnOrThrow(
          ctx,
          {
            code: "XLS_RAW_BIFF_FALLBACK",
            where: "openCfb",
            message: "CFB signature missing; treating file as raw BIFF stream.",
          },
          err,
        );
        return undefined;
  ...
  try {
    ...
    if (err instanceof CfbFormatError) {
      warnOrThrow(
        ctx,
        {
          code: "CFB_NON_STRICT_RETRY",
          where: "openCfb",
          message: "CFB strict read failed; retrying with non-strict reader.",
