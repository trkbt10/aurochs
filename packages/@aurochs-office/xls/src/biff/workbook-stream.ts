...
import type { ErrorValue } from "@aurochs-office/xlsx/domain/cell/types";
  ...
  try {
    ...
    warnOrThrow(
      ctx,
      {
        ...
        message: "BIFF record stream truncated; stopping early.",
        meta: { startOffset },
      },
...
function collectContinueRecordPayloads(
  records: readonly BiffRecord[],
  startIndex: number,
  endIndexExclusive: number,
): { readonly payloads: readonly Uint8Array[]; readonly stopIndex: number } {
...
function readSubstreamRecords(bytes: Uint8Array, startOffset: number, ctx: XlsParseContext): readonly BiffRecord[] {
  ...
  if (cursor.bofDepth > 0) {
    ...
    records.push({
      type: BIFF_RECORD_TYPES.EOF,
      length: 0,
      data: new Uint8Array(),
      offset: Math.min(cursor.offset, bytes.length),
    });
  }
  return records;
}
...
function parseWorkbookGlobals(records: readonly BiffRecord[], ctx: XlsParseContext): WorkbookGlobals {
  ...
  const globals: { palette?: PaletteRecord; dateSystem: DatemodeRecord["dateSystem"]; sharedStrings?: SstRecord } = {
    dateSystem: "1900",
  };
  ...
  switch (record.cached.type) {
    case "number":
      return {
        ...
        row: record.row,
        col: record.col,
        xfIndex: record.xfIndex,
        resultKind: "number",
        value: record.cached.value,
        formula,
      };
    case "empty":
      return {
...
export function parseWorkbookStream(
  bytes: Uint8Array,
  ctx: XlsParseContext = { mode: "strict" },
): WorkbookStreamParseResult {
