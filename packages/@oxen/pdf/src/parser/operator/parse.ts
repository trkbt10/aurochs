...
export function createGfxOpsFromStack(stack: GraphicsStateStack): GraphicsStateOps {
  return {
    ...
    setFillCmyk: (...args: readonly [c: number, m: number, y: number, k: number]) => stack.setFillCmyk(...args),
    setStrokeCmyk: (...args: readonly [c: number, m: number, y: number, k: number]) => stack.setStrokeCmyk(...args),
