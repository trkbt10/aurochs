...
function resolveDataPointColors({
  ...
  series: {
    readonly dataPoints?: readonly { readonly idx: number; readonly shapeProperties?: ChartShapeProperties }[];
  };
  seriesColor: string;
  numPoints: number;
  fillResolver: FillResolver;
...
function renderPlotAreaBackground({
  ...
  // Build rect element
  const strokeAttrs = hasLine ? ` ${toSvgStrokeAttributes(lineStyle)}` : ` stroke="none"`;
  elements.push(`<rect x="0" y="0" width="${plotWidth}" height="${plotHeight}" fill="${fillStyle}"${strokeAttrs}/>`);

  return elements.join("");
}

function resolveFillStyle(fill: ChartShapeProperties["fill"] | undefined, fillResolver: FillResolver): string {
  if (!fill) {
    return "none";
  }
...
export function renderChart({
  ...
  const displayUnitsLabelSvg = renderDisplayUnitsLabel(chart.plotArea.axes, ctx);
