...
import type { DataTable } from "@aurochs-office/chart/domain";
...
const basicInput: DataTableInput = {
  seriesData: [createSeriesData("Series 1", [10, 20, 30]), createSeriesData("Series 2", [15, 25, 35])],
...
describe("renderDataTable", () => {
  it("renders empty string for empty series data", () => {
    ...
    const result = renderDataTable({ dataTable, input: emptyInput, layout: basicLayout, fillResolver });
    expect(result).toBe("");
  });
  ...
  it("renders table with correct group transform", () => {
    ...
    const result = renderDataTable({ dataTable, input: basicInput, layout: basicLayout, fillResolver });
  ...
  it("uses default series name when key is empty", () => {
    ...
    const result = renderDataTable({ dataTable, input: inputWithEmptyKey, layout: basicLayout, fillResolver });
    expect(result).toContain("Series 1");
  });
  ...
  it("formats large numbers with locale formatting", () => {
    ...
    const result = renderDataTable({ dataTable, input: inputWithLargeNumbers, layout: basicLayout, fillResolver });
