...
import type { ValueAxis, CategoryAxis } from "@aurochs-office/chart/domain";
...
describe("axis utilities", () => {
  ...
  describe("renderMinorGridlines - ECMA-376 21.2.2.99", () => {
    ...
    it("returns empty string when no minor gridlines defined", () => {
      ...
      const result = renderMinorGridlines({
        axes: [axis],
        chartWidth: 300,
        chartHeight: 200,
        valueRange: { minVal: 0, maxVal: 100 },
        ctx,
      });
      expect(result).toBe("");
    });
    ...
    it("renders minor gridlines for log scale base 10", () => {
      ...
      const result = renderMinorGridlines({
        ...
        chartWidth: 300,
        chartHeight: 200,
        valueRange: { minVal: 1, maxVal: 100 },
        ctx,
      });
      expect(result).toContain("<line");
      expect(result).toContain(`stroke="${ctx.getGridlineColor()}"`);
...
describe("renderMultiLevelCategoryAxisLabels (ECMA-376 21.2.2.102 multiLvlStrRef)", () => {
  ...
  it("returns empty string when axis is deleted", () => {
    ...
    const result = renderMultiLevelCategoryAxisLabels({
      axes: [axis],
      chartWidth: 400,
      chartHeight: 200,
      multiLevelCategories: multiLevel,
      ctx,
    });
    expect(result).toBe("");
  });
