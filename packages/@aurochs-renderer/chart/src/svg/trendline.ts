...
import type { Trendline, ChartShapeProperties, Layout } from "@aurochs-office/chart/domain";
 ...
 */
function calculateLinearRegression(points: readonly { x: number; y: number }[]): {
  slope: number;
  intercept: number;
  rSquared: number;
} {
  ...
  if (denominator === 0) {
    return { slope: 0, intercept: sums.sumY / n, rSquared: 0 };
  }
 ...
 */
function calculateExponentialRegression(points: readonly { x: number; y: number }[]): {
  a: number;
  b: number;
  rSquared: number;
} {
 ...
 */
function calculateLogarithmicRegression(points: readonly { x: number; y: number }[]): {
  a: number;
  b: number;
  rSquared: number;
} {
 ...
 */
function calculatePowerRegression(points: readonly { x: number; y: number }[]): {
  a: number;
  b: number;
  rSquared: number;
} {
 ...
 */
function getTrendlineStyle(spPr: ChartShapeProperties | undefined, fillResolver: FillResolver): ResolvedLineStyle {
  if (!spPr) {
...
export function renderTrendline({
  ...
  const style = getTrendlineStyle(trendline.shapeProperties, fillResolver);
  ...
  switch (trendline.trendlineType) {
    ...
    case "poly": {
      ...
      trendlineState.pathPoints = buildCurvePoints({
        ...
        minVal,
        range,
        getY: (x) => coefficients.reduce((sum, coeff, index) => sum + coeff * x ** index, 0),
      });
  ...
  if (trendlineState.pathPoints.length < 2) {
    return "";
  }

  // Build path
  const pathD = trendlineState.pathPoints.map((p, i) => (i === 0 ? `M ${p.x} ${p.y}` : `L ${p.x} ${p.y}`)).join(" ");

  const elements: string[] = [];

  // Render trendline
  elements.push(`<path d="${pathD}" fill="none" stroke="${style.color}" stroke-width="${style.width}"${strokeDash}/>`);
  ...
  if (hasEquation || hasRSquared) {
    ...
    const labelPos = resolveTrendlineLabelPosition({
      lastPoint,
      manualLayout: label?.layout?.manualLayout,
      chartWidth,
      chartHeight,
    });
    ...
    const textStyle = ctx.getTextStyle(label?.textProperties);
    const textAttrs = toSvgTextAttributes(textStyle);
    ...
    if (label?.shapeProperties) {
      ...
      const bgColor = bgFill ? resolvedFillToColor(fillResolver.resolve(bgFill)) : undefined;
      if (bgColor) {
