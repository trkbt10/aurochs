...
import type { ChartContent, SeriesData, ValueAxisConfig } from "../render-types";
import { getValueRange, getCategoryLabels, getNumericValues } from "../data";
import type { LineSeries, UpDownBars, ChartLines } from "@aurochs-office/chart/domain";
...
export function generateStockChart({
  ...
  Array.from({ length: pointCount }, (_, i) => {
    ...
    if (config.hiLowLines) {
      ...
      const hiLowStyle = extractHiLowLineStyle(config.hiLowLines, fillResolver);
      svgParts.push(`<line x1="${x}" y1="${highY}" x2="${x}" y2="${lowY}" ${toSvgStrokeAttributes(hiLowStyle)}/>`);
    }
    ...
    if (config.dropLines) {
      const lowY = valueToY(pointValues.low);
      const dropStyle = extractDropLineStyle(config.dropLines, fillResolver);
      svgParts.push(`<line x1="${x}" y1="${lowY}" x2="${x}" y2="${chartHeight}" ${toSvgStrokeAttributes(dropStyle)}/>`);
    }
  });
