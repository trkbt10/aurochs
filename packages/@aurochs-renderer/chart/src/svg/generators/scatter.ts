...
import type { ScatterStyle, Marker } from "@aurochs-office/chart/domain";
...
const MARKER_RENDERERS: Record<Marker["symbol"], MarkerRenderer> = {
  circle: ({ x, y, half, fill }) => `<circle cx="${x}" cy="${y}" r="${half}" fill="${fill}"/>`,
  ...
  star: ({ x, y, half, fill }) =>
    `<polygon points="${generateStarPoints({ x, y, outerR: half, innerR: half * 0.4 })}" fill="${fill}"/>`,

  dash: ({ x, y, half, fill }) =>
    `<line x1="${x - half}" y1="${y}" x2="${x + half}" y2="${y}" stroke="${fill}" stroke-width="2"/>`,

  dot: ({ x, y, half, fill }) => `<circle cx="${x}" cy="${y}" r="${Math.max(1, half * 0.4)}" fill="${fill}"/>`,

  none: () => "",

  // Picture markers require image embedding - fall back to circle
  picture: ({ x, y, half, fill }) => `<circle cx="${x}" cy="${y}" r="${half}" fill="${fill}"/>`,
};
 ...
 */
function generateSmoothPath(points: readonly { x: number; y: number }[], tension: number = 0.5): string {
  if (points.length < 2) {
    return "";
  }
  if (points.length === 2) {
 ...
 */
function renderLine(screenPoints: readonly { x: number; y: number }[], color: string, useSmooth: boolean): string {
...
function renderMarkers(
  ...
  return screenPoints.map((pt) => renderMarkerSymbol({ x: pt.x, y: pt.y, symbol, size: markerSize, fill: color }));
}
...
export function generateScatterChart({
  ...
  if (!baseStyleConfig.renderLines && !baseStyleConfig.renderMarkers) {
    return buildResult(drawAxes({ chartWidth, chartHeight, axes: undefined, ctx }));
  }
  ...
  data.forEach((series, seriesIndex) => {
    ...
    const screenPoints = calculateScreenPoints({ values: series.values, chartWidth, chartHeight, range });
  ...
  return buildResult(drawAxes({ chartWidth, chartHeight, axes: undefined, ctx }) + elements.join(""));
}
...
function extractCategoryLabels(data: readonly SeriesData[]): readonly string[] | undefined {
  ...
  for (const series of data) {
    if (series.xlabels && Object.keys(series.xlabels).length > 0) {
      ...
      const indices = Object.keys(series.xlabels)
        .map(Number)
        .sort((a, b) => a - b);
