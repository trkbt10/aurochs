...
import type { RadarStyle } from "@aurochs-office/chart/domain";
...
function generateRadarGrid({
  ...
  Array.from({ length: numRings }, (_, ringIndex) => {
    ...
    elements.push(`<polygon points="${points.join(" ")}" fill="none" stroke="#ddd" stroke-width="1"/>`);
  });
  ...
  Array.from({ length: numCategories }, (_, i) => {
    ...
    elements.push(`<line x1="${centerX}" y1="${centerY}" x2="${x}" y2="${y}" stroke="#ddd" stroke-width="1"/>`);
  });
...
export function generateRadarChart({
  ...
  data.forEach((series, seriesIndex) => {
    ...
    const points = series.values.map((point, index) => {
      ...
      return calculateRadarPoint({ centerX, centerY, radius, angle, value: point.y, maxValue });
    });
    ...
    if (isFilled) {
      ...
      elements.push(`<polygon points="${polygonPoints}" fill="none" stroke="${color}" stroke-width="2"/>`);
    }
  ...
  Array.from({ length: numCategories }, (_, i) => {
    ...
    elements.push(`<text x="${x}" y="${y + 4}" text-anchor="${textAnchor}" font-size="10" fill="#666">${label}</text>`);
  });
