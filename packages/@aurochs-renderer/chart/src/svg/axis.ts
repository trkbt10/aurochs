...
import type { ChartRenderContext, GenericTextBody } from "./types";
...
function getCategoryLabelX(alignment: "l" | "r" | "ctr", index: number, categoryWidth: number): number {
  switch (alignment) {
    case "l":
 ...
 */
export function formatDateLabel(dateValue: string | number, timeUnit: TimeUnit | undefined): string {
 ...
 * @see ECMA-376 Part 1, Section 21.2.2.48 (dispUnitsLbl)
 */
export function renderDisplayUnitsLabel(axes: readonly Axis[], ctx: ChartRenderContext): string {
  ...
  const style = ctx.getTextStyle(valAxis.dispUnits.dispUnitsLbl?.textBody);
  const styleAttrs = toSvgTextAttributes(style);
  return `<text x="${x}" y="${y}" text-anchor="middle" ${styleAttrs} fill="${style.color}" transform="rotate(-90, ${x}, ${y})">${escapeHtml(label)}</text>`;
}
 ...
 * @see ECMA-376 Part 1, Section 21.2.2.90 (logBase)
 */
function calculateLogMajorValues(minVal: number, maxVal: number, logBase: number): number[] {
  const safeMin = Math.max(minVal, Number.EPSILON);
  const safeMax = Math.max(maxVal, safeMin);
...
export function renderAxisTitles({
  ...
  if (valAxis?.title && !valAxis.delete) {
    ...
    if (titleText) {
      ...
      const style = ctx.getTextStyle(valAxis.title.textBody);
      const styleAttrs = toSvgTextAttributes(style);

      titles.push(
        `<text x="${x}" y="${y}" text-anchor="middle" ${styleAttrs} fill="${style.color}" transform="rotate(-90, ${x}, ${y})">${escapeHtml(titleText)}</text>`,
      );
    }
  }
  ...
  if (catAxis?.title && !catAxis.delete) {
    ...
    if (titleText) {
      ...
      const style = ctx.getTextStyle(catAxis.title.textBody);
      const styleAttrs = toSvgTextAttributes(style);

      titles.push(
        `<text x="${x}" y="${y}" text-anchor="middle" ${styleAttrs} fill="${style.color}">${escapeHtml(titleText)}</text>`,
      );
    }
  }
...
export function drawAxes({
  ...
  const axisColor = ctx.getAxisColor();
  ...
  // Only render if not deleted
  if (!valAxis?.delete) {
    lines.push(`<line x1="0" y1="0" x2="0" y2="${chartHeight}" stroke="${axisColor}" stroke-width="1"/>`);
  }
 ...
 */
export function safeGetLabel(xlabels: Record<string, string> | undefined, index: number): string {
  if (!xlabels) {
...
export function renderGridlines({
  ...
  const gridlineColor = ctx.getGridlineColor();
  const lines: string[] = [];
  ...
  if (isLogScale(valAxis)) {
    ...
    for (const val of majorValues) {
      ...
      lines.push(`<line x1="0" y1="${y}" x2="${chartWidth}" y2="${y}" stroke="${gridlineColor}" stroke-width="1"/>`);
    }
  } else {
    // Linear scale
    ...
    for (let val = Math.ceil(minVal / majorUnit) * majorUnit; val <= maxVal; val += majorUnit) {
 ...
 * @see ECMA-376 Part 1, Section 21.2.2.90 (logBase)
 */
function calculateLogMinorValues(minVal: number, maxVal: number, logBase: number): number[] {
  const safeMin = Math.max(minVal, Number.EPSILON);
  const safeMax = Math.max(maxVal, safeMin);
...
export function renderMinorGridlines({
  ...
  if (isLogScale(valAxis)) {
    ...
    for (const val of minorValues) {
      ...
      lines.push(
        `<line x1="0" y1="${y}" x2="${chartWidth}" y2="${y}" stroke="${gridlineColor}" stroke-width="0.5" stroke-opacity="0.6"/>`,
      );
    }
  } else {
    // Linear scale
    ...
    for (let val = Math.ceil(minVal / minorUnit) * minorUnit; val <= maxVal; val += minorUnit) {
      // Skip if this would overlap with a major gridline
      const isMajor =
        Math.abs(val % majorUnit) < minorUnit / 2 || Math.abs((val % majorUnit) - majorUnit) < minorUnit / 2;
...
function renderTickMark({
  ...
  if (position === "left") {
    ...
    if (tickMarkStyle === "out" || tickMarkStyle === "cross") {
      elements.push(`<line x1="${x - size}" y1="${y}" x2="${x}" y2="${y}" stroke="${axisColor}" stroke-width="1"/>`);
    }
    if (tickMarkStyle === "in" || tickMarkStyle === "cross") {
      elements.push(`<line x1="${x}" y1="${y}" x2="${x + size}" y2="${y}" stroke="${axisColor}" stroke-width="1"/>`);
    }
  } else {
    // Horizontal axis (bottom)
    if (tickMarkStyle === "out" || tickMarkStyle === "cross") {
      elements.push(`<line x1="${x}" y1="${y}" x2="${x}" y2="${y + size}" stroke="${axisColor}" stroke-width="1"/>`);
    }
    if (tickMarkStyle === "in" || tickMarkStyle === "cross") {
      elements.push(`<line x1="${x}" y1="${y - size}" x2="${x}" y2="${y}" stroke="${axisColor}" stroke-width="1"/>`);
    }
  }
  ...
  if (isLogScale(valAxis)) {
    ...
    if (valAxis.majorTickMark !== "none") {
      for (const val of majorValues) {
        ...
        marks.push(renderTickMark({ tickMarkStyle: valAxis.majorTickMark, position: "left", x: 0, y, axisColor }));
      }
    }
    ...
    if (valAxis.minorTickMark !== "none") {
      for (let val = Math.ceil(minVal / minorUnit) * minorUnit; val <= maxVal; val += minorUnit) {
        ...
        if (valAxis.minorTickMark === "out" || valAxis.minorTickMark === "cross") {
          marks.push(`<line x1="${-size}" y1="${y}" x2="0" y2="${y}" stroke="${axisColor}" stroke-width="0.5"/>`);
        }
        if (valAxis.minorTickMark === "in" || valAxis.minorTickMark === "cross") {
          marks.push(`<line x1="0" y1="${y}" x2="${size}" y2="${y}" stroke="${axisColor}" stroke-width="0.5"/>`);
        }
      }
    }
  }
...
export function renderCategoryAxisTickMarks({
  ...
  if (catAxis.majorTickMark !== "none") {
    for (let i = 0; i <= numCategories; i++) {
      ...
      marks.push(
        renderTickMark({ tickMarkStyle: catAxis.majorTickMark, position: "bottom", x, y: chartHeight, axisColor }),
      );
    }
  }
...
export function renderValueAxisLabels({
  ...
  const textStyle = ctx.getTextStyle(valAxis.textProperties);
  ...
  if (isLogScale(valAxis)) {
    ...
    for (const val of majorValues) {
      ...
      labels.push(
        `<text x="${xPos}" y="${y + 4}" text-anchor="${textAnchor}" ${textStyleAttrs} fill="${textColor}">${formatValue(val)}</text>`,
      );
    }
  } else {
    // Linear scale
...
export function renderCategoryAxisLabels({
  ...
  const textAnchor = alignment === "l" ? "start" : alignment === "r" ? "end" : "middle";
  ...
  const textStyle = ctx.getTextStyle(axis.textProperties);
  const textStyleAttrs = toSvgTextAttributes(textStyle);
  const textColor = textStyle.color;
  ...
  for (let i = 0; i < numCategories; i++) {
    ...
    labels.push(
      `<text x="${x}" y="${yPos}" text-anchor="${textAnchor}" ${textStyleAttrs} fill="${textColor}">${escapeHtml(labelText)}</text>`,
    );
  }
...
export function renderMultiLevelCategoryAxisLabels({
  ...
  const textStyle = ctx.getTextStyle(catAxis.textProperties);
  const textStyleAttrs = toSvgTextAttributes(textStyle);
  const textColor = textStyle.color;
  const axisColor = ctx.getAxisColor();
  ...
  for (let levelIdx = 0; levelIdx < levels.length; levelIdx++) {
    ...
    if (levelIdx === 0) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        svgElements.push(
          `<text x="${x}" y="${levelY}" text-anchor="middle" ${textStyleAttrs} fill="${textColor}">${escapeHtml(labelText)}</text>`,
        );
      }
    } else {
      ...
      for (const span of spans) {
        ...
        if (span.end > span.start) {
          ...
          svgElements.push(
            `<line x1="${startX + 5}" y1="${bracketY}" x2="${endX - 5}" y2="${bracketY}" stroke="${axisColor}" stroke-width="1" stroke-opacity="0.6"/>`,
          );
          // Left tick
          svgElements.push(
            `<line x1="${startX + 5}" y1="${bracketY - bracketHeight}" x2="${startX + 5}" y2="${bracketY}" stroke="${axisColor}" stroke-width="1" stroke-opacity="0.6"/>`,
          );
          // Right tick
          svgElements.push(
            `<line x1="${endX - 5}" y1="${bracketY - bracketHeight}" x2="${endX - 5}" y2="${bracketY}" stroke="${axisColor}" stroke-width="1" stroke-opacity="0.6"/>`,
          );
        }

        // Render label text
        svgElements.push(
          `<text x="${centerX}" y="${levelY + 10}" text-anchor="middle" ${textStyleAttrs} fill="${textColor}">${escapeHtml(span.label)}</text>`,
        );
      }
    }
  }
...
export function renderScatterXAxisLabels({
  ...
  const bottomAxis = axes.find((ax): ax is ValueAxis => ax.type === "valAx" && ax.position === "b");
  ...
  const textStyle = ctx.getTextStyle(bottomAxis?.textProperties);
  const textStyleAttrs = toSvgTextAttributes(textStyle);
  const textColor = textStyle.color;
  ...
  for (let i = 0; i < numLabels; i++) {
    ...
    labels.push(
      `<text x="${x}" y="${yPos}" text-anchor="middle" ${textStyleAttrs} fill="${textColor}">${escapeHtml(categoryLabels[i])}</text>`,
    );
  }
