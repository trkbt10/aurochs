  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\n` +
        `Run: bun packages/@aurochs-renderer/fig/scripts/generate-constraint-edge-cases-fixtures.ts`,
    );
  }
...
describe("Constraint Edge Cases", () => {
  beforeAll(async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/fig/scripts/generate-constraint-edge-cases-fixtures.ts");
      return;
    }
    await loadFigFile();
