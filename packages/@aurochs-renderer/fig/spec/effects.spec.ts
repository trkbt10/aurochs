...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@aurochs-renderer/fig/scripts/generate-effect-fixtures.ts`,
    );
  }
...
describe("Effect Fixture Debug", () => {
  it("lists frames in effects.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/fig/scripts/generate-effect-fixtures.ts");
      return;
    }
