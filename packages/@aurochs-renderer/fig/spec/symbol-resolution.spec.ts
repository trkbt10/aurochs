  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\n` +
        `Run: bun packages/@aurochs-renderer/fig/scripts/generate-symbol-resolution-fixtures.ts`,
    );
  }
...
describe("Symbol Resolution", () => {
  beforeAll(async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/fig/scripts/generate-symbol-resolution-fixtures.ts");
      return;
    }
    await loadFigFile();
