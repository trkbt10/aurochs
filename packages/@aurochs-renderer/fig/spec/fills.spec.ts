...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@aurochs-renderer/fig/scripts/generate-fill-fixtures.ts`,
    );
  }
...
describe("Fill Fixture Debug", () => {
  it("lists available layers in fills.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/fig/scripts/generate-fill-fixtures.ts");
      return;
    }
