 ...
 *
 * Usage: bun packages/@aurochs-renderer/fig/scripts/generate-snapshots.ts [scope]
 *
 * Example:
 *   bun packages/@aurochs-renderer/fig/scripts/generate-snapshots.ts twitter-ui
 *   bun packages/@aurochs-renderer/fig/scripts/generate-snapshots.ts text-comprehensive
 */
...
if (!scope) {
  console.log("Usage: bun packages/@aurochs-renderer/fig/scripts/generate-snapshots.ts [scope]");
  console.log("\nAvailable fixtures:");
  const dirs = fs.readdirSync(FIXTURES_DIR).filter((d) => fs.statSync(path.join(FIXTURES_DIR, d)).isDirectory());
  dirs.forEach((d) => console.log(`  - ${d}`));
  process.exit(1);
