...
import type { AnimateMotionBehavior, Point } from "@aurochs-office/pptx/domain/animation";
 ...
 */
type QuadraticBezierPointArgs = [x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, t: number];
...
export function parseMotionPath(pathString: string): MotionPath {
  ...
  for (const cmd of commands) {
    ...
    switch (cmd.type) {
      ...
      case "C":
        endPoint = { x: cmd.x, y: cmd.y };
        length = cubicBezierLength(startPoint.x, startPoint.y, cmd.x1, cmd.y1, cmd.x2, cmd.y2, cmd.x, cmd.y);
        break;

      case "Q":
        endPoint = { x: cmd.x, y: cmd.y };
        length = quadraticBezierLength(startPoint.x, startPoint.y, cmd.x1, cmd.y1, cmd.x, cmd.y);
        break;
...
export function getPointAtProgress(path: MotionPath, progress: number): Point {
  ...
  const foundSegment = path.segments.find((seg) => seg.cumulativeLength + seg.length >= targetLength);
 ...
 */
export function toSVGPathString(path: MotionPath, slideWidth: number = 960, slideHeight: number = 540): string {
  const parts: string[] = [];
  ...
  for (const cmd of path.commands) {
    switch (cmd.type) {
      ...
      case "Q":
        parts.push(
          `Q ${cmd.x1 * slideWidth} ${cmd.y1 * slideHeight} ` + `${cmd.x * slideWidth} ${cmd.y * slideHeight}`,
        );
        break;
