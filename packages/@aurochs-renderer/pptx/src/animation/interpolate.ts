 ...
 */
export function parseAnimateValue(value: AnimateValue | undefined): ParsedValue | undefined {
 ...
 * @returns Pixel value
 */
export function parseCoordinateValue(value: AnimateValue | undefined, slideSize: number): number | undefined {
 ...
 */
export function interpolateValues(from: AnimateValue, to: AnimateValue, t: number): AnimateValue {
 ...
 */
function updateTransform(element: HTMLElement | SVGElement, fn: string, value: string): void {
...
export function createAnimateFunction({
  ...
  if (to !== undefined) {
    ...
    const cssProperty = mapAttributeToCSS(attribute);
    const fromValue: AnimateValue = cssProperty === "opacity" ? parseFloat(element.style.opacity) || 1 : 0;
