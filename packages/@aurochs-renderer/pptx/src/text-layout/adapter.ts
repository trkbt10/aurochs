...
import type { AutoFit } from "@aurochs-office/pptx/domain/text";
 ...
 */
function resolveFontFamily(rawFontFamily: string | undefined, fontScheme: FontScheme | undefined): string {
...
function resolveVerticalAlign(props: RunProperties | undefined): "baseline" | "superscript" | "subscript" {
...
function resolveTextTransform(caps: RunProperties["caps"] | undefined): "none" | "uppercase" | "lowercase" {
  if (caps === "all") {
    return "uppercase";
  }
  if (caps === "small") {
...
function resolveBulletFontSize(bulletStyle: BulletStyle, defaultFontSize: Points): Points {
...
function resolveBulletChar(bullet: BulletStyle["bullet"], autoNumberIndex: number): string | undefined {
  switch (bullet.type) {
    case "none":
      return undefined;
    case "char":
...
function resolveBulletColor(bulletStyle: BulletStyle, colorContext: ColorContext): string {
...
function resolveBulletWithTextColor(
  ...
  const shouldFollowTextColor =
    bulletStyle?.colorFollowText === true || (bulletStyle?.color === undefined && firstSpanColor !== undefined);
 ...
 */
function resolveColor(color: Color | undefined, colorContext: ColorContext): string | undefined {
  if (color === undefined) {
    return undefined;
  }
...
function paragraphToInput({
  ...
  autoNumberIndex,
  resourceResolver,
}: ParagraphToInputOptions): LayoutParagraphInput {
