...
import type { FieldRun, RegularRun } from "@aurochs-office/pptx/domain/text";
...
describe("text-centering-and-color", () => {
  ...
  it("checks layout and master for text style inheritance", async () => {
    ...
    if (masterCtrTitleMatch) {
      ...
      const defRPrMatches: readonly string[] =
        masterCtrTitleMatch[0].match(/<a:defRPr[^>]*>[\s\S]*?<\/a:defRPr>/g) ?? [];
...
function toLayoutRelsPath(layoutPath: string | undefined): string | undefined {
  ...
  return layoutPath.replace(".xml", ".xml.rels").replace("slideLayouts/", "slideLayouts/_rels/");
}
