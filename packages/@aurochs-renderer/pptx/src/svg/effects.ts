...
import type { Effects, GlowEffect, ShadowEffect, SoftEdgeEffect } from "@aurochs-office/pptx/domain";
import { type HtmlString, unsafeHtml } from "./string-utils";
import { filter, feGaussianBlur, feOffset, feColorMatrix, feMerge, feMergeNode } from "./primitives";
 ...
 */
function calculateShadowOffset(distance: number, direction: number): { dx: number; dy: number } {
 ...
 * @see ECMA-376 Part 1, Section 20.1.8.49
 */
function generateOuterShadowFilter(shadow: ShadowEffect, filterId: string, colorContext?: ColorContext): HtmlString {
  const { dx, dy } = calculateShadowOffset(shadow.distance as number, shadow.direction as number);
  ...
  const merge = feMerge(feMergeNode({ in: "shadow" }), feMergeNode({ in: "SourceGraphic" }));

  return filter({ id: filterId, x: "-50%", y: "-50%", width: "200%", height: "200%" }, blur, offset, colorize, merge);
}
 ...
 */
function generateInnerShadowFilter(shadow: ShadowEffect, filterId: string, colorContext?: ColorContext): HtmlString {
  const { dx, dy } = calculateShadowOffset(shadow.distance as number, shadow.direction as number);
  ...
  const merge = feMerge(feMergeNode({ in: "SourceGraphic" }), feMergeNode({ in: "innerShadow" }));
 ...
 */
function generateShadowFilter(shadow: ShadowEffect, filterId: string, colorContext?: ColorContext): HtmlString {
 ...
 */
function generateGlowFilter(glow: GlowEffect, filterId: string, colorContext?: ColorContext): HtmlString {
  ...
  const merge = feMerge(feMergeNode({ in: "glow" }), feMergeNode({ in: "SourceGraphic" }));

  return filter({ id: filterId, x: "-50%", y: "-50%", width: "200%", height: "200%" }, blur, colorize, merge);
}
 ...
 */
function generateSoftEdgeFilter(softEdge: SoftEdgeEffect, filterId: string): HtmlString {
  ...
  return filter({ id: filterId, x: "-10%", y: "-10%", width: "120%", height: "120%" }, blur, composite);
}
 ...
 */
export function getFilterAttribute(effects: Effects | undefined, shapeId: string): string | undefined {
  if (!effects) {
    return undefined;
  }
