 ...
 */
export function generateMarkerId(lineEnd: LineEnd, colorHex: string, position: "head" | "tail"): string {
 ...
 */
function generateTriangleShape(width: number, height: number, color: string): HtmlString {
 ...
 */
function generateStealthShape(width: number, height: number, color: string): HtmlString {
  // Stealth arrow: notched triangle
  // Points: left-quarter, right-center, left-3quarter, center-notch
  const notchDepth = width * 0.3;
  const points = [`0,0`, `${width},${height / 2}`, `0,${height}`, `${notchDepth},${height / 2}`].join(" ");
  return polygon({ points, fill: color });
}
 ...
 */
function generateDiamondShape(width: number, height: number, color: string): HtmlString {
  // Diamond: 4 points at cardinal directions
  const points = [`${width / 2},0`, `${width},${height / 2}`, `${width / 2},${height}`, `0,${height / 2}`].join(" ");
  return polygon({ points, fill: color });
}
 ...
 */
function generateOvalShape(width: number, height: number, color: string): HtmlString {
  return ellipse({
    cx: width / 2,
    cy: height / 2,
    rx: width / 2,
 ...
 */
function generateArrowShape(options: {
