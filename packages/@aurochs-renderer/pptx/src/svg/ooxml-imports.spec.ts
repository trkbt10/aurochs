...
const checks: readonly ImportCheck[] = [
  {
    ...
    message:
      'Avoid importing OOXML unit types/constructors from "@aurochs-office/pptx/domain/types"; import from "@aurochs-office/drawing-ml/domain/units" instead.',
    predicate: (source) =>
  ...
  },
  {
    ...
    message:
      'Avoid importing OOXML Color from "@aurochs-office/pptx/domain" or "../../domain/index"; import from "@aurochs-office/drawing-ml/domain/color" instead.',
    predicate: (source) =>
      !/import\s+(?:type\s+)?\{[^}]*\bColor\b[^}]*\}\s+from\s+["']\.\.\/\.\.\/domain(?:\/index)?["']\s*;/.test(source),
  },
  {
    name: "no-color-imports-from-pptx-color-types",
    message:
      'Avoid importing OOXML Color from "@aurochs-office/pptx/domain/color/types"; import from "@aurochs-office/drawing-ml/domain/color" instead.',
    predicate: (source) =>
