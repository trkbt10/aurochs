...
export function renderResolvedBackgroundSvg(
  ...
  switch (resolved.type) {
    ...
    case "gradient": {
      ...
      const stops = resolved.stops.map((s) => `<stop offset="${s.position}%" stop-color="${s.color}"/>`).join("\n");
  ...
  switch (fill.type) {
    ...
    case "gradientFill": {
      ...
      if (fill.stops.length > 0) {
        const fallbackStyle = renderFillToSvgStyle({ type: "solidFill", color: fill.stops[0].color }, ctx.colorContext);
