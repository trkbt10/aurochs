...
function applyOverflowClip({
  ...
  boxWidth,
  boxHeight,
}: ApplyOverflowClipOptions): string {
...
function applyVerticalTextTransform({
  ...
  boxWidth,
  boxHeight,
}: ApplyVerticalTextTransformOptions): string {
...
function createTextGradientDef(
  fill: {
    type: "gradient";
    stops: readonly { position: number; color: string; alpha: number }[];
    angle: number;
    isRadial: boolean;
 ...
 */
export function resolveColorForSvg(color: Color, colorContext: ColorContext): string {
  const { spec } = color;
