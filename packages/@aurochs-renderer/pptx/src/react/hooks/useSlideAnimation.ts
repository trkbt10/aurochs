...
export function useSlideAnimation(options: UseSlideAnimationOptions): UseSlideAnimationResult {
  const { slideIndex, timing, containerRef, autoPlay = true, onStart, onComplete, speed = 1.0 } = options;
  ...
  const { play, stop, showAll, hideAll, extractShapeIds, isPlaying } = useAnimationPlayer({
    findElement,
    onStart,
    onComplete,
    speed,
