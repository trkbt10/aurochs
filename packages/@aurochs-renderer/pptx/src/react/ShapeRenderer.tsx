 ...
 */
export const ShapeRenderer = memo(function ShapeRenderer({ shape, editingShapeId }: ShapeRendererProps) {
  // Skip hidden shapes
  ...
  switch (shape.type) {
    case "sp":
      return <SpShapeRenderer shape={shape} width={width} height={height} hideText={isEditing} shapeId={shapeId} />;

    case "pic":
      return <PicShapeRenderer shape={shape} width={width} height={height} shapeId={shapeId} />;

    case "cxnSp":
      return <CxnShapeRenderer shape={shape} width={width} height={height} shapeId={shapeId} />;

    case "grpSp":
      return <GrpShapeRenderer shape={shape} editingShapeId={editingShapeId} shapeId={shapeId} />;

    case "graphicFrame":
      return <GraphicFrameRenderer shape={shape} width={width} height={height} shapeId={shapeId} />;
 ...
 */
function areShapeRendererPropsEqual(prev: ShapeRendererProps, next: ShapeRendererProps): boolean {
  ...
  const affectsShape = shapeContainsId(prev.shape, prevId) || shapeContainsId(prev.shape, nextId);
  return !affectsShape;
}
