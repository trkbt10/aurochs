...
} from "@aurochs-renderer/drawing-ml/react";
import { buildFontFamily, applyTextTransform, applyVerticalAlign, toSvgDominantBaseline } from "./text-utils";
 ...
 */
function applyTextFill(props: TextProps, span: PositionedSpan, defs: DefsManager): void {
 ...
 */
function wrapWithLink(textElement: ReactNode, span: PositionedSpan, key: number): ReactNode {
  if (!span.linkId) {
    return textElement;
  }

  return (
    <g key={`link-${key}`} style={{ cursor: "pointer" }} data-link-id={span.linkId}>
...
export function renderSpan({
  ...
  if (span.highlightColor !== undefined) {
    elements.push(
      renderHighlight({
        ...
        lineY,
        fontSizePx,
        spanWidth: span.width as number,
  ...
  // Create text element
  const textElement = (
    <text key={`text-${key}`} {...textProps} filter={effectsFilterUrl}>
      {textContent}
    </text>
  );
...
export function renderLine({
  ...
  line.spans.reduce((cursorX, span, index) => {
