 ...
 */
export function ResolvedBackgroundRenderer({ resolvedBackground, slideSize }: ResolvedBackgroundRendererProps) {
  const { width, height } = slideSize;
  const { getNextId } = useSvgDefs();

  switch (resolvedBackground.type) {
    case "solid":
      return <rect width={width as number} height={height as number} fill={resolvedBackground.color} />;
    ...
    case "gradient": {
      ...
      return (
        <>
          ...
          <rect width={width as number} height={height as number} fill={`url(#${gradId})`} />
        </>
      );
    }
...
export function BackgroundRenderer({ background, slideSize }: BackgroundRendererProps) {
  ...
  if (background === undefined || background.fill === undefined) {
    return <rect width={width as number} height={height as number} fill="#ffffff" />;
  }
  ...
  switch (fill.type) {
    case "noFill":
      return <rect width={width as number} height={height as number} fill="transparent" />;

    case "solidFill": {
      const resolved = resolveFill(fill, colorContext);
      const fillColor = resolved.type === "solid" ? `#${resolved.color.hex}` : "#ffffff";
      return <rect width={width as number} height={height as number} fill={fillColor} />;
    }
    ...
    case "blipFill": {
      ...
      if (imagePath !== undefined) {
        ...
        return (
          <image href={imagePath} width={width as number} height={height as number} preserveAspectRatio={aspectRatio} />
        );
      }
      return <rect width={width as number} height={height as number} fill="#ffffff" />;
    }
...
function createGradientDef(
  ...
  const stops = resolved.stops.map((s, i) => <stop key={i} offset={`${s.position}%`} stopColor={s.color} />);
...
function createCoreGradientDef(
  resolved: {
    stops: readonly { color: { hex: string; alpha: number }; position: number }[];
    angle: number;
    isRadial: boolean;
    radialCenter?: { cx: number; cy: number };
  },
