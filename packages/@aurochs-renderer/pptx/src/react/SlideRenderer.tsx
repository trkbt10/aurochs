...
import type { Slide, SlideSize, Shape } from "@aurochs-office/pptx/domain";
 ...
 */
function SlideContent({ slide, slideSize, resolvedBackground, editingShapeId }: SlideContentProps) {
  ...
  return (
    <>
      ...
      <SlideBackground slide={slide} slideSize={slideSize} resolvedBackground={resolvedBackground} />

      {/* Layout shapes (decorative, rendered behind slide content) */}
      {layoutShapes?.map((shape, index) => (
        <ShapeRenderer key={getShapeKey(shape, index, "layout")} shape={shape} />
      ))}

      {/* Slide shapes (each renders its own defs for gradients/patterns) */}
      {slide.shapes.map((shape, index) => (
        <ShapeRenderer key={getShapeKey(shape, index, "slide")} shape={shape} editingShapeId={editingShapeId} />
      ))}
    </>
  );
}
