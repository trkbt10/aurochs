...
import type { GrpShape as GrpShapeType, Shape } from "@aurochs-office/pptx/domain";
 ...
 */
export function GrpShapeRenderer({ shape, editingShapeId, shapeId }: GrpShapeRendererProps) {
  ...
  return (
    <g transform={transformValue || undefined} data-shape-id={shapeId} data-shape-type="grpSp">
      {children.map((child, index) => (
        <ShapeRenderer key={getShapeKey(child, index)} shape={child} editingShapeId={editingShapeId} />
      ))}
    </g>
  );
}
