...
function OleIconView({
  ...
  return (
    <g>
      ...
      {/* Label */}
      <text x={width / 2} y={labelY} fontSize={fontSize} fontFamily="sans-serif" fill="#333333" textAnchor="middle">
        {displayName}
      </text>
    </g>
  );
 ...
 */
export const OleObjectContent = memo(function OleObjectContent({ data, width, height }: OleObjectContentProps) {
  const { previewUrl, hasPreview, showAsIcon, objectName, progId, imageWidth, imageHeight } = useOlePreview(data);

  // Show as icon mode: render icon representation
  if (showAsIcon) {
    return <OleIconView width={width} height={height} objectName={objectName} progId={progId} />;
  }
