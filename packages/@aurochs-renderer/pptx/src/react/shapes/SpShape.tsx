...
import type { SpShape as SpShapeType } from "@aurochs-office/pptx/domain";
 ...
 */
export function SpShapeRenderer({ shape, width, height, hideText = false, shapeId }: SpShapeRendererProps) {
  ...
  return (
    <g transform={transformValue || undefined} data-shape-id={shapeId} data-shape-type="sp">
      {/* Render geometry (background shape) */}
      <GeometryPath geometry={geometry} width={width} height={height} fill={fill} line={line} />

      {/* Render text content (conditional) */}
      {!hideText && textBody && <TextRenderer textBody={textBody} width={width} height={height} />}
    </g>
  );
}
