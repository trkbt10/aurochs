...
function PicShapeRendererBase({ shape, width, height, shapeId }: PicShapeRendererProps) {
  ...
  const clipId = useMemo(() => `pic-clip-${shapeId ?? "unknown"}`, [shapeId]);
  const hasCrop =
    srcRect !== undefined && (srcRect.left !== 0 || srcRect.top !== 0 || srcRect.right !== 0 || srcRect.bottom !== 0);
  ...
  if (hasCrop && croppedLayout !== null) {
    return (
      <g transform={transformValue || undefined} data-shape-id={shapeId} data-shape-type="pic">
        <defs>
          <clipPath id={clipId}>
  ...
  return (
    <g transform={transformValue || undefined} data-shape-id={shapeId} data-shape-type="pic">
      <image href={imagePath} x={0} y={0} width={width} height={height} preserveAspectRatio="none" />
    </g>
  );
}
...
function areTransformsEqual(a: Transform | undefined, b: Transform | undefined): boolean {
  ...
  return ax === bx && ay === by && aRotation === bRotation && aFlipH === bFlipH && aFlipV === bFlipV;
}
