...
import { getSupportedPatterns } from "@aurochs-renderer/drawing-ml/react";
...
export function FillTest() {
  ...
  return (
    <div className="test-section">
      <h3>Fill Types</h3>
      <p className="section-description">ECMA-376 Section 20.1.8 - Fill properties for shapes.</p>
      ...
      <TestSubsection title="Gradient Variations" items={gradientItems}>
        <div className="shape-row">
          ...
          <ShapePreview
            fill={makeGradient(
              45,
              { pos: 0, color: { scheme: "accent3" } },
              { pos: 100, color: { scheme: "accent4" } },
            )}
            label="45° (↘)"
          />
          <ShapePreview
            fill={makeGradient(
              90,
              { pos: 0, color: { scheme: "accent5" } },
              { pos: 100, color: { scheme: "accent6" } },
            )}
            label="90° (↓)"
          />
          <ShapePreview
      ...
      >
        ...
        <div className="pattern-names">{supportedPatterns.join(", ")}</div>
      </TestSubsection>
    </div>
  );
}
