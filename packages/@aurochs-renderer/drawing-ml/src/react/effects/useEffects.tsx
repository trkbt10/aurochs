 ...
 */
function resolveGlowIfPresent(glow: Effects["glow"], colorContext: ColorContext): ResolvedGlowProps | undefined {
  if (glow === undefined) {
    return undefined;
  }
...
export function resolveEffectsForReact(
  ...
  const activeEffects = [shadowProps !== undefined, glowProps !== undefined, softEdgeRadius !== undefined].filter(
    Boolean,
  ).length;
  ...
  const filterDef = createCombinedFilterDef(filterId, effects, colorContext);
 ...
 */
function createCombinedFilterDef(id: string, effects: Effects, colorContext: ColorContext): ReactNode {
  const hasShadow = effects.shadow !== undefined;
  const hasGlow = effects.glow !== undefined;
  const hasSoftEdge = effects.softEdge !== undefined;
 ...
 */
function CombinedFilterDef({ id, shadow, glow, softEdge, colorContext }: CombinedFilterDefProps): ReactNode {
