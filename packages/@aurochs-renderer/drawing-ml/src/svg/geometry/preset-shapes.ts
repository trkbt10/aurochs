/**
 * @file Preset shape path generators
 *
 * Contains all ECMA-376 preset shape definitions and their path generators.
 *
 * @see ECMA-376 Part 1, Section 20.1.10.55 (ST_ShapeType)
 */

import type { GeometryGuide } from "@aurochs-office/drawing-ml/domain/geometry";
import { createGuideContext, evaluateGuides } from "@aurochs-office/drawing-ml/domain/geometry";
 ...
 */
function renderRightArrowPath(w: number, h: number, adj: Map<string, number>): string {
...
function renderDownArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 50000;
  const adj2 = adj.get("adj2") ?? 50000;
  const context = createGuideContext(w, h, [
    { name: "adj1", value: adj1 },
...
function renderLeftArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 50000;
  const adj2 = adj.get("adj2") ?? 50000;
  const context = createGuideContext(w, h, [
    { name: "adj1", value: adj1 },
...
function renderLeftRightArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 50000;
  const adj2 = adj.get("adj2") ?? 50000;
  const context = createGuideContext(w, h, [
    { name: "adj1", value: adj1 },
...
function renderUpDownArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 50000;
  const adj2 = adj.get("adj2") ?? 50000;
  const context = createGuideContext(w, h, [
    { name: "adj1", value: adj1 },
...
function renderLeftArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
...
function renderRightArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const adj4 = adj.get("adj4") ?? 64977;
...
function renderUpArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const adj4 = adj.get("adj4") ?? 64977;
...
function renderDownArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const adj4 = adj.get("adj4") ?? 64977;
...
function renderLeftRightArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
...
function renderUpDownArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const adj4 = adj.get("adj4") ?? 48123;
...
function renderNotchedRightArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 50000;
  const adj2 = adj.get("adj2") ?? 50000;
  const context = createGuideContext(w, h, [
    { name: "adj1", value: adj1 },
...
function renderQuadArrowCalloutPath(w: number, h: number, adj: Map<string, number>): string {
...
function renderBentArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
...
function renderBentUpArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderUturnArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const adj4 = adj.get("adj4") ?? 43750;
...
function renderLeftUpArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderLeftRightUpArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 25000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderQuadArrowPath(w: number, h: number, adj: Map<string, number>): string {
...
function renderCircularArrowPath(w: number, h: number, adj: Map<string, number>): string {
  ...
  const { xE, yE, rw1, rh1, stAng, swAng, xGp, yGp, xA, yA, xBp, yBp, xC, yC, rw2, rh2, istAng, iswAng } =
    requireGuideValues(
      context,
      [
        "xE",
...
function renderLeftCircularArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 12500;
  const adj2 = adj.get("adj2") ?? -1142319;
  ...
  const { xE, yE, xD, yD, rw2, rh2, istAng, iswAng, xBp, yBp, xA, yA, xGp, yGp, xF, yF, rw1, rh1, stAng0, swAng0 } =
    requireGuideValues(
      context,
      [
        "xE",
...
function renderLeftRightCircularArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 12500;
  const adj2 = adj.get("adj2") ?? 1142319;
  const adj3 = adj.get("adj3") ?? 20457681;
...
function renderCurvedRightArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 50000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderCurvedLeftArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 50000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderCurvedUpArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 50000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderCurvedDownArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
  const adj2 = adj.get("adj2") ?? 50000;
  const adj3 = adj.get("adj3") ?? 25000;
  const context = createGuideContext(w, h, [
...
function renderSwooshArrowPath(w: number, h: number, adj: Map<string, number>): string {
  const adj1 = adj.get("adj1") ?? 25000;
...
const PRESET_SHAPES: Record<string, (w: number, h: number, adj: Map<string, number>) => string> = {
  ...
  moon: (w, h, adj) => {
    ...
    const path = [`M ${rx} 0 A ${rx} ${ry} 0 1 1 ${rx} ${h} `, `A ${innerRx} ${ry} 0 1 0 ${rx} 0 Z`].join("");
    return path;
  },
 ...
 */
export function renderPresetGeometryData(geom: PresetGeometry, width: number, height: number): string {
