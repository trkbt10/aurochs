...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@aurochs-renderer/figma/scripts/generate-component-fixtures.ts`,
    );
  }
...
describe("Component Fixture Debug", () => {
  it("lists symbols and frames in components.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/figma/scripts/generate-component-fixtures.ts");
      return;
    }
