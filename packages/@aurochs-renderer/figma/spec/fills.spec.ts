...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@aurochs-renderer/figma/scripts/generate-fill-fixtures.ts`,
    );
  }
  ...
  for (const [layerName, fileName] of Object.entries(LAYER_FILE_MAP)) {
    it(`renders "${layerName}" with correct fills`, async () => {
      ...
      if (hasActual) {
        ...
        console.log(
          `Rendered fills: ${renderedFillInfo.solidFills.length} solid, ${renderedFillInfo.gradientDefs} gradients`,
        );
...
describe("Fill Fixture Debug", () => {
  it("lists available layers in fills.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/figma/scripts/generate-fill-fixtures.ts");
      return;
    }
    ...
    for (const [name, info] of data.layers) {
      ...
      for (const child of children) {
        ...
        console.log(
          `    - ${child.name} (${childType}) fills=${fillPaints?.length ?? 0}, strokes=${strokePaints?.length ?? 0}`,
        );
      }
    }
