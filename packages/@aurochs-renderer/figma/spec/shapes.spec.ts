...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@aurochs-renderer/figma/scripts/generate-shape-fixtures.ts`,
    );
  }
...
describe("Shape Rendering", () => {
  ...
  for (const [layerName, fileName] of Object.entries(LAYER_FILE_MAP)) {
    it(`renders "${layerName}" with correct structure`, async () => {
      ...
      const actualShapesRef = { value: { ellipses: 0, lines: 0, paths: 0, polygons: 0, rects: 0, circles: 0, total: 0 } };
...
describe("Shape Fixture Debug", () => {
  it("lists available layers in shapes.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@aurochs-renderer/figma/scripts/generate-shape-fixtures.ts");
      return;
    }
