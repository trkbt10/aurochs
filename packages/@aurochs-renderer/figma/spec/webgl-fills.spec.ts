/**
 * @file WebGL fill rendering visual regression test
 *
 * Per-fill-type SVG↔WebGL comparison using fills.fig fixture.
 * Tests: solid fills, linear/radial gradients, and strokes.
 *
 * Run:
 *   npx vitest run packages/@aurochs-renderer/figma/spec/webgl-fills.spec.ts
 */
...
const FIG_FILE = path.join(__dirname, "../fixtures/fills/fills.fig");
const WEBGL_DIR = path.join(__dirname, "../fixtures/webgl-fills");
...
const CATEGORIES: Record<string, string[]> = {
  solid: ["solid-colors", "solid-opacity"],
  "linear-gradient": ["gradient-linear-h", "gradient-linear-v", "gradient-linear-45", "gradient-multi-stop"],
  "radial-gradient": ["gradient-radial", "gradient-radial-offset"],
  stroke: ["stroke-basic", "stroke-caps", "stroke-dash", "stroke-align"],
};
...
describe("WebGL fill rendering", () => {
  ...
  it("summary", async () => {
    ...
    printCategorySummary("WebGL Fill Rendering Summary", categoryResults);
  }, 120000);
});
