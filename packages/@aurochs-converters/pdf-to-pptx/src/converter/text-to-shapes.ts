 ...
 */
export function convertTextToShape(pdfText: PdfText, context: ConversionContext, shapeId: string): SpShape {
  if (shapeId.length === 0) {
...
export function createPptxTextRunFromPdfText(pdfText: PdfText, context: ConversionContext): TextRun {
  ...
  const spacing = convertSpacing(pdfText.charSpacing, pdfText.horizontalScaling, context);
...
function areTextRunPropsEquivalent(
  a: TextRun["properties"] | undefined,
  b: TextRun["properties"] | undefined,
): boolean {
 ...
 */
export function convertGroupedTextToShape(group: GroupedText, context: ConversionContext, shapeId: string): SpShape {
  if (shapeId.length === 0) {
    throw new Error("shapeId is required");
  }

  const size = convertSize(group.bounds.width, group.bounds.height, context);
  const position = convertGroupedTextPosition(group.bounds, size, context);
