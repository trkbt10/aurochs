...
import type { PdfDocument, PdfImage, PdfPage, PdfPath, PdfPathOp, PdfText } from "@aurochs/pdf/domain";
import { decomposeMatrix } from "@aurochs/pdf/domain";
...
export function convertPageToShapes(page: PdfPage, options: ConversionOptions): Shape[] {
  ...
  const filterNestedTableRegions = (regions: readonly TableRegion[]): TableRegion[] => {
    ...
    for (const r of sorted) {
      ...
      for (const k of kept) {
        ...
        const ratio = ov / a;
        const contains = r.x0 >= k.x0 - tol && r.x1 <= k.x1 + tol && r.y0 >= k.y0 - tol && r.y1 <= k.y1 + tol;
    ...
    if (filteredTableRegions.length > 0) {
      type Planned =
        ...
        | {
            ...
            readonly decoration: TableDecorationAnalysis;
  ...
  const splitInferredTableByColumn = (inferred: InferredTable, splitAt: number): InferredTable[] => {
    if (!(splitAt > 0 && splitAt < inferred.columns.length)) {
      return [inferred];
    }
    ...
    const findVerticalDividerRegion = (): { x0: number; x1: number } | null => {
      ...
      for (const p of paths) {
        ...
        for (const sub of splitSubpaths(p.operations)) {
          ...
          const intersects = bb.x1 > region.x0 && bb.x0 < region.x1 && bb.y1 > region.y0 && bb.y0 < region.y1;
          if (!intersects) {
            continue;
          }
          ...
          if (Math.abs(xCenter - splitX) > tol) {
            continue;
          }
      ...
      if (divider) {
        if (colOffset === 0) {
          ...
          if (divider.x0 > last.x0) {
            normalizedCols[normalizedCols.length - 1] = {
              ...last,
              x1: divider.x0,
              xCenter: (last.x0 + divider.x0) / 2,
            };
          }
        } else {
  ...
  const mergeGroups = (indices: readonly number[]): { merged: GroupedText; mergedIndices: readonly number[] } => {
    ...
    if (picked.length === 0) {
      throw new Error("mergeGroups: no groups");
    }
  ...
  const isMeaningfulTableDecoration = (
    decoration: TableDecorationAnalysis | null,
  ): decoration is TableDecorationAnalysis => {
    if (!decoration) {
      return false;
    }
    if (decoration.consumedPathIndices.length > 0) {
      return true;
    }
    if (decoration.cellFills.size > 0) {
      return true;
    }
    if (decoration.verticalBorders.some(Boolean)) {
      return true;
    }
    if (decoration.horizontalBorders.some(Boolean)) {
      return true;
    }
    return false;
  };
  ...
  for (const table of tableCandidates) {
    ...
    for (let j = 0; j < groupArray.length; j++) {
      ...
      if (otherBottom < tableTop - table.fontSize * 1.0) {
        continue;
      }
      if (otherBottom > tableTop + headerWindow) {
        continue;
      }

      const otherTop = other.bounds.y + other.bounds.height;
      if (otherTop > tableTop + table.fontSize * 1.2) {
        continue;
      }
      if (other.bounds.height > table.fontSize * 1.8) {
        continue;
      }
  ...
  if (!hasPlannedTables) {
    ...
    const regions: Region[] = (() => {
      ...
      for (const p of paths) {
        ...
        const w = x1 - x0;
        const h = y1 - y0;
        if (w < page.width * 0.35) {
          continue;
        }
        if (h < page.height * 0.08) {
          continue;
        }
...
export function convertDocumentToSlides(doc: PdfDocument, options: ConversionOptions): DocumentConversionResult {
