...
import type { PdfImage, PdfPage, PdfPath, PdfText } from "@aurochs/pdf/domain";
...
describe("convertPageToShapes", () => {
  it("converts path/text/image into Shape[] with generated IDs", () => {
    ...
    if (!s2 || s2.type !== "pic") {
      throw new Error("Expected pic shape");
    }
  ...
  it("throws for invalid minPathComplexity", () => {
    ...
    expect(() => convertPageToShapes(page, { ...options, minPathComplexity: Number.NaN })).toThrow(
      "Invalid minPathComplexity",
    );
  });
