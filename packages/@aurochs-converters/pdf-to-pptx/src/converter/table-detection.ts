...
function splitByVerticalWhitespace(
  ...
  if (!(typicalGap > 0 && maxGap >= typicalGap * 3.2)) {
    return null;
  }
...
export function detectTableRegionsFromPaths(
  ...
  const boxes = boxes0.filter((b) => b.thickness <= thicknessMax).map((b) => b.bbox);
  if (boxes.length === 0) {
    return [];
  }
  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    while (queue.length > 0) {
      ...
      for (let j = 0; j < boxes.length; j++) {
        ...
        const other = boxes[j]!;
        if (!intersects(curExpanded, other)) {
          continue;
        }
        visited[j] = true;
        queue.push(j);
