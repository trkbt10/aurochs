...
import type { PdfColor, PdfGraphicsState, PdfLineCap, PdfLineJoin, PdfAlternateColorSpace } from "@aurochs/pdf/domain";
import { grayToRgb, cmykToRgb, rgbToHex, clamp01, toByte } from "@aurochs/pdf/domain/color";
 ...
 */
function convertIccBasedToSrgb(components: readonly number[], alternateColorSpace?: PdfAlternateColorSpace): Color {
...
export function convertFill(pdfColor: PdfColor, alpha: number = 1): Fill {
  ...
  const color: Color = a < 1 ? { ...baseColor, transform: { ...baseColor.transform, alpha: pct(a * 100) } } : baseColor;
