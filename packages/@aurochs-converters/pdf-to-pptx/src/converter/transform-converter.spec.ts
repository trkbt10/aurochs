...
describe("createFitContext", () => {
  ...
  it("throws for invalid PDF size", () => {
    expect(() =>
      createFitContext({ pdfWidth: 0, pdfHeight: 100, slideWidth: px(300), slideHeight: px(300), fit: "contain" }),
    ).toThrow("Invalid pdfWidth");
    expect(() =>
      createFitContext({ pdfWidth: 100, pdfHeight: 0, slideWidth: px(300), slideHeight: px(300), fit: "contain" }),
    ).toThrow("Invalid pdfHeight");
  });

  it("throws for invalid slide size", () => {
    expect(() =>
      createFitContext({ pdfWidth: 100, pdfHeight: 100, slideWidth: px(0), slideHeight: px(300), fit: "contain" }),
    ).toThrow("Invalid slideWidth");
    expect(() =>
      createFitContext({ pdfWidth: 100, pdfHeight: 100, slideWidth: px(300), slideHeight: px(0), fit: "contain" }),
...
describe("convertMatrix", () => {
  ...
  it("falls back when slide scaling introduces shear (scaleX ≠ scaleY with rotation)", () => {
    ...
    const scaledContext = createFitContext({
      pdfWidth: 400,
      pdfHeight: 300,
      slideWidth: px(800),
      slideHeight: px(450),
