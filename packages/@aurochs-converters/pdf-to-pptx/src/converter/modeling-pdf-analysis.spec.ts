...
describe("modeling.pdf analysis", () => {
  ...
  it("should identify bold/italic fonts", () => {
    ...
    const boldFonts = [...fontNames].filter(
      (n) => n.toLowerCase().includes("bold") || n.toLowerCase().includes("bd") || n.includes("-B"),
    );
    const italicFonts = [...fontNames].filter(
      (n) =>
  ...
  it("should find shapes near text (potential inline references)", () => {
    ...
    const refTexts = texts.filter((t) => /\(\d+\)/.test(t.text) || /^\d+$/.test(t.text.trim()));
  ...
  it("should analyze text widths for debugging", () => {
    ...
    console.log(
      `Observed ratio: ${(sampleTexts[0]?.width ?? 0) / (sampleTexts[0]?.text.length ?? 1) / (sampleTexts[0]?.fontSize ?? 1)}`,
    );
...
describe("Text Grouping analysis", () => {
  ...
  it("should analyze grouping behavior on modeling.pdf", async () => {
    ...
    const singleRunGroups = groups.filter((g) => g.paragraphs.length === 1 && g.paragraphs[0].runs.length === 1);
    console.log(
      `Single-run groups: ${singleRunGroups.length} (${((singleRunGroups.length / groups.length) * 100).toFixed(1)}%)`,
    );
  ...
  it("should analyze text run structure for wrapping issues", async () => {
    ...
    const largestGroup = groups.reduce((max, g) => (g.paragraphs.length > max.paragraphs.length ? g : max));

    console.log(`\n--- Largest Group: ${largestGroup.paragraphs.length} paragraphs ---`);
    console.log(
      `Bounds: x=${largestGroup.bounds.x.toFixed(1)} y=${largestGroup.bounds.y.toFixed(1)} ` +
    ...
    if (tb) {
      ...
      tb.paragraphs.forEach((para, i) => {
        const runsText = para.runs
          ...
          .join("");
        console.log(
          `  Para[${i}] (${para.runs.length} runs): "${runsText.slice(0, 60)}${runsText.length > 60 ? "..." : ""}"`,
        );
      });
...
describe("Receipt PDF analysis", () => {
  ...
  it("should analyze receipt structure for table extraction", async () => {
    ...
    for (const text of texts.slice(0, 50)) {
      ...
      for (const shape of filledRects) {
        ...
        if (
          textCenterX >= shapeMinX &&
          textCenterX <= shapeMaxX &&
          textCenterY >= shapeMinY &&
          textCenterY <= shapeMaxY
        ) {
    ...
    const singleRunGroups = groups.filter((g) => g.paragraphs.length === 1 && g.paragraphs[0].runs.length === 1);
    console.log(
      `Single-run groups (ungrouped texts): ${singleRunGroups.length} (${((singleRunGroups.length / groups.length) * 100).toFixed(1)}%)`,
    );
  ...
  it("should analyze table cell structure for grouping", async () => {
    ...
    const filledRects = paths.filter(
      (p) => (p.paintOp === "fill" || p.paintOp === "fillStroke") && p.operations.length === 5, // Simple rectangle = moveto + 3 lineto + closepath
    );
    ...
    for (const text of texts) {
      for (const rect of filledRects) {
        ...
        if (textCenterX >= minX && textCenterX <= maxX && textCenterY >= minY && textCenterY <= maxY) {
...
describe("K-resource PDF table analysis", () => {
  ...
  it("should analyze table structure on page with tables", async () => {
    ...
    const filledRects = paths.filter(
      (p) =>
        (p.paintOp === "fill" || p.paintOp === "fillStroke") && p.operations.length >= 4 && p.operations.length <= 6,
    );
    ...
    const tryPlaceTextIntoRects = (text: PdfText): boolean => {
      for (const rect of filledRects) {
        ...
        if (
          text.x >= minX - 2 &&
          text.x + text.width <= maxX + 2 &&
          text.y >= minY - 2 &&
          text.y + text.height <= maxY + 2
        ) {
          if (!textsByRect.has(rect)) {
            textsByRect.set(rect, []);
          }
...
describe("TextBox width and spacing verification", () => {
  ...
  it("should verify text width includes charSpacing", async () => {
    ...
    const multiRunGroups = groups.filter((g) => g.paragraphs.some((p) => p.runs.length > 1));
  ...
  it("should check if adjacent runs are being grouped", async () => {
    ...
    const sameStyleAdjacent = adjacentExamples.filter(
      ({ prev, curr }) => prev.fontName === curr.fontName && Math.abs(prev.fontSize - curr.fontSize) < 0.5,
    );
