...
function computeRowBandBounds(
  rowBaselines: readonly number[],
  bounds: InferredTable["bounds"],
): { y0: number; y1: number }[] {
  if (rowBaselines.length === 0) {
    return [];
  }
...
function assignSegmentToColumn(seg: { x0: number; x1: number }, columns: readonly InferredTableColumn[]): number {
  const center = (seg.x0 + seg.x1) / 2;
  for (const col of columns) {
    if (col.x0 <= center && center < col.x1) {
      return col.index;
    }
  }
...
function inferGridFromPaths({
  ...
  if (subBBoxes.length < minSubpaths) {
    return null;
  }
  ...
  const acceptHorizontalRule = (yc: number): boolean => {
    ...
    const x1 =
      xBoundariesForCoverage?.[xBoundariesForCoverage.length - 1] ?? approxTableBounds.x + approxTableBounds.width;
    const cov = unionCoverage1D(segs, x0, x1);
    if (cov / Math.max(1, x1 - x0) >= coverageRatioTh) {
      return true;
    }
  ...
  if (horizontalCentersForRules.length < minRuleCount) {
    return null;
  }

  const yLines = cluster1D(horizontalCentersForRules, clusterTol).sort((a, b) => b - a);
  if (yLines.length < minRuleCount) {
    return null;
  }
  ...
  if (yBoundaries.length >= 2) {
    ...
    if (Math.abs(approxTop - top) > edgeTol) {
      yBoundaries = [approxTop, ...yBoundaries].sort((a, b) => b - a);
    }
    if (Math.abs(bottom - approxBottom) > edgeTol) {
      yBoundaries = [...yBoundaries, approxBottom].sort((a, b) => b - a);
    }
  }
...
function resolveCellAlignment(
  ...
  if (left <= fontSize * 0.2 && right > left * 1.5) {
    return "left";
  }
  if (right <= fontSize * 0.2 && left > right * 1.5) {
    return "right";
  }
  return "left";
}
...
export function inferTableFromGroupedText(
  ...
  if (group.paragraphs.length < opts.minRows) {
    return null;
  }
  ...
  if (paragraphs.length < opts.minRows) {
    return null;
  }
  ...
  const lineSegs: LineSeg[] = paragraphs.map((p) => {
    const segments = segmentRunsIntoCells(p.runs, fontSize)
      .map((segRuns) => ({
  ...
  grid =
    grid ??
    (opts.paths ? inferGridFromPaths({ paths: opts.paths, approxBounds: group.bounds, fontSize, targetCols }) : null);
  ...
  if (trainingLines.length === 0) {
    trainingLines = lineSegs.filter((l) => l.segments.length === targetCols);
  }
  ...
  const boundaries: number[] = (() => {
    ...
    const refCenterLeft = median(
      trainingLines.map((l) => {
        const s = l.segments[2]!;
        return (s.x0 + s.x1) / 2;
      }),
    );
    const refCenterRight = median(
      trainingLines.map((l) => {
    ...
    const leftCodeNameBoundary =
      leftCodeName.length > 0 ? median(leftCodeName) : boundsX0 + (blockSplitX - boundsX0) * 0.3;
    const rightCodeNameBoundary =
      rightCodeName.length > 0 ? median(rightCodeName) : blockSplitX + (boundsX1 - blockSplitX) * 0.3;
  ...
  if (grid?.xBoundaries && grid.xBoundaries.length === targetCols + 1) {
    boundaries.length = 0;
    for (const x of grid.xBoundaries) {
      boundaries.push(x);
    }
  }
  ...
  if (grid?.yBoundaries && grid.yBoundaries.length >= 3) {
      ...
      for (let ri = 0; ri < out.length - 1 && inserted < maxInserts; ri++) {
        ...
        const colsForCenter = (center: number): number => {
          ...
          for (const p of bandParas) {
            if (Math.abs(p.baselineY - center) > baselineEps) {
              continue;
            }
            for (const run of p.runs) {
              if (!isMeaningfulText(run.text)) {
                continue;
              }
              const x0 = run.x;
        ...
        if (!(yTop - splitY > margin && splitY - yBottom > margin)) {
          continue;
        }
    ...
    const findOrCreateBaselineKey = (mp: Map<number, PdfText[]>, baselineY: number): number => {
      for (const k of mp.keys()) {
        if (Math.abs(k - baselineY) <= baselineEps) {
          return k;
        }
      }
      mp.set(baselineY, []);
    ...
    const findRowIndex = (baselineY: number): number | null => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        if (baselineY <= yTop && baselineY >= yBottom) {
          return i;
        }
      }
      // eslint-disable-next-line no-restricted-syntax
      ...
      for (let i = 0; i < rowCount; i++) {
        const yTop = yBounds[i]!;
        const yBottom = yBounds[i + 1]!;
        if (baselineY > yTop + baselineEps) {
          continue;
        }
        if (baselineY < yBottom - baselineEps) {
          continue;
        }
    ...
    const colHasText: boolean[][] = Array.from({ length: rowCount }, () =>
      Array.from({ length: targetCols }, () => false),
    );
    ...
    for (const s of hSegs) {
      ...
      if (dist <= tol) {
        hBuckets[index]!.push(s);
      }
    }

    const vBoundaryPresent: boolean[][] = Array.from({ length: rowCount }, () =>
      Array.from({ length: targetCols + 1 }, () => false),
    );
    for (let ri = 0; ri < rowCount; ri++) {
      ...
      for (let bi = 1; bi < targetCols; bi++) {
        ...
        vBoundaryPresent[ri]![bi] = cov / bandH >= 0.3;
      }
    }
    ...
    const hBoundaryPresentBelow: boolean[][] = Array.from({ length: Math.max(0, rowCount - 1) }, () =>
      Array.from({ length: targetCols }, () => true),
    );
    for (let ri = 0; ri < rowCount - 1; ri++) {
      ...
      for (let ci = 0; ci < targetCols; ci++) {
        ...
        hBoundaryPresentBelow[ri]![ci] = cov / bandW >= 0.6;
      }
    }

    const coveredByRowSpan: boolean[][] = Array.from({ length: rowCount }, () =>
      Array.from({ length: targetCols }, () => false),
    );

    const mergeAccForRegion = ({
      r0,
      r1Exclusive,
      c0,
      c1Exclusive,
      ...
      while (ci < targetCols) {
        ...
        if (ri < maxRowSpanScanRows - 1) {
          ...
          while (rowSpan < maxSpan) {
            ...
            if (boundaryRow >= hBoundaryPresentBelow.length) {
              break;
            }
            ...
            for (let c = ci; c < colEnd; c++) {
              ...
              if (hBoundaryPresentBelow[boundaryRow]![c]) {
                canExtend = false;
                break;
              }
            }
  ...
  if (rowBands.length < opts.minRows) {
    return null;
  }
  ...
  const rowBandBounds = computeRowBandBounds(
    rowBands.map((r) => r.baselineY),
    effectiveBounds,
  );
