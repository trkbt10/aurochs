 ...
 */
function convertToLocal(point: PdfPoint, context: LocalConversionContext): { readonly x: Pixels; readonly y: Pixels } {
...
export function isApproximateEllipse(pdfPath: PdfPath): boolean {
  ...
  if (ops.length !== 5 && ops.length !== 6) {
    return false;
  }
  if (ops[0]?.type !== "moveTo") {
    return false;
  }

  for (let i = 1; i <= 4; i++) {
    if (ops[i]?.type !== "curveTo") {
      return false;
    }
  }

  if (ops.length === 6 && ops[5]?.type !== "closePath") {
    return false;
  }
...
export function detectRoundedRectangle(pdfPath: PdfPath): number | null {
  ...
  if (counts.curveCount !== 4) {
    return null;
  }
  if (counts.lineCount < 2) {
    return null;
  } // At least 2 lines (could be 4, but short edges may be omitted)
  ...
  if (radiusRatio < 0.01 || radiusRatio > 0.55) {
    return null;
  }
