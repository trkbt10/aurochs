...
import type { PdfPath } from "@aurochs/pdf/domain";
import {
...
import type { ParsedPath } from "@aurochs/pdf/parser/operator/index";
import {
  buildPath,
  buildPaths,
  builtPathToPdfPath,
  ...
  mergePaths,
} from "@aurochs/pdf/parser/path/path-builder";
...
describe("pdf/parser/graphics-state", () => {
  ...
  it("inverts matrices when possible", () => {
    ...
    if (!inv) {
      throw new Error("Expected invertible matrix");
    }
...
describe("pdf/parser/path-builder", () => {
  ...
  it("computePathBBox handles all op variants and empty paths", () => {
    ...
    expect(computePathBBox({ type: "path", operations: [] as const, paintOp: "stroke", graphicsState })).toEqual([
      0, 0, 0, 0,
    ]);
  });
  ...
  it("merges paths when paintOp matches", () => {
    ...
    const p2 = {
      operations: [{ type: "lineTo", x: 10, y: 10 }] as const,
      bounds: [10, 10, 10, 10] as const,
      paintOp: "fill" as const,
      graphicsState: g,
    };
    ...
    if (!merged) {
      throw new Error("Expected merged path");
    }
