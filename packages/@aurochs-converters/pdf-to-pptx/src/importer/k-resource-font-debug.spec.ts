...
import { getTableTags, extractTrueTypeMetrics, normalizeMetricsTo1000 } from "@aurochs/pdf/domain/font";
...
describe("PDF embedded font extraction", () => {
  ...
  it("should generate valid @font-face CSS", async () => {
    ...
    for (
      let match = fontFaceRegex.exec(document.embeddedFontCss!);
      match !== null;
      match = fontFaceRegex.exec(document.embeddedFontCss!)
    ) {
