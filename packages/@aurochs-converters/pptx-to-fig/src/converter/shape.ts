...
function convertGraphicFrame(
  ...
  switch (content.type) {
    ...
    case "diagram": {
      // Diagram content requires the layout engine to produce renderable shapes.
      // The domain type (DiagramReference) only contains resource IDs.
      // Without the integration layer's rendered output, we retain the bounding box.
      console.warn(
        `[pptx-to-fig] Diagram content in "${shape.nonVisual.name}" cannot be decomposed without the diagram layout engine. Retaining bounding box.`,
      );
      break;
    }
    case "oleObject": {
    ...
    case "chart": {
      console.warn(
        `[pptx-to-fig] Chart content in "${shape.nonVisual.name}" cannot be decomposed without the chart rendering pipeline. Retaining bounding box.`,
      );
      break;
    }
    case "unknown": {
...
function defaultTransform(): FigTransformResult {
  return {
    transform: { m00: 1, m01: 0, m02: 0, m10: 0, m11: 1, m12: 0 },
    size: { x: 100, y: 100 },
  };
