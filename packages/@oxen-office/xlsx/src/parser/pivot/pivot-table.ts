...
function parsePivotField(fieldElement: XmlElement): XlsxPivotField {
  ...
  const items = itemsEl
    ? getChildren(itemsEl, "item").map(parsePivotItem)
    : undefined;
 ...
 */
export function parsePivotTable(
  pivotTableElement: XmlElement,
  xmlPath: string,
): XlsxPivotTable {
  ...
  const pivotFields = pivotFieldsEl
    ? getChildren(pivotFieldsEl, "pivotField").map(parsePivotField)
    : undefined;

  // Parse row fields
  const rowFieldsEl = getChild(pivotTableElement, "rowFields");
  const rowFields = rowFieldsEl
    ? getChildren(rowFieldsEl, "field").map(parseFieldReference)
    : undefined;

  // Parse column fields
  const colFieldsEl = getChild(pivotTableElement, "colFields");
  const colFields = colFieldsEl
    ? getChildren(colFieldsEl, "field").map(parseFieldReference)
    : undefined;

  // Parse page fields
  const pageFieldsEl = getChild(pivotTableElement, "pageFields");
  const pageFields = pageFieldsEl
    ? getChildren(pageFieldsEl, "pageField").map(parseFieldReference)
    : undefined;

  // Parse data fields
  const dataFieldsEl = getChild(pivotTableElement, "dataFields");
  const dataFields = dataFieldsEl
    ? getChildren(dataFieldsEl, "dataField").map(parseDataField)
    : undefined;
