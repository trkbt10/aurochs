...
import { createEmptyZipPackage, type ZipPackage } from "@oxen/zip";
import type { PptPresentation, PptSlide } from "../domain/types";
import type { PptParseContext } from "../parse-context";
import {
  APP_XML, ROOT_RELS_XML,
  MINIMAL_THEME, MINIMAL_SLIDE_MASTER, MINIMAL_SLIDE_LAYOUT,
} from "./template";
import {
...
import {
  embedSlideImages,
  buildImageRelationships,
  collectUsedImageIndices,
} from "./image-embed";
 ...
 */
export function convertPptToPptx(
  ppt: PptPresentation,
  _ctx: PptParseContext,
): PptxConvertResult {
  ...
  for (let i = 0; i < slideCount; i++) {
    ...
    // Build relationships
    const extraRels: SlideRelationship[] = [
