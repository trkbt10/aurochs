...
import type { Axis, CategoryAxis, ValueAxis, DateAxis, ChartTitle, TickMark, DisplayUnits } from "@oxen-office/chart/domain";
...
export function renderGridlines(
  ...
  if (isLogScale(valAxis)) {
    ...
    for (const val of majorValues) {
      ...
      lines.push(
        `<line x1="0" y1="${y}" x2="${chartWidth}" y2="${y}" stroke="${gridlineColor}" stroke-width="1"/>`
      );
    }
  } else {
    // Linear scale
...
export function renderCategoryAxisTickMarks(
  ...
  if (catAxis.majorTickMark !== "none") {
    for (let i = 0; i <= numCategories; i++) {
      ...
      marks.push(renderTickMark({ tickMarkStyle: catAxis.majorTickMark, position: "bottom", x, y: chartHeight, axisColor }));
    }
  }
