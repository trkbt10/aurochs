...
import type { ErrorBars } from "@oxen-office/chart/domain";
import type { SeriesData } from "./render-types";
import { createTestChartRenderContext } from "./test-utils";

const { fillResolver } = createTestChartRenderContext();
...
describe("Error Bars Rendering (ECMA-376 21.2.2.58)", () => {
  describe("Fixed value error bars", () => {
    test("renders fixed value error bars", () => {
      ...
      const svg = renderErrorBars({ errorBars, seriesData: mockSeriesData, chartWidth: mockChartWidth, chartHeight: mockChartHeight, valueRange: mockValueRange, fillResolver });
  ...
  describe("renderAllErrorBars helper", () => {
    test("returns empty string for undefined error bars", () => {
      const svg = renderAllErrorBars({ errorBarsArray: undefined, seriesData: mockSeriesData, chartWidth: mockChartWidth, chartHeight: mockChartHeight, valueRange: mockValueRange, fillResolver });

      expect(svg).toBe("");
    });

    test("returns empty string for empty error bars array", () => {
      const svg = renderAllErrorBars({ errorBarsArray: [], seriesData: mockSeriesData, chartWidth: mockChartWidth, chartHeight: mockChartHeight, valueRange: mockValueRange, fillResolver });
