...
import type { SeriesData, ValueAxisConfig, ChartContent, LineChartConfig } from "../render-types";
...
import type { ChartRenderContext, FillResolver } from "../types";
...
export function generateLineChart(
  ...
  data.forEach((series, seriesIndex) => {
    ...
    if (lineConfig?.dropLines) {
      const dropStyle = extractDropLineStyle(lineConfig.dropLines, fillResolver);
      coords.forEach((coord) => {
        elements.push(
  ...
  if (lineConfig?.hiLowLines && data.length >= 2) {
    const hiLowStyle = extractHiLowLineStyle(lineConfig.hiLowLines, fillResolver);
  ...
  const axes =
    ...
    `<line x1="0" y1="${zeroY}" x2="${chartWidth}" y2="${zeroY}" stroke="${axisColor}" stroke-width="1"/>`;
