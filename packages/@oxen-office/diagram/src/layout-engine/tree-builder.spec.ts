...
import type { DiagramDataModel, DiagramPoint, DiagramConnection } from "../domain/types";
import {
  buildDiagramTree,
  traverseTree,
  countNodes,
...
type FakeTextBody = ReturnType<typeof createTextBody>;

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function isFakeTextBody(value: unknown): value is FakeTextBody {
  if (!isRecord(value)) {
    return false;
  }
...
function extractTextFromFakeTextBody(textBody: unknown): string | undefined {
  if (!isFakeTextBody(textBody)) {
    return undefined;
  }
  const text = textBody.paragraphs.flatMap((p) => p.runs.map((r) => r.text)).join("");
...
// eslint-disable-next-line custom/max-params -- Test helper is clearer with positional args.
...
describe("getNodeText", () => {
  it("extracts text from node", () => {
    ...
    expect(getNodeTextWith(result.roots[0], extractTextFromFakeTextBody)).toBe("Hello World");
  });
  ...
  it("returns empty string for node without text", () => {
    ...
    expect(getNodeTextWith(result.roots[0], extractTextFromFakeTextBody)).toBe("");
  });
