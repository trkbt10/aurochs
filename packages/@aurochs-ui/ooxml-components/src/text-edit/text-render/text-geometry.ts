...
import type { LayoutLine, PositionedSpan } from "@aurochs-renderer/pptx/text-layout";
 ...
 */
export function getFontSizeForRange(line: LayoutLine, startOffset: number, endOffset: number): Points {
  if (line.spans.length === 0) {
    return DEFAULT_FONT_SIZE_PT;
  }
 ...
 */
export function getTextVisualBounds(baselineY: Pixels, fontSizePt: Points, fontFamily?: string): TextVisualBounds {
 ...
 */
export function getVisualBoundsAtOffset(line: LayoutLine, charOffset: number): TextVisualBounds {
 ...
 */
export function getVisualBoundsForRange(line: LayoutLine, startOffset: number, endOffset: number): TextVisualBounds {
  const { fontSizePt, fontFamily } = getFontMetricsForRange(line, startOffset, endOffset);
  return getTextVisualBounds(line.y, fontSizePt, fontFamily);
}

function getSpanAtOffset(line: LayoutLine, charOffset: number): PositionedSpan | undefined {
  const match = line.spans.reduce<{
    readonly remaining: number;
...
export function getLineEndX(line: LayoutLine): Pixels {
  return line.spans.reduce((x, span) => (x + (span.width as number) + (span.dx as number)) as Pixels, line.x);
}
