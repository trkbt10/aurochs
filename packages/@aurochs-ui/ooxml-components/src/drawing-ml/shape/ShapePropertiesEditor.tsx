...
import { LineEditor, createDefaultLine } from "../line";
...
export function ShapePropertiesEditor({
  ...
  return (
    <>
      ...
      {showFill && <FillEditor value={value.fill ?? createNoFill()} onChange={handleFillChange} disabled={disabled} />}

      {showLine && (
        <LineEditor value={value.line ?? createDefaultLine()} onChange={handleLineChange} disabled={disabled} />
      )}
      ...
      {showScene3d && (
        <>
          <Toggle checked={!!value.scene3d} onChange={handleScene3dToggle} label="3D Scene" disabled={disabled} />
          {value.scene3d && <Scene3dEditor value={value.scene3d} onChange={handleScene3dChange} disabled={disabled} />}
        </>
      )}

      {showShape3d && (
        <>
          <Toggle checked={!!value.shape3d} onChange={handleShape3dToggle} label="3D Shape" disabled={disabled} />
          {value.shape3d && <Shape3dEditor value={value.shape3d} onChange={handleShape3dChange} disabled={disabled} />}
        </>
      )}
    </>
  );
