...
export type EditablePathSegmentLine = {
  readonly key: string;
...
export function getEditableControlLines(commands: readonly EditablePathCommand[]): readonly EditablePathSegmentLine[] {
  return commands.flatMap((command, commandIndex) => {
    ...
    switch (command.type) {
      ...
      case "C": {
        ...
        const lines: EditablePathSegmentLine[] = [];
        if (previous) {
