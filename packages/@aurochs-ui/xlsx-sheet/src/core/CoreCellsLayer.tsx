/**
 * @file Core cells layer
 *
 * Renders visible cell contents for the current viewport range.
 * This is the single source of truth for cell rendering, shared by both
...
import {
  ...
  formatFormulaScalarForDisplay,
  resolveCellFormatCode,
} from "../selectors/cell-display-text";
...
import { spacingTokens, colorTokens } from "@aurochs-ui/ui-components";
import type { SheetLayout } from "../selectors/sheet-layout";
import { XlsxCellCanvasText, type XlsxCellCanvasTextProps } from "../grid/cell-text";
import type { VisibleRange } from "./types";

export type CoreCellsLayerProps = {
  ...
  /**
   * Whether to evaluate and apply conditional formatting rules.
   * Default: true. Set to false to skip conditional formatting evaluation
   * (e.g. for lightweight viewing scenarios).
   */
 ...
 *
 * This is the single source of truth for cell rendering. Both the editor
 * and viewer use this component for consistent visual output.
 */
export function CoreCellsLayer({
  sheetIndex,
  sheet,
  styles,
  layout,
  ...
  const cellNodes = useMemo(() => {
    ...
    const hasConditionalFormatting = enableConditionalFormatting && (sheet.conditionalFormattings?.length ?? 0) > 0;
      ...
      for (let col0 = colRange.start; col0 <= colRange.end; col0 += 1) {
        ...
        if (merge) {
          ...
          const conditionalFormat = resolveConditionalFormatMaybe({
            enabled: hasConditionalFormatting,
            sheet,
            styles,
            sheetIndex,
