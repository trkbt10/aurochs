...
import type { XlsxConditionalFormattingRule } from "@aurochs-office/xlsx/domain/conditional-formatting";
...
import { isFormulaError } from "@aurochs-office/xlsx/formula/types";
...
function evaluateRuleMatchesCell({
  ...
  if (rule.type === "cellIs") {
    ...
    const evaluated = formulaEvaluator.evaluateFormulaResult(
      sheetIndex,
      address,
      shiftConditionalFormula(criterion, anchor, address),
    );
  ...
  if (rule.type === "expression") {
    ...
    const evaluated = formulaEvaluator.evaluateFormulaResult(
      sheetIndex,
      address,
      shiftConditionalFormula(expr, anchor, address),
    );
    if (Array.isArray(evaluated)) {
      return false;
    }
