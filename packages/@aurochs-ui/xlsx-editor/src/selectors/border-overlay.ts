...
export function buildBorderOverlayLines(params: {
  ...
  const { sheet, styles, layout, rowRange, colRange, rowCount, colCount, scrollTop, scrollLeft, defaultBorderColor } =
    params;
  ...
  const getMergeKey = (col0: number, row0: number): string | undefined => {
    ...
    const col1 = col0 + 1;
    const row1 = row0 + 1;
    const merge = findMergeForCell(normalizedMerges, {
      col: colIdx(col1),
      row: rowIdx(row1),
      colAbsolute: false,
      rowAbsolute: false,
  ...
  const getCellEdges = (col0: number, row0: number): ReturnType<typeof resolveCellBorderDecoration> | undefined => {
    ...
    if (mergeKey) {
      ...
      const resolved = resolveCellBorderDecoration({
        styles,
        sheet,
        address: merge.origin,
        cell: originCell,
        defaultBorderColor,
      });
    ...
    for (let row0 = rowRange.start; row0 <= rowRange.end; row0 += 1) {
      ...
      if (!chosen) {
        if (activeState.active) {
          emitLine({
            lines,
            baseKey: `vb-${boundaryCol0}-${row0}-seg`,
            edge: activeState.active.edge,
            x1: x,
            y1: activeState.active.startY,
            x2: x,
            y2: activeState.active.endY,
          });
      ...
      if (
        activeState.active &&
        sameEdge(activeState.active.edge, chosen) &&
        Math.abs(activeState.active.endY - y1) < 0.001
      ) {
    ...
    for (let col0 = colRange.start; col0 <= colRange.end; col0 += 1) {
      ...
      if (!chosen) {
        if (activeState.active) {
          emitLine({
            lines,
            baseKey: `hb-${boundaryRow0}-${col0}-seg`,
            edge: activeState.active.edge,
            x1: activeState.active.startX,
            y1: y,
            x2: activeState.active.endX,
            y2: y,
          });
          activeState.active = undefined;
        }
      ...
      if (
        activeState.active &&
        sameEdge(activeState.active.edge, chosen) &&
        Math.abs(activeState.active.endX - x1) < 0.001
      ) {
