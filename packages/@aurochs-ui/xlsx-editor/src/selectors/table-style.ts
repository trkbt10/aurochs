...
import type { XlsxTableStyle, XlsxTableStyleElementType } from "@aurochs-office/xlsx/domain/style/table-style";
...
export function resolveCellTableStyleDifferentialFormat(params: {
  ...
  const table = tables.find(
    (candidate) => candidate.sheetIndex === sheetIndex && rangeContainsAddress(candidate.ref, address),
  );
  if (!table) {
    return undefined;
  }
