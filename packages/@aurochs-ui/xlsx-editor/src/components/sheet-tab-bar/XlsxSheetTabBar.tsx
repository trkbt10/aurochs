...
export function XlsxSheetTabBar({ style }: XlsxSheetTabBarProps) {
  ...
  const [contextMenuState, setContextMenuState] = useState<SheetTabContextMenuState>(createClosedContextMenuState());
  ...
  const { handleDragStart, handleDragOver, handleDrop, handleDragEnd, isDragging, isDropTarget } = useSheetTabDragDrop({
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent, sheetIndex: number) => {
    e.preventDefault();
    setContextMenuState({
      type: "open",
      x: e.clientX,
