...
function mergeIfContiguous(active: SegmentAccumulator, nextStart: number, nextEnd: number): SegmentAccumulator {
  if (!active) {
...
export function getVisibleGridLineSegments(params: {
  ...
  const getMergeKey = (col0: number, row0: number): string | undefined =>
    getMergeKeyForCell({ merges: normalizedMerges, cache: mergeKeyCache, col0, row0 });
  ...
  for (let boundaryCol0 = colRange.start; boundaryCol0 <= colRange.end + 1; boundaryCol0 += 1) {
    ...
    for (let row0 = rowRange.start; row0 <= rowRange.end; row0 += 1) {
      ...
      if (isInternalMergeBoundary) {
        if (state.active) {
          vertical.push({
            key: `v-${boundaryCol0}-${row0}-seg`,
            x1: x,
            y1: state.active.start,
            x2: x,
            y2: state.active.end,
          });
          state.active = null;
  ...
  for (let boundaryRow0 = rowRange.start; boundaryRow0 <= rowRange.end + 1; boundaryRow0 += 1) {
    ...
    for (let col0 = colRange.start; col0 <= colRange.end; col0 += 1) {
      ...
      if (isInternalMergeBoundary) {
        if (state.active) {
          horizontal.push({
            key: `h-${boundaryRow0}-${col0}-seg`,
            x1: state.active.start,
            y1: y,
            x2: state.active.end,
            y2: y,
          });
