...
describe("xlsx-editor/components/sheet-grid/selection-geometry", () => {
  ...
  it("computes active cell rect", () => {
    ...
    expect(getActiveCellRect({ cell: addr(1, 1), layout, scrollTop: 0, scrollLeft: 0 })).toEqual({
      left: 0,
      top: 0,
      width: 50,
      height: 20,
  ...
  it("computes selected range rect", () => {
    ...
    expect(
      getSelectedRangeRect({ range: { start: addr(1, 1), end: addr(2, 2) }, layout, scrollTop: 0, scrollLeft: 0 }),
    ).toEqual({
      left: 0,
      top: 0,
      width: 100,
