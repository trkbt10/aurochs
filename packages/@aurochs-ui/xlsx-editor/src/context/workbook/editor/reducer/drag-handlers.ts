...
type PreviewRangeSelectAction = Extract<XlsxEditorAction, { type: "PREVIEW_RANGE_SELECT" }>;
type StartFillDragAction = Extract<XlsxEditorAction, { type: "START_FILL_DRAG" }>;
type PreviewFillDragAction = Extract<XlsxEditorAction, { type: "PREVIEW_FILL_DRAG" }>;
type StartRowResizeAction = Extract<XlsxEditorAction, { type: "START_ROW_RESIZE" }>;
type PreviewRowResizeAction = Extract<XlsxEditorAction, { type: "PREVIEW_ROW_RESIZE" }>;
type StartColumnResizeAction = Extract<XlsxEditorAction, { type: "START_COLUMN_RESIZE" }>;
type PreviewColumnResizeAction = Extract<XlsxEditorAction, { type: "PREVIEW_COLUMN_RESIZE" }>;

function handleStartRangeSelect(state: XlsxEditorState, action: StartRangeSelectAction): XlsxEditorState {
  return {
    ...state,
...
function handlePreviewRangeSelect(state: XlsxEditorState, action: PreviewRangeSelectAction): XlsxEditorState {
...
function handleStartFillDrag(state: XlsxEditorState, action: StartFillDragAction): XlsxEditorState {
  return {
    ...state,
...
function handlePreviewFillDrag(state: XlsxEditorState, action: PreviewFillDragAction): XlsxEditorState {
...
function handleStartRowResize(state: XlsxEditorState, action: StartRowResizeAction): XlsxEditorState {
  return {
    ...state,
...
function handlePreviewRowResize(state: XlsxEditorState, action: PreviewRowResizeAction): XlsxEditorState {
  const { drag } = state;
...
function handleCommitRowResize(state: XlsxEditorState): XlsxEditorState {
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
    setRowHeight(worksheet, drag.rowIndex, drag.originalHeight),
  );
...
function handleStartColumnResize(state: XlsxEditorState, action: StartColumnResizeAction): XlsxEditorState {
  return {
    ...state,
    drag: startColumnResizeDrag(action.colIndex, action.startX, action.originalWidth),
  };
}

function handlePreviewColumnResize(state: XlsxEditorState, action: PreviewColumnResizeAction): XlsxEditorState {
  const { drag } = state;
...
function handleCommitColumnResize(state: XlsxEditorState): XlsxEditorState {
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
    setColumnWidth(worksheet, drag.colIndex, drag.originalWidth),
  );
