...
} from "@aurochs-office/xlsx/domain/style/mutation";
import type { StyleId } from "@aurochs-office/xlsx/domain/types";
import { isEqualCellXf } from "@aurochs-office/xlsx/domain/style/mutation/equality";
import type { XlsxCellStyle, XlsxStyleSheet } from "@aurochs-office/xlsx/domain/style/types";
 ...
 */
function addCellStyle(styles: XlsxStyleSheet, cellStyle: XlsxCellStyle): XlsxStyleSheet {
 ...
 */
function removeCellStyleAtIndex(styles: XlsxStyleSheet, index: number): XlsxStyleSheet {
...
function handleApplyStyle(state: XlsxEditorState, action: ApplyStyleAction): XlsxEditorState {
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
    applyStyleToRange(worksheet, action.range, action.styleId),
  );
 ...
 */
function handleApplyNamedStyle(state: XlsxEditorState, action: ApplyNamedStyleAction): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  }
  ...
  const workbookWithSheetUpdated = updateWorksheetInWorkbook(currentWorkbook, sheetIndex, (worksheet) =>
    applyStyleToRange(worksheet, action.range, nextStyleId),
  );
...
function handleSetSelectionFormat(state: XlsxEditorState, action: SetSelectionFormatAction): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  }
  ...
  const updatedWorkbook =
    workbookWithSheetUpdated.styles === styles ? workbookWithSheetUpdated : { ...workbookWithSheetUpdated, styles };
 ...
 */
function handleCreateNamedStyle(state: XlsxEditorState, action: CreateNamedStyleAction): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  }
 ...
 */
function handleDeleteNamedStyle(state: XlsxEditorState, action: DeleteNamedStyleAction): XlsxEditorState {
  const currentWorkbook = state.workbookHistory.present;
  const { styles } = currentWorkbook;
...
export const formattingHandlers: HandlerMap = {
  ...
  MERGE_CELLS: (state: XlsxEditorState, action: MergeCellsAction) => {
    ...
    const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
      mergeCells(worksheet, action.range),
    );
  ...
  UNMERGE_CELLS: (state: XlsxEditorState, action: UnmergeCellsAction) => {
    ...
    const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
      unmergeCells(worksheet, action.range),
    );
