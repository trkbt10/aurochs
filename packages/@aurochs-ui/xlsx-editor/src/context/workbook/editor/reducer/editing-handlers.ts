...
type CommitCellEditAction = Extract<XlsxEditorAction, { type: "COMMIT_CELL_EDIT" }>;
...
function handleCommitCellEdit(state: XlsxEditorState, action: CommitCellEditAction): XlsxEditorState {
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, (worksheet) =>
    updateCell(worksheet, address, action.value),
  );
