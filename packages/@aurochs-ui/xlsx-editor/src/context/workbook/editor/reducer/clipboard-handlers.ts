...
function clearSourceIfCut(worksheet: XlsxWorksheet, clipboard: XlsxClipboardContent): XlsxWorksheet {
  if (clipboard.isCut) {
...
export const clipboardHandlers: HandlerMap = {
  ...
  CUT: (state) => {
    ...
    const updatedWorkbook = replaceWorksheetInWorkbook(state.workbookHistory.present, sheetIndex, updatedWorksheet);
