...
export function XlsxWorkbookEditorProvider({
  ...
  const [state, dispatch] = useReducer(xlsxEditorReducer, initialWorkbook, createInitialState);

  const workbook = state.workbookHistory.present;
  const activeSheetIndex = state.activeSheetIndex;
  const activeSheet = activeSheetIndex === undefined ? undefined : workbook.sheets[activeSheetIndex];
  ...
  return <XlsxWorkbookEditorContext.Provider value={value}>{children}</XlsxWorkbookEditorContext.Provider>;
}
