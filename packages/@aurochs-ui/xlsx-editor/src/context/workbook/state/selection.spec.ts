...
describe("isSelectionEmpty", () => {
  it("returns true only when selectedRange and activeCell are undefined", () => {
    ...
    expect(isSelectionEmpty({ selectedRange: undefined, activeCell: addr(1, 1), multiRanges: undefined })).toBe(false);
  });
});
...
describe("isSingleCellSelection", () => {
  ...
  it("returns false for range selection", () => {
    expect(isSingleCellSelection(createRangeSelection(range({ startCol: 1, startRow: 1, endCol: 2, endRow: 2 })))).toBe(
      false,
    );
  });

  it("returns false when multiRanges exist", () => {
    const selection = addRangeToSelection(
      createSingleCellSelection(addr(1, 1)),
      range({ startCol: 2, startRow: 2, endCol: 2, endRow: 2 }),
    );
