...
function buildCellLookup(worksheet: XlsxWorksheet): ReadonlyMap<number, ReadonlyMap<number, Cell>> {
 ...
 */
export function getCellValue(worksheet: XlsxWorksheet, address: CellAddress): CellValue | undefined {
 ...
 */
export function getCellsInRange(worksheet: XlsxWorksheet, range: CellRange): readonly Cell[] {
 ...
 */
export function getCellValuesInRange(worksheet: XlsxWorksheet, range: CellRange): readonly (readonly CellValue[])[] {
  const { minRow, maxRow, minCol, maxCol } = getRangeBounds(range);
...
function updateBounds(bounds: RangeBounds | undefined, rowNumber: number, colNumber: number): RangeBounds {
  if (bounds === undefined) {
 ...
 */
export function findCells(worksheet: XlsxWorksheet, predicate: (cell: Cell) => boolean): readonly Cell[] {
  const result: Cell[] = [];
  for (const row of worksheet.rows) {
    for (const cell of row.cells) {
      if (predicate(cell)) {
