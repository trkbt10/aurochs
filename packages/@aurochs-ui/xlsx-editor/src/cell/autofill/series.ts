 ...
 */
export function computeNumericFillValue(
  series: Extract<PatternSeries, { type: "numeric" }>,
  stepAmount: number,
  isForward: boolean,
): number {
  if (isForward) {
 ...
 */
export function computeDateFillValue(
  series: Extract<PatternSeries, { type: "date" }>,
  stepAmount: number,
  isForward: boolean,
): Date {
  if (isForward) {
