...
describe("cell/query", () => {
  ...
  it("getCellValuesInRange returns a 2D array (row-major) and fills missing with empty", () => {
    ...
    expect(values).toEqual([
      [{ type: "string", value: "A1" }, { type: "empty" }, { type: "empty" }],
      [{ type: "empty" }, { type: "empty" }, { type: "number", value: 10 }],
    ]);
  });
  ...
  it("findCells filters by predicate", () => {
    ...
    expect(cells.map((c) => (c.value.type === "string" ? c.value.value : ""))).toEqual(["hello", "world"]);
  });
});
