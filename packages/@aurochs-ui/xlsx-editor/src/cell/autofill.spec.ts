...
describe("applyAutofillToWorksheet", () => {
  it("fills numeric series down for a single cell", () => {
    ...
    expect(getCell(updated, { col: colIdx(1), row: rowIdx(1), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 5 },
    );
    expect(getCell(updated, { col: colIdx(1), row: rowIdx(2), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 6 },
    );
    expect(getCell(updated, { col: colIdx(1), row: rowIdx(3), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 7 },
    );
  });
  ...
  it("shifts formula references when filling down", () => {
    ...
    expect(
      getCell(updated, { col: colIdx(2), row: rowIdx(2), colAbsolute: false, rowAbsolute: false })?.formula,
    ).toMatchObject({ expression: "A2+1" });
    expect(
      getCell(updated, { col: colIdx(2), row: rowIdx(3), colAbsolute: false, rowAbsolute: false })?.formula,
    ).toMatchObject({ expression: "A3+1" });
  });
  ...
  it("fills numeric series right and overwrites destination cells without touching baseRange", () => {
    ...
    expect(getCell(updated, { col: colIdx(2), row: rowIdx(1), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 2 },
    );
    expect(getCell(updated, { col: colIdx(3), row: rowIdx(1), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 3 },
    );
    expect(getCell(updated, { col: colIdx(4), row: rowIdx(1), colAbsolute: false, rowAbsolute: false })?.value).toEqual(
      { type: "number", value: 4 },
    );
  });
  ...
  it("copies effective styleId from base cells", () => {
    ...
    expect(
      getCell(updated, { col: colIdx(1), row: rowIdx(2), colAbsolute: false, rowAbsolute: false })?.styleId,
    ).toEqual(styleId(5));
  });
});
