...
function isNameUnique(workbook: XlsxWorkbook, name: string, excludeIndex?: number): boolean {
 ...
 */
export function addSheet(workbook: XlsxWorkbook, name: string, afterIndex?: number): XlsxWorkbook {
 ...
 * Rename a sheet
 */
export function renameSheet(workbook: XlsxWorkbook, sheetIndex: number, newName: string): XlsxWorkbook {
  assertValidSheetIndex(workbook, sheetIndex);
  ...
  const sheets = workbook.sheets.map((sheet, idx) => (idx === sheetIndex ? { ...sheet, name: newName } : sheet));
  return { ...workbook, sheets };
}
 ...
 */
export function moveSheet(workbook: XlsxWorkbook, fromIndex: number, toIndex: number): XlsxWorkbook {
