...
import type { ChartTitle, Layout, ChartShapeProperties } from "@aurochs-office/chart/domain";
 ...
 */
export function ChartTitleEditor({ value, onChange, disabled, className, style, adapters }: ChartTitleEditorProps) {
  const title = value ?? {};
      ...
      <div style={{ ...containerStyle, ...style }} className={className}>
        <FieldRow>
          <FieldGroup label="Overlay" style={{ flex: 1 }}>
            <Toggle checked={title.overlay ?? false} onChange={(v) => updateField("overlay", v)} disabled={disabled} />
          </FieldGroup>
        </FieldRow>
