...
import type { DataLabels, DataLabel, ChartShapeProperties, Layout } from "@aurochs-office/chart/domain";
 ...
 */
export function DataLabelsEditor({ value, onChange, disabled, className, style }: DataLabelsEditorProps) {
      ...
      <>
        <FieldRow>
          <FieldGroup label="Show Value" style={{ flex: 1 }}>
            <Toggle checked={value.showVal ?? false} onChange={(v) => updateField("showVal", v)} disabled={disabled} />
          </FieldGroup>
        ...
        <FieldRow>
          <FieldGroup label="Position" style={{ flex: 1 }}>
            <Select
              ...
              onChange={(v) => updateField("position", v as DataLabels["position"])}
...
function DataLabelEditor({ value, onChange, onRemove, disabled }: DataLabelEditorProps) {
  const updateField = useCallback(
      ...
      <>
        ...
        <FieldRow>
          <FieldGroup label="Position" style={{ flex: 1 }}>
            <Select
              value={value.position ?? "ctr"}
              onChange={(v) => updateField("position", v as DataLabel["position"])}
              options={positionOptions}
              disabled={disabled}
            />
          </FieldGroup>
