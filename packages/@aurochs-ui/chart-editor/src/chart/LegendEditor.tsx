...
import type { Legend, LegendEntry, Layout, ChartShapeProperties } from "@aurochs-office/chart/domain";
 ...
 */
export function LegendEditor({ value, onChange, disabled, className, style }: LegendEditorProps) {
      ...
      <>
        ...
        <FieldRow>
          <FieldGroup label="Overlay" style={{ flex: 1 }}>
            <Toggle checked={value.overlay ?? false} onChange={(v) => updateField("overlay", v)} disabled={disabled} />
          </FieldGroup>
        </FieldRow>
      </>
...
function LegendEntryEditor({ value, onChange, onRemove, disabled }: LegendEntryEditorProps) {
  const updateField = useCallback(
