...
import type { TableWidth, TableAlignment, TableGridColumn } from "@aurochs-office/ooxml/domain/table";
 ...
 */
export function createTable(rows: number = 1, columns: number = 1, properties?: DocxTableProperties): DocxTable {
 ...
 */
export function createTableRow(cellCount: number = 1, properties?: DocxTableRowProperties): DocxTableRow {
 ...
 */
export function setRowProperties(row: DocxTableRow, properties: DocxTableRowProperties | undefined): DocxTableRow {
  return {
    ...row,
    properties,
  };
 ...
 */
export function mergeRowProperties(row: DocxTableRow, properties: Partial<DocxTableRowProperties>): DocxTableRow {
  return {
    ...row,
    properties: {
      ...row.properties,
...
export function setColumnWidth(table: DocxTable, colIndex: number, width: Pixels): DocxTable {
  ...
  const newColumns = columns.map((col, i) => (i === colIndex ? { ...col, width } : col));
 ...
 */
function updateCellAtIndex(cells: readonly DocxTableCell[], colIndex: number, newCell: DocxTableCell): DocxTableCell[] {
 ...
 */
function updateRowCells(row: DocxTableRow, colIndex: number, newCell: DocxTableCell): DocxTableRow {
 ...
 * Set cell properties.
 */
export function setCellProperties(cell: DocxTableCell, properties: DocxTableCellProperties | undefined): DocxTableCell {
  return {
    ...cell,
    properties,
  };
 ...
 */
export function mergeCellProperties(cell: DocxTableCell, properties: Partial<DocxTableCellProperties>): DocxTableCell {
  return {
    ...cell,
    properties: {
      ...cell.properties,
 ...
 * Set cell content.
 */
export function setCellContent(cell: DocxTableCell, content: readonly (DocxParagraph | DocxTable)[]): DocxTableCell {
  return {
    ...cell,
    content,
  };
 ...
 */
export function appendCellContent(cell: DocxTableCell, content: DocxParagraph | DocxTable): DocxTableCell {
  return {
    ...cell,
 ...
 */
export function setTableProperties(table: DocxTable, properties: DocxTableProperties | undefined): DocxTable {
  return {
    ...table,
    properties,
  };
 ...
 */
export function mergeTableProperties(table: DocxTable, properties: Partial<DocxTableProperties>): DocxTable {
  return {
    ...table,
    properties: {
      ...table.properties,
...
export function mergeCellsHorizontally({
  ...
  startCol,
  endCol,
}: MergeCellsHorizontallyOptions): DocxTable {
  ...
  const newCells = [...row.cells.slice(0, startCol), mergedCell, ...row.cells.slice(endCol + 1)];
 ...
 */
function getUpdatedCellForVerticalMerge(cell: DocxTableCell, merge: "restart" | "continue" | undefined): DocxTableCell {
  if (merge) {
...
export function mergeCellsVertically({ table, colIndex, startRow, endRow }: MergeCellsVerticallyOptions): DocxTable {
  ...
  const rowIndices = Array.from({ length: endRow - startRow }, (_, i) => startRow + 1 + i);
