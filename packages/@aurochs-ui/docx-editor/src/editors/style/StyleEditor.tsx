...
import type { DocxStyle, DocxStyleType } from "@aurochs-office/docx/domain/styles";
...
export function StyleEditor({
  ...
  const basedOnOptions: SelectOption<string>[] = [
    { value: "", label: "(None)" },
    ...compatibleStyles.filter((s) => s.id !== value.styleId).map((s) => ({ value: s.id, label: s.name })),
  ];
  ...
  return (
    <div className={containerClassName} style={style}>
      ...
      <FieldGroup label="Style Type">
        <Select value={value.type} onChange={handleTypeChange} options={STYLE_TYPE_OPTIONS} disabled={disabled} />
      </FieldGroup>
