 ...
 */
export function insertRunContent(run: DocxRun, index: number, content: DocxRunContent): DocxRun {
  const newContent = [...run.content.slice(0, index), content, ...run.content.slice(index)];
  return {
    ...run,
    content: newContent,
  };
 ...
 */
export function setRunProperties(run: DocxRun, properties: DocxRunProperties | undefined): DocxRun {
  return {
    ...run,
    properties,
  };
 ...
 */
export function mergeRunProperties(run: DocxRun, properties: Partial<DocxRunProperties>): DocxRun {
  return {
    ...run,
    properties: {
      ...run.properties,
 ...
 */
export function removeRunProperty<K extends keyof DocxRunProperties>(run: DocxRun, ...keys: K[]): DocxRun {
  if (!run.properties) {
    return run;
  }
 ...
 */
export function setHighlightColor(run: DocxRun, color: DocxHighlightColor | undefined): DocxRun {
