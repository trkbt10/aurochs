...
import type { Twips, DocxNumId, DocxIlvl } from "@aurochs-office/docx/domain/types";
 ...
 */
export function setParagraphContent(paragraph: DocxParagraph, content: readonly DocxParagraphContent[]): DocxParagraph {
  return {
    ...paragraph,
    content,
  };
 ...
 */
export function appendParagraphContent(paragraph: DocxParagraph, content: DocxParagraphContent): DocxParagraph {
  return {
    ...paragraph,
 ...
 */
export function prependParagraphContent(paragraph: DocxParagraph, content: DocxParagraphContent): DocxParagraph {
  return {
    ...paragraph,
...
export function insertParagraphContent(
  ...
  const newContent = [...paragraph.content.slice(0, index), content, ...paragraph.content.slice(index)];
  return {
    ...paragraph,
    content: newContent,
  };
 ...
 */
export function removeParagraphContent(paragraph: DocxParagraph, index: number): DocxParagraph {
  return {
    ...paragraph,
...
export function getParagraphText(paragraph: DocxParagraph): string {
  return getParagraphRuns(paragraph).map(getRunText).join("");
}
 ...
 */
export function deleteTextRange(paragraph: DocxParagraph, startOffset: number, endOffset: number): DocxParagraph {
 ...
 */
export function setParagraphSpacing(paragraph: DocxParagraph, spacing: DocxParagraphSpacing): DocxParagraph {
 ...
 */
export function setParagraphIndent(paragraph: DocxParagraph, indent: DocxParagraphIndent): DocxParagraph {
 ...
 */
export function splitParagraph(paragraph: DocxParagraph, charOffset: number): [DocxParagraph, DocxParagraph] {
  ...
  const { beforeContent, afterContent } = splitParagraphContent(paragraph.content, runIndex, offsetInRun);
 ...
 */
function appendRunIfNotEmpty(runs: DocxParagraphContent[], run: DocxRun): DocxParagraphContent[] {
 ...
 */
function getMergedParagraphContent(first: DocxParagraph, second: DocxParagraph): DocxParagraphContent[] {
  ...
  if (propertiesMatch(lastFirstRun.properties, firstSecondRun.properties)) {
    ...
    return [...first.content.slice(0, -1), mergedRun, ...second.content.slice(1)];
  }
 ...
 */
export function mergeParagraphs(first: DocxParagraph, second: DocxParagraph): DocxParagraph {
    ...
    if (runIdx < start.runIndex || runIdx > end.runIndex) {
      ...
      if (start.offsetInRun === 0 && end.offsetInRun >= text.length) {
        ...
        if (middleText.length > 0) {
          newContent.push(
            mergeRunProperties({ ...content, content: [{ type: "text", value: middleText }] }, properties),
          );
        }
      ...
      if (afterText.length > 0) {
        newContent.push(mergeRunProperties({ ...content, content: [{ type: "text", value: afterText }] }, properties));
      }
      ...
      if (beforeText.length > 0) {
        newContent.push(mergeRunProperties({ ...content, content: [{ type: "text", value: beforeText }] }, properties));
      }
      if (end.offsetInRun < text.length) {
        const afterText = text.slice(end.offsetInRun);
        newContent.push({ ...content, content: [{ type: "text", value: afterText }] });
 ...
 */
function findPositionInRuns(runs: readonly DocxRun[], charOffset: number): { runIndex: number; offsetInRun: number } {
 ...
 */
function propertiesMatch(a: DocxRunProperties | undefined, b: DocxRunProperties | undefined): boolean {
  if (a === b) {
    return true;
  }
  if (!a || !b) {
 ...
 */
function replaceParagraphContentAtRunIndex(paragraph: DocxParagraph, runIndex: number, newRun: DocxRun): DocxParagraph {
 ...
 */
function spliceRunsAtIndex(paragraph: DocxParagraph, runIndex: number, ...newRuns: DocxRun[]): DocxParagraph {
  const contentIndex = findContentIndex(paragraph, runIndex);
  if (contentIndex < 0) {
    return paragraph;
  }
