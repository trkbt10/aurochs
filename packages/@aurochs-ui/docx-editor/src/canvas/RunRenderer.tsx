 ...
 */
export function computeRunStyles(properties: DocxRunProperties | undefined): CSSProperties {
  if (!properties) {
    return {};
  }
 ...
 */
export function RunContentRenderer({ content }: RunContentRendererProps): ReactNode {
  switch (content.type) {
    case "text":
  ...
  return (
    <span style={style}>
      {run.content.map((content, index) => (
        <RunContentRenderer key={index} content={content} properties={run.properties} />
      ))}
    </span>
  );
}
