...
import { halfPoints, twips } from "@aurochs-office/docx/domain/types";
...
describe("RunContentRenderer", () => {
  ...
  it("renders tab as whitespace", () => {
    const { container } = render(<RunContentRenderer content={{ type: "tab" }} />);
    const span = container.querySelector("span");
    expect(span?.style.whiteSpace).toBe("pre");
  });

  it("renders break as br element", () => {
    const { container } = render(<RunContentRenderer content={{ type: "break" }} />);
    expect(container.querySelector("br")).not.toBeNull();
  });

  it("renders page break with page-break-after", () => {
    const { container } = render(<RunContentRenderer content={{ type: "break", breakType: "page" }} />);
    const span = container.querySelector("span");
...
describe("RunRenderer", () => {
  ...
  it("renders multiple content items", () => {
    const run: DocxRun = {
      type: "run",
      content: [{ type: "text", value: "Hello" }, { type: "tab" }, { type: "text", value: "World" }],
    };
    const { container } = render(<RunRenderer run={run} />);
