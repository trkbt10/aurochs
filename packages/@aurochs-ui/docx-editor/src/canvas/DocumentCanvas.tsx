...
import { useCallback, useMemo, type ReactNode, type CSSProperties, type MouseEvent } from "react";
...
export type DocumentCanvasProps = {
  ...
  readonly renderBlockContent?: (element: DocxBlockContent, index: number) => ReactNode;
  /** Custom class name */
...
export function DocumentCanvas({
  ...
  const defaultRenderBlockContent = (element: DocxBlockContent, index: number): ReactNode => {
...
function renderPlaceholderContent(element: DocxBlockContent): ReactNode {
  switch (element.type) {
    case "paragraph": {
      ...
      return <p style={{ margin: 0, minHeight: "1em" }}>{text || "\u00A0"}</p>;
    }
    case "table": {
      return (
        <div
    ...
    default: {
      return (
        <div style={{ color: "var(--text-tertiary)", fontSize: "var(--font-size-md)" }}>[Unknown element type]</div>
      );
    }
  }
}
