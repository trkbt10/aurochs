...
import type { DocxTable, DocxTableProperties, DocxTableBorders } from "@aurochs-office/docx/domain/table";
...
export type TableRendererProps = {
  ...
  /** Cell click handler */
  readonly onCellClick?: (rowIndex: number, colIndex: number, event: MouseEvent) => void;
  /** Table click handler */
 ...
 */
function computeBorderStyle(edge: DocxTableBorders[keyof DocxTableBorders]): string {
  if (!edge) {
    return "none";
  }
 ...
 */
export function computeTableStyles(properties: DocxTableProperties | undefined): CSSProperties {
 ...
 */
export function TableRenderer({ table, elementId, isSelected, onCellClick, onClick }: TableRendererProps): ReactNode {
  ...
  return (
    <div style={containerStyle} onClick={handleClick} data-element-id={elementId}>
