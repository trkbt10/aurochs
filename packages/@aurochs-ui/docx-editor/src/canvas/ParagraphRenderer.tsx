 ...
 */
export function computeParagraphStyles(paragraph: DocxParagraph): CSSProperties {
 ...
 */
function renderParagraphContent(content: DocxParagraphContent, index: number): ReactNode {
  switch (content.type) {
    case "run":
...
export function ParagraphRenderer({
  ...
  const isEmpty =
    ...
    paragraph.content.every((c) => {
      ...
      return c.content.length === 0 || c.content.every((rc) => rc.type === "text" && rc.value === "");
    });

  return (
    <div style={containerStyle} onClick={handleClick} onDoubleClick={handleDoubleClick} data-element-id={elementId}>
      <p style={{ ...paragraphStyle, ...defaultRunStyle }}>
        {paragraph.content.map((content, index) => renderParagraphContent(content, index))}
