...
export function isPositionInRange(position: TextPosition, range: TextRange): boolean {
  ...
  return comparePositions(position, normalized.start) >= 0 && comparePositions(position, normalized.end) < 0;
}
...
export function rangesOverlap(a: TextRange, b: TextRange): boolean {
  ...
  return comparePositions(aNorm.start, bNorm.end) < 0 && comparePositions(bNorm.start, aNorm.end) < 0;
}
...
export function rangeContains(outer: TextRange, inner: TextRange): boolean {
  ...
  return comparePositions(outerNorm.start, innerNorm.start) <= 0 && comparePositions(outerNorm.end, innerNorm.end) >= 0;
}
...
export function mergeRanges(a: TextRange, b: TextRange): TextRange | undefined {
  ...
  const adjacent =
    rangesOverlap(a, b) || positionsEqual(aNorm.end, bNorm.start) || positionsEqual(bNorm.end, aNorm.start);
...
export function isSelectionEmpty(selection: DocxSelectionState): boolean {
  return !hasTextSelection(selection) && !hasElementSelection(selection) && selection.text.cursor === undefined;
}
 ...
 */
export function getParagraphAtIndex(document: DocxDocument, paragraphIndex: number): DocxParagraph | undefined {
...
export function getParagraphCount(document: DocxDocument): number {
  return document.body.content.filter((c): c is DocxParagraph => c.type === "paragraph").length;
}
 ...
 */
export function findRunAtOffset(paragraph: DocxParagraph, charOffset: number): [number, number] | undefined {
 ...
 */
export function movePositionForward(document: DocxDocument, position: TextPosition): TextPosition {
  const paragraph = getParagraphAtIndex(document, position.paragraphIndex);
  if (!paragraph) {
    return position;
  }
 ...
 */
export function movePositionBackward(document: DocxDocument, position: TextPosition): TextPosition {
 ...
 */
export function getPositionAtParagraphEnd(document: DocxDocument, paragraphIndex: number): TextPosition {
  const paragraph = getParagraphAtIndex(document, paragraphIndex);
  if (!paragraph) {
    return { paragraphIndex, charOffset: 0 };
  }
 ...
 */
export function getSelectedBodyContent(document: DocxDocument, selection: ElementSelectionState): DocxBlockContent[] {
