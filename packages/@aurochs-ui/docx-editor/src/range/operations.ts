...
import { normalizeRange, getParagraphAtIndex, isRangeCollapsed, clampPosition } from "../selection/utils";
...
export function deleteRange(document: DocxDocument, range: TextRange): DocxDocument {
  ...
  const firstModified = deleteTextRange(firstParagraph, start.charOffset, getParagraphText(firstParagraph).length);
 ...
 */
export function splitParagraphAtPosition(document: DocxDocument, position: TextPosition): DocxDocument {
 ...
 * @returns Updated document.
 */
export function mergeParagraphWithNext(document: DocxDocument, paragraphIndex: number): DocxDocument {
 ...
 * @returns Updated document.
 */
export function insertParagraphAtIndex(document: DocxDocument, index: number, paragraph?: DocxParagraph): DocxDocument {
  const newParagraph = paragraph ?? createParagraph();

  const newContent = [...document.body.content.slice(0, index), newParagraph, ...document.body.content.slice(index)];
 ...
 */
function updateParagraphInDocument(document: DocxDocument, index: number, paragraph: DocxParagraph): DocxDocument {
  const newContent = document.body.content.map((content, i) => (i === index ? paragraph : content));
 ...
 */
export function deleteForward(document: DocxDocument, position: TextPosition): DocxDocument {
  const paragraph = getParagraphAtIndex(document, position.paragraphIndex);
  if (!paragraph) {
    return document;
  }
