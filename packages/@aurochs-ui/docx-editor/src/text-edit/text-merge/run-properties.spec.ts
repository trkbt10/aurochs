...
import { halfPoints, docxRelId } from "@aurochs-office/docx/domain/types";
import { areRunPropertiesEqual, getBaseRunProperties, getRunPropertiesAtPosition } from "./run-properties";

// =============================================================================
// Test Fixtures
// =============================================================================

function createParagraphWithRuns(runs: Array<{ text: string; properties?: DocxRunProperties }>): DocxParagraph {
  return {
    type: "paragraph",
...
describe("areRunPropertiesEqual", () => {
  ...
  it("compares font family property", () => {
    expect(areRunPropertiesEqual({ rFonts: { ascii: "Arial" } }, { rFonts: { ascii: "Arial" } })).toBe(true);
    expect(areRunPropertiesEqual({ rFonts: { ascii: "Arial" } }, { rFonts: { ascii: "Times" } })).toBe(false);
  });

  it("compares color property", () => {
    expect(areRunPropertiesEqual({ color: { val: "FF0000" } }, { color: { val: "FF0000" } })).toBe(true);
    expect(areRunPropertiesEqual({ color: { val: "FF0000" } }, { color: { val: "0000FF" } })).toBe(false);
  });

  it("compares vertAlign property", () => {
    expect(areRunPropertiesEqual({ vertAlign: "superscript" }, { vertAlign: "superscript" })).toBe(true);
    expect(areRunPropertiesEqual({ vertAlign: "superscript" }, { vertAlign: "subscript" })).toBe(false);
  });
...
describe("getBaseRunProperties", () => {
  it("returns properties from first run", () => {
    const paragraph = createParagraphWithRuns([{ text: "Bold", properties: { b: true } }, { text: "Normal" }]);
    expect(getBaseRunProperties(paragraph)?.b).toBe(true);
  });

  it("skips runs without properties", () => {
    const paragraph = createParagraphWithRuns([{ text: "Normal" }, { text: "Bold", properties: { b: true } }]);
    expect(getBaseRunProperties(paragraph)?.b).toBe(true);
  });
