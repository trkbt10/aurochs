...
function getTextFromRun(run: DocxRun): string {
  return run.content.map((c) => (c.type === "text" ? c.value : "")).join("");
}
...
describe("mergeAdjacentRuns", () => {
  it("merges runs with same properties", () => {
    const content: DocxRun[] = [createTextRun("Hello", { b: true }), createTextRun(" World", { b: true })];
  ...
  it("does not merge runs with different properties", () => {
    const content: DocxRun[] = [createTextRun("Bold", { b: true }), createTextRun("Italic", { i: true })];
    const result = mergeAdjacentRuns(content);
  ...
  it("does not merge runs across non-run content", () => {
    const content = [createTextRun("A"), { type: "bookmarkStart" as const, id: 1, name: "test" }, createTextRun("B")];
    const result = mergeAdjacentRuns(content);
