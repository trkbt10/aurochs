...
import { colorTokens, editorLayoutTokens } from "@aurochs-ui/ui-components/design-tokens";
 ...
 */
function computeFinalHeight(accumulatedY: number, pageGap: number, pages: PagedLayoutResult["pages"]): number {
  if (accumulatedY > 0) {
 ...
 */
export const DocumentTextOverlay = forwardRef<SVGSVGElement, DocumentTextOverlayProps>(function DocumentTextOverlay(
  {
    pagedLayout,
    cursorState,
    isFocused = false,
  ...
  return (
    <div className="docx-text-overlay" style={overlayContainerStyle}>
      ...
      >
        ...
        <rect x={0} y={0} width={totalWidth} height={totalHeight} fill="transparent" pointerEvents="all" />
 ...
 */
export function SelectionHighlight({ rects, color = "rgba(59, 130, 246, 0.3)" }: SelectionHighlightProps): ReactNode {
  if (rects.length === 0) {
    return null;
  }
 ...
 */
function FloatingImagesLayer({ images, pageYOffset, className }: FloatingImagesLayerProps): ReactNode {
  if (images.length === 0) {
    return null;
  }
