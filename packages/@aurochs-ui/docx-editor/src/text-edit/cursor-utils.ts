...
import { getParagraphPlainText } from "@aurochs-office/docx/adapters";
import { editorLayoutTokens } from "@aurochs-ui/ui-components/design-tokens";
import { getAscenderRatio, PT_TO_PX } from "@aurochs/glyph";
 ...
 */
export function offsetToCursorPosition(paragraphs: readonly DocxParagraph[], offset: number): ContinuousCursorPosition {
 ...
 */
function getCharOffsetInLine(
  line: LayoutLine,
  targetInline: number,
  _writingMode: WritingMode = "horizontal-tb",
): number {
 ...
 */
function getLayoutParagraphTextLength(paragraph: PagedLayoutResult["pages"][number]["paragraphs"][number]): number {
 ...
 * @param writingMode - Writing mode for coordinate interpretation
 */
function findCursorCoordinatesInParagraph(args: {
  readonly paragraph: PagedLayoutResult["pages"][number]["paragraphs"][number];
  readonly charOffset: number;
  readonly pageYOffset: number;
  ...
  return lineToCoordinates({
    line: lastLine,
    offsetInLine: getLineTextLength(lastLine.spans),
    pageYOffset,
    writingMode,
  });
}
 ...
 * @param writingMode - Writing mode for coordinate interpretation
 */
function lineToCoordinates(args: {
 ...
 * @param writingMode - Writing mode for coordinate interpretation
 */
function calculateLineDistance(args: {
...
function findClosestLine(
  ...
  return allCandidates.reduce((best, candidate) => (candidate.distance < best.distance ? candidate : best));
}
 ...
 */
function createLineSelectionRect(args: {
  readonly line: LayoutLine;
 ...
 */
function getParagraphSelectionRects(args: {
  readonly paragraph: PagedLayoutResult["pages"][number]["paragraphs"][number];
  readonly pageYOffset: number;
 ...
 */
function findCurrentLineIndex(flatLines: readonly FlatLineInfo[], position: ContinuousCursorPosition): number {
 ...
 */
export function moveCursorVertically(args: {
