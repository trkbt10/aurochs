...
import type { DocxRun, DocxRunContent, DocxRunProperties } from "@aurochs-office/docx/domain/run";
import type { DocxStyles } from "@aurochs-office/docx/domain/styles";
...
function resolveFontSpec(
  ...
  const fontSizePx = merged?.sz !== undefined ? halfPointsToPx(merged.sz) : DEFAULT_FONT_SIZE_PX;
 ...
 */
export function layoutParagraphText(paragraph: DocxParagraph, bounds: DOMRect, styles?: DocxStyles): LayoutResult {
...
export function getCursorCoordinates(
  ...
  if (layout.lines.length === 0) {
    const defaultFontSizePx =
      defaultRunProperties?.sz !== undefined ? halfPointsToPx(defaultRunProperties.sz) : DEFAULT_FONT_SIZE_PX;
 ...
 */
function findCharOffsetInSpan(span: SpanLayout, targetX: number, font: FontSpec): number {
...
export function DocxTextOverlay({
  ...
  const layout = useMemo(() => layoutParagraphText(paragraph, bounds, styles), [paragraph, bounds, styles]);
