...
import { getLineTextLength } from "@aurochs-office/text-layout";
 ...
 */
export function useContinuousCursor({ pagedLayout }: UseContinuousCursorOptions): ContinuousCursorResult {
   ...
   * Check if two positions are equal.
   */
  const isSamePosition = useCallback((a: ContinuousCursorPosition, b: ContinuousCursorPosition): boolean => {
   ...
   */
  const isBefore = useCallback((a: ContinuousCursorPosition, b: ContinuousCursorPosition): boolean => {
  ...
  const selectionToRects = useCallback(
    (selection: ContinuousSelection): readonly SelectionRect[] => {
      ...
      for (let pIdx = startPara; pIdx <= endPara; pIdx++) {
        ...
        const paraEndOffset = pIdx === endPara ? normalized.focus.charOffset : getParagraphTextLength(paragraph);
