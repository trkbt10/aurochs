 ...
 */
export function PointEditor({ value, onChange, disabled }: PointEditorProps) {
  return (
    <FieldRow>
      <FieldGroup label="X" style={fieldStyle}>
        <PixelsEditor value={value.x} onChange={(x) => onChange({ ...value, x })} disabled={disabled} />
      </FieldGroup>
      <FieldGroup label="Y" style={fieldStyle}>
        <PixelsEditor value={value.y} onChange={(y) => onChange({ ...value, y })} disabled={disabled} />
      </FieldGroup>
    </FieldRow>
  );
}
