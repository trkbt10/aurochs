...
import type { Scene3d } from "@aurochs-office/pptx/domain";
 ...
 */
export function Scene3dEditor({ value, onChange, disabled }: Scene3dEditorProps) {
  ...
  return (
    <>
      ...
      <Camera3dEditor value={value.camera} onChange={(camera) => onChange({ ...value, camera })} disabled={disabled} />
      ...
      <Toggle checked={!!value.backdrop} onChange={handleBackdropToggle} label="Backdrop" disabled={disabled} />
      {value.backdrop && (
        <Backdrop3dEditor
