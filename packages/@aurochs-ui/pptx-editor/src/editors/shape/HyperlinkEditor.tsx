...
import type { Hyperlink } from "@aurochs-office/pptx/domain/types";
 ...
 */
export function HyperlinkEditor({ value, onChange, disabled, toggleLabel = "Enable" }: HyperlinkEditorProps) {
  ...
  const handleFieldChange = <K extends keyof Hyperlink>(field: K, newValue: Hyperlink[K]) => {
    if (!value) {
      return;
    }
  ...
  return (
    <>
      <Toggle checked={!!value} onChange={handleToggle} label={toggleLabel} disabled={disabled} />
      {value && (
        <>
