...
import type { Backdrop3d } from "@aurochs-office/pptx/domain";
 ...
 */
export function Backdrop3dEditor({ value, onChange, disabled }: Backdrop3dEditorProps) {
  return (
    <>
      <FieldGroup label="Anchor">
        <PointEditor value={value.anchor} onChange={(anchor) => onChange({ ...value, anchor })} disabled={disabled} />
      </FieldGroup>
      <FieldGroup label="Normal">
        <PointEditor value={value.normal} onChange={(normal) => onChange({ ...value, normal })} disabled={disabled} />
      </FieldGroup>
      <FieldGroup label="Up Vector">
        <PointEditor value={value.up} onChange={(up) => onChange({ ...value, up })} disabled={disabled} />
      </FieldGroup>
    </>
  );
}
