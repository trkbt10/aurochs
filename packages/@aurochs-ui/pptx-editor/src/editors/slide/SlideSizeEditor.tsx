  ...
  return (
    <div style={{ ...containerStyle, ...style }}>
      ...
      <FieldGroup label="Preset">
        <Select value={value.type ?? ""} onChange={handleTypeChange} options={SLIDE_SIZE_OPTIONS} disabled={disabled} />
      </FieldGroup>
...
function getAspectRatioText(ratio: number): string {
  ...
  if (Math.abs(ratio - 16 / 10) < tolerance) {
    return "16:10";
  }
  if (Math.abs(ratio - 4 / 3) < tolerance) {
    return "4:3";
  }
  if (Math.abs(ratio - 3 / 2) < tolerance) {
    return "3:2";
  }
  if (Math.abs(ratio - 1) < tolerance) {
    return "1:1";
  }
  if (Math.abs(ratio - 21 / 9) < tolerance) {
    return "21:9";
  }
