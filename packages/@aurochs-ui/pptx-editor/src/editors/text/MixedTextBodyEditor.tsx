 ...
 */
function applyRunPropertiesToAll(textBody: TextBody, update: Partial<RunProperties>): TextBody {
 ...
 */
function applyParagraphPropertiesToAll(textBody: TextBody, update: Partial<ParagraphProperties>): TextBody {
  const newParagraphs = textBody.paragraphs.map((paragraph): Paragraph => {
...
export function MixedTextBodyEditor({
  ...
  const allRunProperties = useMemo(() => getAllRunProperties(value), [value]);

  const allParagraphProperties = useMemo(() => getAllParagraphProperties(value), [value]);

  // Compute Mixed properties
  const mixedRunProperties = useMemo(() => extractMixedRunProperties(allRunProperties), [allRunProperties]);
