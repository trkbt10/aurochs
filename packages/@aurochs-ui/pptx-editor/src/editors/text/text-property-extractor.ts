 ...
 */
function findRunAtOffset(paragraph: Paragraph, offset: number): { runIndex: number; offsetInRun: number } | undefined {
...
export function getRunsInSelection(textBody: TextBody, selection: TextSelection): readonly RunRange[] {
  ...
  for (let pIdx = start.paragraphIndex; pIdx <= end.paragraphIndex; pIdx++) {
    ...
    const paragraphRanges = getRunRangesInParagraph({
      paragraph,
      paragraphIndex: pIdx,
 ...
 */
export function getSelectionForCursor(textBody: TextBody, position: CursorPosition): TextSelection | undefined {
 ...
 */
export function extractPropertiesAtCursor(textBody: TextBody, position: CursorPosition): ExtractedTextProperties {
 ...
 */
export function extractPropertiesFromTextBody(textBody: TextBody): ExtractedTextProperties {
  // Collect all runs
 ...
 */
export function extractTextProperties(textBody: TextBody, context: TextSelectionContext): ExtractedTextProperties {
  switch (context.type) {
    case "none":
      return createEmptyExtraction();
