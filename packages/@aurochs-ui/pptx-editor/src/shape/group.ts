 ...
 */
export function transformChildToSlideCoords(childTransform: Transform, groupTransform: GroupTransform): Transform {
  ...
  const newX = (groupTransform.x as number) + ((childTransform.x as number) - (childOffX as number)) * scaleX;
  const newY = (groupTransform.y as number) + ((childTransform.y as number) - (childOffY as number)) * scaleY;
 ...
 */
export function transformSlideToChildCoords(slideTransform: Transform, groupTransform: GroupTransform): Transform {
  const childExtX = groupTransform.childExtentWidth ?? groupTransform.width;
  const childExtY = groupTransform.childExtentHeight ?? groupTransform.height;
  const childOffX = groupTransform.childOffsetX ?? px(0);
  const childOffY = groupTransform.childOffsetY ?? px(0);
  ...
  const newX = (childOffX as number) + ((slideTransform.x as number) - (groupTransform.x as number)) / scaleX;
  const newY = (childOffY as number) + ((slideTransform.y as number) - (groupTransform.y as number)) / scaleY;
...
export function transformGroupToSlideCoords(
  ...
  const converted = transformChildToSlideCoords(childGroupTransform as Transform, parentGroupTransform);
...
export function transformGroupToChildCoords(
  ...
  const converted = transformSlideToChildCoords(childGroupTransform as Transform, parentGroupTransform);
...
export function findGroupById(
  ...
  const index = shapes.findIndex((s) => s.type === "grpSp" && s.nonVisual.id === groupId);
  if (index === -1) {
    return undefined;
  }
...
export function extractChildIds(children: readonly Shape[]): ShapeId[] {
  return children.filter((s) => "nonVisual" in s).map((s) => (s as Shape & { nonVisual: { id: string } }).nonVisual.id);
}
...
export function ungroupShape(
  ...
  const newShapes = [...shapes.slice(0, index), ...transformedChildren, ...shapes.slice(index + 1)];
 ...
 */
export function createGroupShape(groupId: ShapeId, transform: GroupTransform, children: readonly Shape[]): GrpShape {
  return {
    type: "grpSp",
    nonVisual: {
      id: groupId,
...
export function groupShapes(
  ...
  const idSet = new Set(shapeIds);
  const newShapes = shapes.filter((s) => !hasShapeId(s) || !idSet.has(s.nonVisual.id));
