...
function insertAtDisplayIndex(shapes: readonly Shape[], shape: Shape, displayIndex: number): readonly Shape[] {
...
function updateShapeTransform(shape: Shape, transform: Transform): Shape {
  switch (shape.type) {
    case "sp":
    case "pic":
    case "cxnSp":
...
function updateGroupTransform(shape: GrpShape, transform: GroupTransform): GrpShape {
  return {
    ...shape,
    properties: {
      ...shape.properties,
...
function isInvalidDropTarget(shapes: readonly Shape[], shapeId: ShapeId, targetParentId: ShapeId | null): boolean {
  if (!targetParentId) {
    return false;
  }
...
function getTargetGroup(shapes: readonly Shape[], targetParentId: ShapeId | null): GrpShape | undefined {
  if (!targetParentId) {
    return undefined;
  }
...
export function moveShapeInHierarchy(
  ...
  const sourceIndex = sourceSiblings.findIndex((shape) => hasShapeId(shape) && shape.nonVisual.id === shapeId);
  if (sourceIndex === -1) {
    return undefined;
  }
  ...
  const nextShape = adjustShapeTransformForMove(removedResult.removed, sourceParent, targetParent);
