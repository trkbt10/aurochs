...
import { colorTokens, fontTokens, iconTokens } from "@aurochs-ui/ui-components/design-tokens";
...
function isShapeLocked(shape: Shape): boolean {
  switch (shape.type) {
    ...
    case "graphicFrame":
      return shape.nonVisual.graphicFrameLocks?.noMove === true || shape.nonVisual.graphicFrameLocks?.noResize === true;
    default:
      return false;
  }
}
  ...
  switch (shape.type) {
    case "sp":
      return {
        ...
        nonVisual: {
          ...shape.nonVisual,
          shapeLocks: setLockFields(shape.nonVisual.shapeLocks, ["noMove", "noResize", "noRot"], locked),
        },
      };
    case "pic":
      return {
        ...
        nonVisual: {
          ...shape.nonVisual,
          pictureLocks: setLockFields(shape.nonVisual.pictureLocks, ["noMove", "noResize", "noRot"], locked),
        },
      };
    case "grpSp":
      return {
        ...
        nonVisual: {
          ...shape.nonVisual,
          groupLocks: setLockFields(shape.nonVisual.groupLocks, ["noMove", "noResize", "noRot"], locked),
        },
      };
    case "graphicFrame":
      return {
        ...
        nonVisual: {
          ...shape.nonVisual,
          graphicFrameLocks: setLockFields(shape.nonVisual.graphicFrameLocks, ["noMove", "noResize"], locked),
        },
      };
    default:
      return shape;
...
function ShapeList({
  ...
  if (shapes.length === 0) {
    return (
      <div style={listStyle} onClick={onEmptyClick} onDragOver={onListDragOver} onDrop={onListDrop}>
...
export function LayerPanel({
  ...
  const canHide = useMemo(() => selectedShapes.some((shape) => !isShapeHidden(shape)), [selectedShapes]);
  const canShow = useMemo(() => selectedShapes.some((shape) => isShapeHidden(shape)), [selectedShapes]);
  const canLock = useMemo(() => selectedShapes.some((shape) => !isShapeLocked(shape)), [selectedShapes]);
  const canUnlock = useMemo(() => selectedShapes.some((shape) => isShapeLocked(shape)), [selectedShapes]);
  ...
  const handleDragStart = useCallback((shapeId: ShapeId, event: DragEvent) => {
