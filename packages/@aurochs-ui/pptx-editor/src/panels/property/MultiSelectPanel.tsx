...
function SelectionHeader({
  ...
  return (
    <div style={headerStyle}>
      ...
      <div style={typeStyle}>{isSameType ? `All ${commonType}` : "Mixed types"}</div>
    </div>
  );
}
    ...
    <Accordion title="Transform" defaultExpanded>
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", padding: "12px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
          ...
          <MixedInput label="Y" value={commonTransform.y} onChange={(y) => onTransformChange({ y: px(y) })} />
          <MixedInput
            label="Width"
 ...
 */
export function MultiSelectPanel({ shapes, onShapeChange }: MultiSelectPanelProps) {
  // Analyze shape types
  ...
  return (
    <div>
      ...
      <MultiTransformSection commonTransform={commonTransform} onTransformChange={handleTransformChange} />

      {canEditFill && <MultiFillSection commonFill={commonFill} onFillChange={handleFillChange} />}

      {canEditLine && <MultiLineSection commonLine={commonLine} onLineChange={handleLineChange} />}
    </div>
  );
}
