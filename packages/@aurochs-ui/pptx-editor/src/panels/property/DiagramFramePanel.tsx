 ...
 * - Diagram content
 */
export function DiagramFramePanel({ shape, onChange, diagramData, onDiagramChange }: DiagramFramePanelProps) {
  ...
  const renderDiagramContent = () => {
    ...
    if (diagramData?.shapes && diagramData.shapes.length > 0) {
      return (
        ...
        >
          Diagram with {diagramData.shapes.length} shapes (data model not available)
        </div>
      );
    }
