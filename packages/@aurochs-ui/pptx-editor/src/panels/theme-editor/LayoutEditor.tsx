...
import type { PresentationFile, SlideSize, Shape } from "@aurochs-office/pptx/domain";
...
import { CheckIcon, AddIcon } from "@aurochs-ui/ui-components/icons";
...
export function LayoutEditor({
  ...
  useEffect(() => {
    if (presentationFile && availableLayouts.length > 0 && !layoutEdit.activeLayoutPath) {
    ...
    return (
      <div style={containerStyle}>
        <div style={emptyStateStyle}>
          ...
          <span style={{ fontSize: fontTokens.size.sm }}>Load a presentation to edit layouts</span>
        </div>
      </div>
    );
  }
      ...
      <div style={sidebarStyle}>
        ...
        <div style={layoutListStyle}>
          {availableLayouts.map((layout) => (
            <div key={layout.id} style={getLayoutItemStyle(layout.id)} onClick={() => handleSelectLayout(layout.id)}>
              {/* Mini thumbnail */}
              <div
                style={{
                  width: 40,
              ...
              {currentLayoutPath === layout.id && <CheckIcon size={12} style={{ color: colorTokens.accent.primary }} />}
            </div>
          ))}
        </div>
      </div>
