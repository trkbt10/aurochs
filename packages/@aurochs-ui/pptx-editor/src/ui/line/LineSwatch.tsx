...
const containerStyle = ({ size, hasOnClick, disabled, selected }: ContainerStyleInput): CSSProperties => {
  return {
    position: "relative",
    width: `${size}px`,
    height: `${size}px`,
...
export function LineSwatch({
  ...
  const scaledWidth = useMemo(() => getScaledWidth(line.width, sizeValue), [line.width, sizeValue]);
