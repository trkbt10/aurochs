...
/**
 * Convert generic DrawingPath (from @aurochs-ui/path-tools) to PPTX DrawingPath (with Pixels)
 */
 ...
 */
function isSmooth(handleIn: GenericPoint | undefined, anchor: GenericPoint, nextCmd: PathCommand | undefined): boolean {
  if (!handleIn || !nextCmd) {
    return false;
  }
...
export function customGeometryToDrawingPath(
  ...
  // Scale the commands
  const scaledCommands = geoPath.commands.map((cmd) => scaleCommand(cmd, scaleX, scaleY));
 ...
 */
export function isPathElementSelected(selection: PathPointSelection, element: PathElementId): boolean {
 ...
 */
export function addPointToSelection(selection: PathPointSelection, element: PathElementId): PathPointSelection {
 ...
 */
export function togglePointInSelection(selection: PathPointSelection, element: PathElementId): PathPointSelection {
  if (isPathElementSelected(selection, element)) {
    // Remove from selection
    const filtered = selection.selectedElements.filter((e) => !pathElementIdsEqual(e, element));
 ...
 */
export function pathElementIdsEqual(a: PathElementId, b: PathElementId): boolean {
  return (
    a.shapeId === b.shapeId &&
