...
import { createInitialHoverState, hoverReducer, type ItemHoverState } from "./hooks/item-hover-logic";
import { calculateTargetIndexFromGap, isValidGapDrop, createDragStartState } from "./drag-drop";
...
describe("Hover Operation Invariants", () => {
  ...
  function getHoveredIndices(list: HoverableList): number[] {
    return list.items.map((item, i) => (item.hover.isHovered ? i : -1)).filter((i) => i >= 0);
  }
