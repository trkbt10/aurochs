...
export type UseSlideSelectionResult = {
  ...
  readonly handleClick: (slideId: SlideId, index: number, event: React.MouseEvent | React.KeyboardEvent) => void;
...
export function useSlideSelection(options: UseSlideSelectionOptions): UseSlideSelectionResult {
  ...
  const [selection, setSelectionState] = useState<SlideSelectionState>(initialSelection ?? createEmptySlideSelection());
  ...
  const isSelected = useCallback((slideId: SlideId) => isSelectedFn(selection, slideId), [selection]);
