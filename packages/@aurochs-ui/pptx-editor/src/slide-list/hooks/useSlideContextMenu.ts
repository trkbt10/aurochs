 ...
 */
export function useSlideContextMenu(options: UseSlideContextMenuOptions): UseSlideContextMenuResult {
  const { slides, selectedIds, onDeleteSlides, onDuplicateSlides, onMoveSlides } = options;
  ...
  const openContextMenu = useCallback((x: number, y: number, slideId: SlideId) => {
  ...
  const handleMenuAction = useCallback(
    (actionId: string) => {
      ...
      switch (actionId) {
        ...
        case SLIDE_LIST_MENU_ACTIONS.MOVE_UP: {
          const minIndex = Math.min(...effectiveIds.map((id) => slides.findIndex((s) => s.id === id)));
          if (minIndex > 0) {
        ...
        case SLIDE_LIST_MENU_ACTIONS.MOVE_DOWN: {
          const maxIndex = Math.max(...effectiveIds.map((id) => slides.findIndex((s) => s.id === id)));
    ...
    [getEffectiveSelection, slides, onDeleteSlides, onDuplicateSlides, onMoveSlides, closeContextMenu],
  );
  ...
  const getMenuItems = useCallback((): readonly MenuEntry[] => {
    ...
    const indices = effectiveIds.map((id) => slides.findIndex((s) => s.id === id));
