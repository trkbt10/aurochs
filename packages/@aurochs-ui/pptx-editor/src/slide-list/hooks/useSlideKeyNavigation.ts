 ...
 */
function getCurrentIndex(slides: readonly SlideWithId[], selection: SlideSelectionState): number {
 ...
 */
function isNavigationKey(key: string, orientation: SlideListOrientation): { isNext: boolean; isPrev: boolean } {
...
export function useSlideKeyNavigation(options: UseSlideKeyNavigationOptions): UseSlideKeyNavigationResult {
  const { slides, selection, orientation, onNavigate, onExtendSelection, enabled = true, containerRef } = options;
  ...
  const handleKey = useCallback(
    ...
    [enabled, slides, orientation, selection, onNavigate, onExtendSelection],
  );
