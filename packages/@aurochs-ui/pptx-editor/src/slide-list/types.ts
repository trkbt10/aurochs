 ...
 */
export function createSingleSlideSelection(slideId: SlideId, index: number): SlideSelectionState {
  return {
    selectedIds: [slideId],
    primaryId: slideId,
    anchorIndex: index,
...
export type SlideListItemProps = {
  ...
  readonly onItemClick: (slideId: SlideId, index: number, e: React.MouseEvent | React.KeyboardEvent) => void;
  ...
  readonly onItemTransitionChange?: (slideId: SlideId, transition: SlideTransition | undefined) => void;
