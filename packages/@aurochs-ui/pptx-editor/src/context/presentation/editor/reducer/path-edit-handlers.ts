 ...
 */
function findShapeById(state: PresentationEditorState, shapeId: ShapeId): Shape | undefined {
 ...
 * Join two paths
 */
const handleJoinPaths: ActionHandler<Extract<PresentationEditorAction, { type: "JOIN_PATHS" }>> = (state, _action) => {
  // Implementation deferred to path operations utilities
  return state;
};
 ...
 */
const handleSplitPath: ActionHandler<Extract<PresentationEditorAction, { type: "SPLIT_PATH" }>> = (state, _action) => {
  // Implementation deferred to path operations utilities
  return state;
};
