...
import type { PresentationEditorState, PresentationEditorAction, LayoutEditState } from "../types";
...
import { collectBoundsForIds, getCombinedBounds, getCombinedCenter } from "../../../../shape/bounds";
...
type SelectLayoutAction = Extract<PresentationEditorAction, { type: "SELECT_LAYOUT" }>;
type LoadLayoutShapesAction = Extract<PresentationEditorAction, { type: "LOAD_LAYOUT_SHAPES" }>;
type SelectLayoutShapeAction = Extract<PresentationEditorAction, { type: "SELECT_LAYOUT_SHAPE" }>;
type SelectMultipleLayoutShapesAction = Extract<PresentationEditorAction, { type: "SELECT_MULTIPLE_LAYOUT_SHAPES" }>;
type StartLayoutMoveAction = Extract<PresentationEditorAction, { type: "START_LAYOUT_MOVE" }>;
type StartLayoutResizeAction = Extract<PresentationEditorAction, { type: "START_LAYOUT_RESIZE" }>;
type StartLayoutRotateAction = Extract<PresentationEditorAction, { type: "START_LAYOUT_ROTATE" }>;
type PreviewLayoutMoveAction = Extract<PresentationEditorAction, { type: "PREVIEW_LAYOUT_MOVE" }>;
type PreviewLayoutResizeAction = Extract<PresentationEditorAction, { type: "PREVIEW_LAYOUT_RESIZE" }>;
type PreviewLayoutRotateAction = Extract<PresentationEditorAction, { type: "PREVIEW_LAYOUT_ROTATE" }>;
type DeleteLayoutShapesAction = Extract<PresentationEditorAction, { type: "DELETE_LAYOUT_SHAPES" }>;
type AddLayoutShapeAction = Extract<PresentationEditorAction, { type: "ADD_LAYOUT_SHAPE" }>;
type UpdateLayoutShapeAction = Extract<PresentationEditorAction, { type: "UPDATE_LAYOUT_SHAPE" }>;
 ...
 */
function updateLayoutEdit(state: PresentationEditorState, updates: Partial<LayoutEditState>): PresentationEditorState {
  return {
    ...state,
    layoutEdit: {
      ...state.layoutEdit,
...
function handleSelectLayout(state: PresentationEditorState, action: SelectLayoutAction): PresentationEditorState {
...
function handleSelectLayoutShape(
  ...
  if (action.addToSelection) {
    ...
    if (action.toggle && isAlreadySelected) {
      ...
      const newPrimaryId = getPrimaryIdAfterDeselect(layoutSelection.primaryId, action.shapeId, newSelectedIds);
      return updateLayoutEdit(state, {
        layoutSelection: {
...
function handleClearLayoutShapeSelection(state: PresentationEditorState): PresentationEditorState {
  return updateLayoutEdit(state, {
    layoutSelection: createEmptySelection(),
  });
}
...
function handleStartLayoutMove(state: PresentationEditorState, action: StartLayoutMoveAction): PresentationEditorState {
  const { layoutShapes, layoutSelection } = state.layoutEdit;
  if (layoutSelection.selectedIds.length === 0) {
    return state;
  }

  const initialBounds = collectBoundsForIds(layoutShapes as Shape[], layoutSelection.selectedIds);
...
function handleStartLayoutResize(
  ...
  const initialBoundsMap = collectBoundsForIds(layoutShapes as Shape[], selectedIds);
  const combinedBounds = getCombinedBounds(
    selectedIds.map((id) => findShapeById(layoutShapes as Shape[], id)).filter((s): s is Shape => s !== undefined),
  );
...
function handlePreviewLayoutRotate(
  ...
  const angleDelta = deg((action.currentAngle as number) - (state.layoutEdit.layoutDrag.startAngle as number));
  return updateLayoutEdit(state, {
    layoutDrag: {
      ...state.layoutEdit.layoutDrag,
...
function applyLayoutMoveCommit(state: PresentationEditorState): PresentationEditorState {
...
function applyLayoutResizeCommit(state: PresentationEditorState): PresentationEditorState {
  const { layoutDrag, layoutShapes } = state.layoutEdit;
...
function applyLayoutRotateCommit(state: PresentationEditorState): PresentationEditorState {
  const { layoutDrag, layoutShapes } = state.layoutEdit;
...
function handleCommitLayoutDrag(state: PresentationEditorState): PresentationEditorState {
...
function handleEndLayoutDrag(state: PresentationEditorState): PresentationEditorState {
  return updateLayoutEdit(state, {
    layoutDrag: createIdleDragState(),
  });
}
...
function handleAddLayoutShape(state: PresentationEditorState, action: AddLayoutShapeAction): PresentationEditorState {
  const { layoutShapes } = state.layoutEdit;
...
function handleUpdateLayoutShape(
  ...
  const { layoutShapes } = state.layoutEdit;
  const newShapes = updateShapeById(layoutShapes as Shape[], action.shapeId, action.updater);
