...
type UpdateTextBodyAction = Extract<PresentationEditorAction, { type: "UPDATE_TEXT_BODY" }>;
type UpdateTextBodyInEditAction = Extract<PresentationEditorAction, { type: "UPDATE_TEXT_BODY_IN_EDIT" }>;
type ApplyRunFormatAction = Extract<PresentationEditorAction, { type: "APPLY_RUN_FORMAT" }>;
type ApplyParagraphFormatAction = Extract<PresentationEditorAction, { type: "APPLY_PARAGRAPH_FORMAT" }>;
 ...
 */
function getActiveSlideForTextEdit(state: PresentationEditorState): ReturnType<typeof findSlideById> {
...
function handleEnterTextEdit(state: PresentationEditorState, action: EnterTextEditAction): PresentationEditorState {
...
function handleExitTextEdit(state: PresentationEditorState): PresentationEditorState {
  return {
    ...state,
...
function handleUpdateTextBody(state: PresentationEditorState, action: UpdateTextBodyAction): PresentationEditorState {
 ...
 * Creates an undo history entry.
 */
function handleApplyRunFormat(state: PresentationEditorState, action: ApplyRunFormatAction): PresentationEditorState {
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, (slide) => ({
    ...slide,
    shapes: updateShapeById(slide.shapes, action.shapeId, (shape) => {
      if (shape.type !== "sp") {
