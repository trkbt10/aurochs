...
function addShapeToActiveSlide(
  ...
  const updatedSlides = document.slides.map((s) => (s.id === state.activeSlideId ? updatedSlide : s));
 ...
 */
const handleStartPenDraw: ActionHandler<Extract<PresentationEditorAction, { type: "START_PEN_DRAW" }>> = (state) => {
  return {
    ...state,
 ...
 */
const handleUpdatePenPointHandles: ActionHandler<
  Extract<PresentationEditorAction, { type: "UPDATE_PEN_POINT_HANDLES" }>
> = (state, action) => {
  if (!isPathDrawDrawing(state.pathDraw)) {
    return state;
  }
 ...
 * Close the pen path
 */
const handleClosePenPath: ActionHandler<Extract<PresentationEditorAction, { type: "CLOSE_PEN_PATH" }>> = (state) => {
  if (!isPathDrawDrawing(state.pathDraw)) {
    return state;
  }
 ...
 */
const handleCommitPenPath: ActionHandler<Extract<PresentationEditorAction, { type: "COMMIT_PEN_PATH" }>> = (state) => {
  if (!isPathDrawDrawing(state.pathDraw)) {
    return state;
  }
 ...
 * Cancel pen drawing
 */
const handleCancelPenPath: ActionHandler<Extract<PresentationEditorAction, { type: "CANCEL_PEN_PATH" }>> = (state) => {
  return {
    ...state,
    pathDraw: createIdlePathDrawState(),
    creationMode: createSelectMode(),
 ...
 */
const handleEndPencilDraw: ActionHandler<Extract<PresentationEditorAction, { type: "END_PENCIL_DRAW" }>> = (state) => {
  if (!isPathDrawPencil(state.pathDraw)) {
    return state;
  }
