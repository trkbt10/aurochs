 ...
 */
export function getActiveSlide(state: PresentationEditorState): SlideWithId | undefined {
 ...
 */
export function exitTextEditIfActive(state: PresentationEditorState): TextEditState {
 ...
 */
export function exitTextEditIfDifferentShape(state: PresentationEditorState, newShapeId: ShapeId): TextEditState {
  if (isTextEditActive(state.textEdit) && state.textEdit.shapeId !== newShapeId) {
    return createInactiveTextEditState();
  }
  return state.textEdit;
}
