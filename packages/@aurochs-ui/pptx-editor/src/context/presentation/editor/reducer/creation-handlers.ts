...
type AddPictureAction = Extract<PresentationEditorAction, { type: "ADD_PICTURE" }>;
type AddChartAction = Extract<PresentationEditorAction, { type: "ADD_CHART" }>;
type AddDiagramAction = Extract<PresentationEditorAction, { type: "ADD_DIAGRAM" }>;

/**
 * Get selection state for creation mode change
 */
function getSelectionForModeChange(mode: CreationMode, currentSelection: SelectionState): SelectionState {
  if (mode.type === "select") {
 ...
 */
function getSelectionForCreatedShape(shapeId: ShapeId | undefined, currentSelection: SelectionState): SelectionState {
  if (shapeId) {
...
function handleSetCreationMode(state: PresentationEditorState, action: SetCreationModeAction): PresentationEditorState {
  return {
    ...state,
    creationMode: action.mode,
    shapeSelection: getSelectionForModeChange(action.mode, state.shapeSelection),
  };
}

function handleCreateShape(state: PresentationEditorState, action: CreateShapeAction): PresentationEditorState {
...
function handleAddPicture(state: PresentationEditorState, action: AddPictureAction): PresentationEditorState {
...
function handleAddChart(state: PresentationEditorState, action: AddChartAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide) {
    return state;
  }
...
function handleAddDiagram(state: PresentationEditorState, action: AddDiagramAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide) {
    return state;
  }
