...
function handleCopy(state: PresentationEditorState): PresentationEditorState {
 ...
 */
function applyPasteOffset(shape: Shape, transform: ReturnType<typeof getShapeTransform>, offset: number): Shape {
  if (!transform) {
    return shape;
  }
...
function handlePaste(state: PresentationEditorState): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  ...
  for (const shape of state.clipboard.shapes) {
    ...
    // Update document
    currentDoc = updateActiveSlideInDocument(currentDoc, state.activeSlideId, (slide) => ({
      ...slide,
      shapes: [...slide.shapes, offsetShape],
    }));
  }
