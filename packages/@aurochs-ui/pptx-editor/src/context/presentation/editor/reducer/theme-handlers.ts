...
type SetEditorModeAction = Extract<PresentationEditorAction, { type: "SET_EDITOR_MODE" }>;

type UpdateColorSchemeAction = Extract<PresentationEditorAction, { type: "UPDATE_COLOR_SCHEME" }>;

type UpdateFontSchemeAction = Extract<PresentationEditorAction, { type: "UPDATE_FONT_SCHEME" }>;

type ApplyThemePresetAction = Extract<PresentationEditorAction, { type: "APPLY_THEME_PRESET" }>;

/**
 * Handle SET_EDITOR_MODE action
 */
function handleSetEditorMode(state: PresentationEditorState, action: SetEditorModeAction): PresentationEditorState {
 ...
 */
function applyThemePresetToDocument(doc: PresentationDocument, preset: ThemePreset): PresentationDocument {
  return {
    ...doc,
    colorContext: {
      ...doc.colorContext,
