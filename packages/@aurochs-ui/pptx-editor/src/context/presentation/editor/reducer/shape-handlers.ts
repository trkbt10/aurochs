...
type DeleteShapesAction = Extract<PresentationEditorAction, { type: "DELETE_SHAPES" }>;
type AddShapeAction = Extract<PresentationEditorAction, { type: "ADD_SHAPE" }>;
type ReorderShapeAction = Extract<PresentationEditorAction, { type: "REORDER_SHAPE" }>;
type UngroupShapeAction = Extract<PresentationEditorAction, { type: "UNGROUP_SHAPE" }>;
type GroupShapesAction = Extract<PresentationEditorAction, { type: "GROUP_SHAPES" }>;
type MoveShapeToIndexAction = Extract<PresentationEditorAction, { type: "MOVE_SHAPE_TO_INDEX" }>;
type MoveShapeInHierarchyAction = Extract<PresentationEditorAction, { type: "MOVE_SHAPE_IN_HIERARCHY" }>;
 ...
 */
function getSelectionForAddedShape(shapeId: ShapeId | undefined, currentSelection: SelectionState): SelectionState {
  if (shapeId) {
...
function handleUpdateActiveSlide(
  ...
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, action.updater);
  return {
    ...state,
...
function handleUpdateShape(state: PresentationEditorState, action: UpdateShapeAction): PresentationEditorState {
...
function handleDeleteShapes(state: PresentationEditorState, action: DeleteShapesAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  ...
  const remainingSelectedIds = state.shapeSelection.selectedIds.filter((id) => !idsToDelete.has(id));
  ...
  return {
    ...
    shapeSelection: {
      ...
      primaryId: getPrimaryIdAfterDeletion(remainingSelectedIds, state.shapeSelection.primaryId),
    },
  };
}

function handleAddShape(state: PresentationEditorState, action: AddShapeAction): PresentationEditorState {
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, (slide) => ({
    ...slide,
...
function handleReorderShape(state: PresentationEditorState, action: ReorderShapeAction): PresentationEditorState {
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, (slide) => ({
    ...slide,
...
function handleUngroupShape(state: PresentationEditorState, action: UngroupShapeAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide) {
    return state;
  }
...
function handleGroupShapes(state: PresentationEditorState, action: GroupShapesAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide) {
    return state;
  }
...
function handleMoveShapeToIndex(
  ...
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, (slide) => {
    ...
    const currentIndex = shapes.findIndex((s) => "nonVisual" in s && s.nonVisual.id === action.shapeId);
...
function handleMoveShapeInHierarchy(
  ...
  const nextShapes = moveShapeInHierarchy(activeSlide.slide.shapes, action.shapeId, action.target);
  if (!nextShapes) {
    return state;
  }
