...
function handleSelectShape(state: PresentationEditorState, action: SelectShapeAction): PresentationEditorState {
  ...
  if (action.addToSelection) {
    ...
    if (action.toggle && isAlreadySelected) {
      const newSelectedIds = state.shapeSelection.selectedIds.filter((id) => id !== action.shapeId);
      const newPrimaryId = getPrimaryIdAfterDeselect(state.shapeSelection.primaryId, action.shapeId, newSelectedIds);
      return {
        ...state,
...
function handleClearShapeSelection(state: PresentationEditorState): PresentationEditorState {
  return {
    ...state,
    textEdit: exitTextEditIfActive(state),
