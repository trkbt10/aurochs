...
type StartResizeAction = Extract<PresentationEditorAction, { type: "START_RESIZE" }>;
type StartRotateAction = Extract<PresentationEditorAction, { type: "START_ROTATE" }>;
type StartPendingMoveAction = Extract<PresentationEditorAction, { type: "START_PENDING_MOVE" }>;
type StartPendingResizeAction = Extract<PresentationEditorAction, { type: "START_PENDING_RESIZE" }>;
type StartPendingRotateAction = Extract<PresentationEditorAction, { type: "START_PENDING_ROTATE" }>;
type StartMarqueeAction = Extract<PresentationEditorAction, { type: "START_MARQUEE" }>;
type UpdateMarqueeAction = Extract<PresentationEditorAction, { type: "UPDATE_MARQUEE" }>;
type StartCreateDragAction = Extract<PresentationEditorAction, { type: "START_CREATE_DRAG" }>;
type UpdateCreateDragAction = Extract<PresentationEditorAction, { type: "UPDATE_CREATE_DRAG" }>;
// Note: EndDragAction type not needed since handler doesn't use action payload
type PreviewMoveAction = Extract<PresentationEditorAction, { type: "PREVIEW_MOVE" }>;
type PreviewResizeAction = Extract<PresentationEditorAction, { type: "PREVIEW_RESIZE" }>;
type PreviewRotateAction = Extract<PresentationEditorAction, { type: "PREVIEW_ROTATE" }>;
 ...
 */
function getPrimaryTransform(shape: Shape | undefined): ReturnType<typeof getShapeTransform> {
  if (!shape) {
    return undefined;
  }
...
function handleStartMove(state: PresentationEditorState, action: StartMoveAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide || state.shapeSelection.selectedIds.length === 0) {
    return state;
  }

  const initialBounds = collectBoundsForIds(activeSlide.slide.shapes, state.shapeSelection.selectedIds);
...
function handleStartResize(state: PresentationEditorState, action: StartResizeAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide || state.shapeSelection.selectedIds.length === 0) {
    return state;
  }
  ...
  const combinedBounds = getCombinedBounds(
    selectedIds.map((id) => findShapeById(activeSlide.slide.shapes, id)).filter((s): s is Shape => s !== undefined),
  );
...
function handleStartRotate(state: PresentationEditorState, action: StartRotateAction): PresentationEditorState {
  const activeSlide = getActiveSlide(state);
  if (!activeSlide || state.shapeSelection.selectedIds.length === 0) {
    return state;
  }
...
function handleEndDrag(state: PresentationEditorState): PresentationEditorState {
  return {
    ...state,
...
function handlePreviewMove(state: PresentationEditorState, action: PreviewMoveAction): PresentationEditorState {
...
function handlePreviewResize(state: PresentationEditorState, action: PreviewResizeAction): PresentationEditorState {
...
function handlePreviewRotate(state: PresentationEditorState, action: PreviewRotateAction): PresentationEditorState {
  if (state.drag.type !== "rotate") {
    return state;
  }
  const angleDelta = deg((action.currentAngle as number) - (state.drag.startAngle as number));
  return {
    ...state,
    drag: {
      ...state.drag,
 ...
 */
function applyMoveCommit(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "move") {
    return state;
  }
  ...
  const newDoc = updateActiveSlideInDocument(state.documentHistory.present, state.activeSlideId, () => newSlide);
 ...
 */
function applyResizeCommit(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "resize") {
    return state;
  }
  ...
  const { handle, initialBoundsMap, combinedBounds, aspectLocked, shapeIds } = state.drag;
 ...
 */
function applyRotateCommit(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "rotate") {
    return state;
  }
...
function handleCommitDrag(state: PresentationEditorState): PresentationEditorState {
  const { drag } = state;
...
function handleConfirmMove(state: PresentationEditorState): PresentationEditorState {
...
function handleConfirmResize(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "pending-resize") {
    return state;
  }

  const { handle, startX, startY, shapeIds, initialBoundsMap, combinedBounds, aspectLocked, shapeId, initialBounds } =
    state.drag;
...
function handleConfirmRotate(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "pending-rotate") {
    return state;
  }

  const { startAngle, shapeIds, initialRotationsMap, initialBoundsMap, centerX, centerY, shapeId, initialRotation } =
    state.drag;
...
function handleStartMarquee(state: PresentationEditorState, action: StartMarqueeAction): PresentationEditorState {
  return {
    ...state,
    drag: {
      type: "marquee",
...
function handleUpdateMarquee(state: PresentationEditorState, action: UpdateMarqueeAction): PresentationEditorState {
...
function handleEndMarquee(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "marquee") {
    return state;
  }
...
function handleStartCreateDrag(state: PresentationEditorState, action: StartCreateDragAction): PresentationEditorState {
  return {
    ...state,
    drag: {
      type: "create",
...
function handleEndCreateDrag(state: PresentationEditorState): PresentationEditorState {
  if (state.drag.type !== "create") {
    return state;
  }
