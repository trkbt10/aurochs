...
import type { Slide, SpShape, Shape } from "@aurochs-office/pptx/domain";
import { px, deg } from "@aurochs-office/drawing-ml/domain/units";
import { createSlideEditorState, type SlideEditorState, type SlideEditorAction } from "./types";
...
const dispatchAll = (initialState: SlideEditorState, actions: SlideEditorAction[]): SlideEditorState => {
  return actions.reduce((state, action) => slideEditorReducer(state, action), initialState);
};

// Helper to get shape ID from a shape (handles union type)
const getShapeId = (shape: Shape): string | undefined => ("nonVisual" in shape ? shape.nonVisual.id : undefined);
...
describe("SlideEditorContext Reducer - Copy/Paste", () => {
  ...
  it("PASTE offsets position each time", () => {
    ...
    expect(shape4.properties.transform!.x).not.toBe(shape5.properties.transform!.x);
  });
