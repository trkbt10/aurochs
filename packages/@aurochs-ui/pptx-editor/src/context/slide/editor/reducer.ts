 ...
 */
export function slideEditorReducer(state: SlideEditorState, action: SlideEditorAction): SlideEditorState {
  ...
  switch (action.type) {
    ...
    case "UPDATE_SHAPE": {
      const newShapes = updateShapeById(currentSlide.shapes, action.shapeId, action.updater);
    ...
    case "DELETE_SHAPES": {
      ...
      const remainingSelected = state.selection.selectedIds.filter((id) => !action.shapeIds.includes(id));
    ...
    case "REORDER_SHAPE": {
      const newShapes = reorderShape(currentSlide.shapes, action.shapeId, action.direction);
      const newSlide: Slide = { ...currentSlide, shapes: newShapes };
      return {
        ...state,
        slideHistory: pushHistory(state.slideHistory, newSlide),
    ...
    case "MOVE_SHAPE_TO_INDEX": {
      const currentIndex = currentSlide.shapes.findIndex((s) => "nonVisual" in s && s.nonVisual.id === action.shapeId);
      if (currentIndex === -1) {
        return state;
      }
      if (currentIndex === action.newIndex) {
        return state;
      }
    ...
    case "SELECT": {
      if (action.addToSelection) {
        const isAlreadySelected = state.selection.selectedIds.includes(action.shapeId);
        if (isAlreadySelected) {
          // Deselect
          const newSelected = state.selection.selectedIds.filter((id) => id !== action.shapeId);
          const newPrimaryId = getPrimaryIdAfterDeselect(newSelected, action.shapeId, state.selection.primaryId);
          return {
            ...state,
    ...
    case "START_MOVE": {
      ...
      const initialBounds = collectBoundsForIds(currentSlide.shapes, state.selection.selectedIds);
      return {
        ...state,
        drag: {
          type: "move",
    ...
    case "START_RESIZE": {
      ...
      const initialBoundsMap = collectBoundsForIds(currentSlide.shapes, state.selection.selectedIds);
    ...
    case "START_ROTATE": {
      ...
      const startAngle = deg(
        Math.atan2((action.startY as number) - (centerY as number), (action.startX as number) - (centerX as number)) *
          (180 / Math.PI),
      );
    ...
    case "PASTE": {
      ...
      const pastedShapes: Shape[] = state.clipboard.shapes.map((shape) => {
        const newId = generateShapeId([...currentSlide.shapes, ...state.clipboard!.shapes]);
