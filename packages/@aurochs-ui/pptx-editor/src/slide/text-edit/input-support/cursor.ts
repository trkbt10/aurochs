...
import type { LayoutResult, LayoutLine } from "@aurochs-renderer/pptx/text-layout";
import {
 ...
 */
function getCursorInLineCoordinates(line: LayoutLine, charOffset: number): CursorCoordinates {
 ...
 */
function getEndOfTextCoordinates(layoutResult: LayoutResult): CursorCoordinates | undefined {
...
export function selectionToRects(selection: TextSelection, layoutResult: LayoutResult): readonly SelectionRect[] {
  ...
  if (normalized.start.paragraphIndex === normalized.end.paragraphIndex) {
    ...
    if (startPara) {
      rects.push(...getSelectionRectsInParagraph(startPara, normalized.start.charOffset, normalized.end.charOffset));
    }
  } else {
 ...
 */
function getParagraphTextLength(para: { readonly lines: readonly LayoutLine[] }): number {
