...
import { Button, Popover } from "@aurochs-ui/ui-components/primitives";
import { FillPreview } from "@aurochs-ui/color-editor";
...
import type { GradientStop, SolidFill } from "@aurochs-office/drawing-ml/domain/fill";
import { pct, type Percent } from "@aurochs-office/drawing-ml/domain/units";
...
function buildGradientPreview(stops: readonly GradientStop[]): string {
  ...
  const gradientStops = sortedStops.map((stop) => `#${getHexFromColor(stop.color)} ${stop.position}%`).join(", ");
...
export function GradientStopsEditor({
  ...
  const handleStopChange = useCallback(
    (index: number, updatedStop: GradientStop) => {
      const newStops = value.map((stop, i) => (i === index ? updatedStop : stop));
      onChange(newStops);
    },
    [value, onChange],
  );
  ...
  const handleStopPointerUp = useCallback((index: number, event: PointerEvent<HTMLDivElement>) => {
    const dragState = dragStateRef.current;
    if (!dragState || dragState.index !== index || dragState.pointerId !== event.pointerId) {
      return;
    }
  ...
  const handleStopPointerCancel = useCallback((index: number, event: PointerEvent<HTMLDivElement>) => {
    const dragState = dragStateRef.current;
    if (!dragState || dragState.index !== index || dragState.pointerId !== event.pointerId) {
      return;
    }
  ...
  return (
    <div style={{ ...containerStyle, ...style }} className={className}>
      ...
      {/* Empty State */}
      {value.length === 0 && <div style={emptyMessageStyle}>Click on the bar above to add color stops</div>}

      {/* Add Stop Button */}
      {value.length > 0 && value.length < maxStops && (
        <div style={actionsStyle}>
          <Button variant="secondary" onClick={handleAddStop} disabled={disabled}>
            + Add Stop
          </Button>
        </div>
      )}
