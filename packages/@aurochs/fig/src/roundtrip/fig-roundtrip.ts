...
function parseClientMeta(raw: Record<string, unknown>): FigMetadata["clientMeta"] | undefined {
  const clientMeta = raw.client_meta as Record<string, unknown> | undefined;
  if (!clientMeta) {
    return undefined;
  }
  return {
    backgroundColor: clientMeta.background_color as FigMetadata["clientMeta"] extends { backgroundColor?: infer T }
      ? T
      : never,
    thumbnailSize: clientMeta.thumbnail_size as FigMetadata["clientMeta"] extends { thumbnailSize?: infer T }
      ? T
      : never,
    renderCoordinates: clientMeta.render_coordinates as FigMetadata["clientMeta"] extends {
...
function buildClientMeta(clientMeta: FigMetadata["clientMeta"]): Record<string, unknown> | undefined {
  if (!clientMeta) {
    return undefined;
  }
  return {
