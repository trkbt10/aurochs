/**
 * @file Immutable XML mutation helpers
 *
 * All functions produce new objects without mutating the original.
 * This is the SoT for immutable XML AST operations.
 */

import type { XmlElement, XmlNode, XmlDocument } from "./ast";
import { isXmlElement } from "./ast";
...
/** Replace the first child element with the given name. If not found, returns the parent unchanged. */
...
/** Update a child element by name using an updater function. If the child doesn't exist, returns parent unchanged. */
export function updateChildByName(
  ...
  return {
    ...
    children: parent.children.map((child) => {
      if (isXmlElement(child) && child.name === name) { return updater(child); }
      return child;
    }),
  };
}
...
export function findElements(root: XmlElement, predicate: (el: XmlElement) => boolean): XmlElement[] {
  ...
  for (const child of root.children) {
    if (isXmlElement(child)) { results.push(...findElements(child, predicate)); }
  }
  return results;
}
...
/** Update an element at a path. Path is an array of element names to traverse. Returns a new tree with the updated element. */
export function updateAtPath(
  ...
  return {
    ...
    children: root.children.map((child, index) => {
      if (index !== firstIndex) { return child; }
