...
type ClearCellContentsAction = Extract<
  XlsxEditorAction,
  { type: "CLEAR_CELL_CONTENTS" }
>;
type ClearCellFormatsAction = Extract<
  XlsxEditorAction,
...
function handleUpdateCell(
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    ...
    sheetIndex,
    (worksheet) => updateCell(worksheet, action.address, action.value),
  );
...
function handleUpdateCells(
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    ...
    sheetIndex,
    (worksheet) =>
      action.updates.reduce(
        (current, update) => updateCell(current, update.address, update.value),
        worksheet,
      ),
  );
...
function handleDeleteCells(
  state: XlsxEditorState,
  action: DeleteCellsAction,
): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    state.workbookHistory.present,
    sheetIndex,
    (worksheet) => deleteCellRange(worksheet, action.range),
  );
...
function handleSetCellFormula(
  state: XlsxEditorState,
  action: SetCellFormulaAction,
): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    state.workbookHistory.present,
    sheetIndex,
    (worksheet) => setCellFormula(worksheet, action.address, action.formula),
  );
...
function handleClearCellContents(
  state: XlsxEditorState,
  action: ClearCellContentsAction,
): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    state.workbookHistory.present,
    sheetIndex,
    (worksheet) => clearCellContents(worksheet, action.range),
  );
...
function handleClearCellFormats(
  state: XlsxEditorState,
  action: ClearCellFormatsAction,
): XlsxEditorState {
  const sheetIndex = state.activeSheetIndex;
  if (sheetIndex === undefined) {
    return state;
  ...
  const updatedWorkbook = updateWorksheetInWorkbook(
    state.workbookHistory.present,
    sheetIndex,
    (worksheet) => clearCellFormats(worksheet, action.range),
  );
