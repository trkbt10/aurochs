...
function computeMaxColumn(sheet: XlsxWorksheet): number {
  const cellMax = sheet.rows.reduce((acc, row) => row.cells.reduce((a, cell) => Math.max(a, cell.address.col), acc), 0);
  const mergeMax = (sheet.mergeCells ?? []).reduce((acc, r) => Math.max(acc, r.end.col), 0);
...
function computeMaxRow(sheet: XlsxWorksheet): number {
  ...
  const mergeMax = (sheet.mergeCells ?? []).reduce((acc, r) => Math.max(acc, r.end.row), 0);
