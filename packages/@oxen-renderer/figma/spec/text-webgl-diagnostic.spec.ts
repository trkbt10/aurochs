...
describe("Text node diagnostics (real .fig data)", () => {
  ...
  it("samples contour data from a successful text node", () => {
    ...
    for (const { frameName, node } of allTextNodes) {
      ...
      console.log(`  Fill: rgba(${node.fill.color.r.toFixed(2)}, ${node.fill.color.g.toFixed(2)}, ${node.fill.color.b.toFixed(2)}, ${node.fill.opacity})`);
      console.log(`  Transform: [${node.transform.m00.toFixed(2)}, ${node.transform.m01.toFixed(2)}, ${node.transform.m02.toFixed(2)}, ${node.transform.m10.toFixed(2)}, ${node.transform.m11.toFixed(2)}, ${node.transform.m12.toFixed(2)}]`);
